...
use crate::transaction::amount::{Amount, IouValue, Currency};
...
///   Currently returns tecPATH_DRY for unsupported path configurations.
    ...
    match deliver_amount {
        Amount::Iou { value, currency, issuer } => {
            ...
            let is_cross_currency = match send_max {
                Some(Amount::Iou { currency: sm_cur, .. }) => sm_cur != currency,
            ...
            if !is_cross_currency && !has_paths {
                ...
                RippleCalcResult { success: false, ter: "tecPATH_DRY" }
            } else {
        ...
        _ => RippleCalcResult { success: false, ter: "tecPATH_DRY" },
    }
}
...
fn direct_iou_payment(
    ...
    let send_amount = if transfer_rate > QUALITY_ONE && *sender != *issuer && *destination != *issuer {
    ...
    if sender_balance.mantissa <= 0 && amount.mantissa > 0 {
        ...
        if *sender != *issuer {
            return RippleCalcResult { success: false, ter: "tecPATH_DRY" };
        }
    }
    ...
    RippleCalcResult { success: true, ter: "tesSUCCESS" }
}
...
fn apply_transfer_rate(amount: &IouValue, rate: u32, round_up: bool) -> IouValue {
    ...
    let mut v = IouValue { mantissa: result as i64, exponent: amount.exponent };
    v.normalize();
    v
}
