...
fn credential_key(subject: &[u8; 20], issuer: &[u8; 20], credential_type: &[u8]) -> crate::ledger::Key {
...
fn sle_flags(sle: &crate::ledger::meta::ParsedSLE) -> u32 {
    sle.fields
        ...
        .map(|field| u32::from_be_bytes([field.data[0], field.data[1], field.data[2], field.data[3]]))
        .unwrap_or(0)
}
...
pub(crate) fn apply_credential_create(
    ...
    let flags: u32 = if subject == tx.account { LSF_ACCEPTED } else { 0 };
