        ...
        let (target_peers, total_reqs, _trigger_sync_seq) = if should_issue_reply_followup(
            ...
            if !reqs.is_empty() {
                if target_peers.is_empty() {
                    ...
                    // A NuDB-backed walk can surface a single frontier probe.
                    // Send that probe to a few peers so one slow/duplicate
                    // response does not starve the acquisition pipe.
                    let fanout = target_peers.len().min(3);
                    for &pid in target_peers.iter().take(fanout) {
                        total_reqs += self.sync_send_request(&reqs[0], sync_seq, Some(pid)).await;
                    }
                    if total_reqs == 0 {
                        total_reqs += self.sync_send_request(&reqs[0], sync_seq, None).await;
                    }
                } else {
