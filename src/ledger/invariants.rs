...
/// (key, before_bytes) pairs.  We look up current (after) state from `state`.
///
...
pub fn check_invariants(
    ...
    // Build deltas
    let deltas: Vec<EntryDelta> = touched.iter().map(|(key, before)| {
...
fn field_data_len(type_code: u8, _field_code: u8, data: &[u8], pos: usize) -> Option<usize> {
    match type_code {
        ...
        4 => Some(16),          // Hash128
        5 => Some(32),          // Hash256
        6 => {                  // Amount — 8 bytes XRP, 48 IOU, 33 MPT
        ...
        7 | 8 | 19 => {         // VL-encoded: Blob, AccountID, Vector256
            let (vl_len, vl_header) = crate::ledger::meta::decode_vl_length(data, pos);
            Some(vl_header + vl_len)
        }
        14 => {                 // STObject — scan to end marker 0xE1
            let mut p = pos;
            while p < data.len() && data[p] != 0xE1 { p += 1; }
            if p < data.len() { Some(p - pos + 1) } else { None }
        }
        15 => {                 // STArray — scan to end marker 0xF1
            let mut p = pos;
...
fn check_xrp_not_created(deltas: &[EntryDelta], fee: u64) -> Option<&'static str> {
    ...
    for delta in deltas {
        let entry_type = delta.after.as_deref()
        ...
        let before_drops = delta.before.as_deref()
            .map(|d| sle_xrp_held(d, entry_type))
            .unwrap_or(0);

        let after_drops = delta.after.as_deref()
            .map(|d| sle_xrp_held(d, entry_type))
            .unwrap_or(0);
...
fn check_xrp_balance_valid(deltas: &[EntryDelta]) -> Option<&'static str> {
    for delta in deltas {
        ...
        if let Some(balance) = sle_amount_field(data, 2) { // sfBalance = field 2
            if balance < 0 {
...
fn check_account_roots_not_deleted(
    ...
    let allows_delete = tx.tx_type == 21 || tx.tx_type == 40 || tx.tx_type == 67 || tx.tx_type == 75;
...
fn check_no_bad_offers(deltas: &[EntryDelta]) -> Option<&'static str> {
    for delta in deltas {
        ...
        if sle_entry_type(after) != Some(OFFER_TYPE) { continue; }
        ...
        if let Some(pays) = sle_amount_field(after, 4) {
            if pays < 0 { return Some("NoBadOffers: negative TakerPays"); }
        }
        if let Some(gets) = sle_amount_field(after, 5) {
            if gets < 0 { return Some("NoBadOffers: negative TakerGets"); }
        }
    }
    None
}
...
fn check_no_zero_escrow(deltas: &[EntryDelta]) -> Option<&'static str> {
    for delta in deltas {
        ...
        if et == ESCROW_TYPE || et == PAYCHAN_TYPE {
            if let Some(amt) = sle_amount_field(after, 1) { // sfAmount
                if amt <= 0 { return Some("NoZeroEscrow: zero or negative amount"); }
            }
        }
    }
    None
...
fn check_no_xrp_trust_lines(deltas: &[EntryDelta]) -> Option<&'static str> {
    for delta in deltas {
        ...
        if sle_entry_type(after) != Some(RIPPLE_STATE_TYPE) { continue; }
...
mod tests {
    ...
    fn dummy_tx(fee: u64) -> ParsedTx {
        crate::transaction::parse::ParsedTx {
            tx_type: 0, account: [1u8; 20], sequence: 1, fee, flags: 0,
            amount_drops: Some(100), destination: Some([2u8; 20]),
            amount: None, limit_amount: None, taker_pays: None, taker_gets: None,
    ...
    fn account_not_deleted_by_payment() {
        ...
        let tx = dummy_tx(12);
        assert!(check_account_roots_not_deleted(
            &deltas, ter::TES_SUCCESS, &tx
        ).is_some());
    }
    ...
    fn account_deleted_by_account_delete_ok() {
        ...
        assert_eq!(check_account_roots_not_deleted(
            &deltas, ter::TES_SUCCESS, &tx
        ), None);
    }
}
