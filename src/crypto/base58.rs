...
pub fn decode_account(address: &str) -> Result<[u8; 20], Base58Error> {
    ...
    if payload.len() != 20 {
        return Err(Base58Error::WrongLength { expected: 20, got: payload.len() });
    }
    let mut out = [0u8; 20];
...
pub fn decode_seed(seed: &str) -> Result<[u8; 16], Base58Error> {
    ...
    if payload.len() != 16 {
        return Err(Base58Error::WrongLength { expected: 16, got: payload.len() });
    }
    let mut out = [0u8; 16];
    out.copy_from_slice(&payload);
    Ok(out)
...
mod tests {
    ...
    fn test_genesis_account() {
        ...
        assert!(addr.starts_with('r'), "address must start with r, got {addr}");
    }
    ...
    fn test_account_roundtrip() {
        let id: [u8; 20] = [
            0x5E, 0x7B, 0x0F, 0xA1, 0x23, 0x45, 0x67, 0x89,
    ...
    fn test_known_address() {
        ...
        assert!(result.is_ok(), "failed to decode known address: {:?}", result);
    }
    ...
    fn test_seed_roundtrip() {
        ...
        assert!(encoded.starts_with('s'), "seed must start with s, got {encoded}");
