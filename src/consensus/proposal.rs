...
impl Proposal {
    ...
    /// internally, so we return the raw pre-hash payload here.
    ...
    pub fn new_signed(
        ...
        let mut p = Self::new_unsigned(
            ledger_seq, tx_set_hash, previous_ledger, close_time, prop_seq,
            kp.public_key_bytes(),
        );
...
mod tests {
    ...
    fn test_signing_bytes_includes_prefix() {
        ...
        assert_eq!(&bytes[..4], &[0x50, 0x52, 0x50, 0x00], "must start with PRP prefix");
    }
