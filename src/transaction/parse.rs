...
pub struct ParsedTx {
    ...
    /// Payment `Amount` in drops (XRP only for now).
...
fn parse_signers_array(data: &[u8]) -> Result<Vec<SignerEntry>, ParseError> {
    ...
    while pos < data.len() {
        ...
        signers.push(SignerEntry {
            ...
            signing_pubkey: signing_pubkey.ok_or(ParseError::MissingField("Signer.SigningPubKey"))?,
...
impl Default for ParsedTx {
    fn default() -> Self {
        Self {
            tx_type: 0, flags: 0, sequence: 0, fee: 0,
            account: [0u8; 20], destination: None,
            amount_drops: None, amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, deliver_min: None,
            offer_sequence: None, finish_after: None, cancel_after: None,
            settle_delay: None, expiration: None,
            set_flag: None, clear_flag: None, transfer_rate: None,
            tick_size: None, last_ledger_seq: None,
            ticket_count: None, ticket_sequence: None,
            domain: None, channel: None, public_key: None,
            paychan_sig: None, nftoken_id: None,
            nft_sell_offer: None, nft_buy_offer: None,
            uri: None, did_document: None, did_data: None,
            nftoken_taxon: None, transfer_fee_field: None,
            asset_scale: None, maximum_amount: None,
            mutable_flags: None, mptoken_metadata: None,
            owner: None, regular_key: None,
            ...
            authorize: None, permissions_raw: None,
            holder: None, mptoken_issuance_id: None,
            asset: None, asset2: None, vault_id: None,
            amendment: None,
            base_fee_field: None, reserve_base_field: None,
            reserve_increment_field: None,
            unl_modify_disabling: None, unl_modify_validator: None,
            send_max: None, paths: Vec::new(),
            signing_pubkey: Vec::new(), signature: Vec::new(),
            signing_hash: [0u8; 32], signing_payload: Vec::new(),
            signers: Vec::new(),
        }
    }
}
...
pub fn parse_blob(data: &[u8]) -> Result<ParsedTx, ParseError> {
    ...
    while pos < data.len() {
        ...
        let (type_code, field_code) = if top == 0 && bot == 0 {
            // Both extended
            if pos + 2 > data.len() { return Err(ParseError::Truncated("field header (both ext)")); }
            ...
            // Type extended
            if pos >= data.len() { return Err(ParseError::Truncated("field header (type ext)")); }
            let t = data[pos] as u16;
            pos += 1;
            (t, bot)
        } else if bot == 0 {
            // Field extended
            if pos >= data.len() { return Err(ParseError::Truncated("field header (field ext)")); }
        ...
        match type_code {
            1 => {
                // UInt16 (2 bytes)
                if pos + 2 > data.len() { return Err(ParseError::Truncated("UInt16")); }
            ...
            2 => {
                // UInt32 (4 bytes)
                if pos + 4 > data.len() { return Err(ParseError::Truncated("UInt32")); }
            ...
            3 => {
                // UInt64 (8 bytes)
                if pos + 8 > data.len() { return Err(ParseError::Truncated("UInt64")); }
            ...
            4 => {
                ...
                if pos + 16 > data.len() { return Err(ParseError::Truncated("Hash128")); }
                pos += 16;
            }
            5 => {
                // Hash256 (32 bytes)
                if pos + 32 > data.len() { return Err(ParseError::Truncated("Hash256")); }
                {
                    let mut h = [0u8; 32];
            ...
            6 => {
                ...
                if pos >= data.len() { return Err(ParseError::Truncated("Amount")); }
                let (amt, consumed) = Amount::from_bytes(&data[pos..])
                    .map_err(|_| ParseError::Truncated("Amount data"))?;
                match field_code {
                    1 => {
                        // sfAmount — XRP or IOU
                        if let Amount::Xrp(d) = &amt { amount_drops = Some(*d); }
                        amount_full = Some(amt);
                    }
                    3  => { limit_amount = Some(amt); }  // sfLimitAmount
                    4  => { taker_pays  = Some(amt); }  // sfTakerPays
                    5  => { taker_gets  = Some(amt); }  // sfTakerGets
                    9  => { send_max    = Some(amt); }  // sfSendMax
                    10 => { deliver_min = Some(amt); }  // sfDeliverMin
                    8 => {
                        // sfFee (always XRP)
                        if let Amount::Xrp(d) = &amt { fee = Some(*d); }
                    }
                    _ => {}
                }
                pos += consumed;
            ...
            7 => {
                // Blob (VL-encoded)
                if pos >= data.len() { return Err(ParseError::Truncated("Blob VL")); }
                let (vlen, ladv) = decode_length(&data[pos..]);
                pos += ladv;
                if pos + vlen > data.len() { return Err(ParseError::Truncated("Blob data")); }
                let blob = data[pos..pos + vlen].to_vec();
                match field_code {
            ...
            8 => {
                ...
                if pos >= data.len() { return Err(ParseError::Truncated("AccountID VL")); }
                let (vlen, ladv) = decode_length(&data[pos..]);
                pos += ladv;
                if pos + vlen > data.len() { return Err(ParseError::Truncated("AccountID data")); }
                if vlen == 20 {
                    let mut id = [0u8; 20];
                    id.copy_from_slice(&data[pos..pos + 20]);
                    match field_code {
            ...
            9 | 11 => {
                ...
                if pos + 8 > data.len() { return Err(ParseError::Truncated("Int64")); }
                pos += 8;
            }
            10 => {
                // INT32 — 4 bytes
                if pos + 4 > data.len() { return Err(ParseError::Truncated("Int32")); }
                pos += 4;
            }
            14 => {
                ...
                if pos < data.len() { pos += 1; } // skip 0xE1
            }
            15 => {
                ...
                if pos < data.len() { pos += 1; } // skip 0xF1
            ...
            16 => {
                // UInt8 — 1 byte
                if pos >= data.len() { return Err(ParseError::Truncated("UInt8")); }
                match field_code {
            ...
            17 => {
                // Hash160 — 20 bytes
                if pos + 20 > data.len() { return Err(ParseError::Truncated("Hash160")); }
                pos += 20;
            }
            18 => {
                ...
                while pos < data.len() && data[pos] != 0x00 {
                    ...
                    let ptype = data[pos]; pos += 1;
                    let mut step = PathStep { account: None, currency: None, issuer: None };
                ...
                if pos < data.len() { pos += 1; } // skip 0x00 terminator
            }
            19 => {
                // Vector256 — VL-encoded
                if pos >= data.len() { return Err(ParseError::Truncated("Vector256")); }
                let (vlen, ladv) = decode_length(&data[pos..]);
                pos += ladv + vlen;
            }
            20 => {
                // UINT96 — 12 bytes
                if pos + 12 > data.len() { return Err(ParseError::Truncated("UINT96")); }
                pos += 12;
            }
            21 => {
                // UINT192 — 24 bytes
                if pos + 24 > data.len() { return Err(ParseError::Truncated("UINT192")); }
                if field_code == 1 {
            ...
            22 => {
                // UINT384 — 48 bytes
                if pos + 48 > data.len() { return Err(ParseError::Truncated("UINT384")); }
                pos += 48;
            }
            23 => {
                // UINT512 — 64 bytes
                if pos + 64 > data.len() { return Err(ParseError::Truncated("UINT512")); }
                pos += 64;
            }
            24 => {
                // ISSUE — variable size: XRP (20), IOU (40), MPT (44)
                if pos + 20 > data.len() { return Err(ParseError::Truncated("ISSUE")); }
                if let Some((issue, consumed)) = crate::transaction::amount::Issue::from_bytes(&data[pos..]) {
                    match field_code {
            ...
            26 => {
                // CURRENCY — 20 bytes
                if pos + 20 > data.len() { return Err(ParseError::Truncated("CURRENCY")); }
                pos += 20;
            }
            _ => {
    ...
    Ok(ParsedTx {
        ...
        signer_quorum,
        signer_entries_raw,
        domain_id: domain_id_val, ledger_fix_type,
        ...
        // neither.  For replay of validated txs we don't need signatures at all.
...
mod tests {
    ...
    use crate::transaction::serialize::{Field, FieldValue, serialize_fields};
    ...
    fn multisigned_payment_blob() -> Vec<u8> {
        ...
        let mut fields = vec![
            Field { def: field::TRANSACTION_TYPE, value: FieldValue::UInt16(0) },
            Field { def: field::FLAGS, value: FieldValue::UInt32(0) },
            Field { def: field::SEQUENCE, value: FieldValue::UInt32(1) },
            Field { def: field::AMOUNT, value: FieldValue::Amount(Amount::Xrp(1_000_000)) },
            Field { def: field::FEE, value: FieldValue::Amount(Amount::Xrp(12)) },
            Field { def: field::SIGNING_PUB_KEY, value: FieldValue::Blob(Vec::new()) },
            Field { def: field::ACCOUNT, value: FieldValue::AccountID(genesis_id()) },
            Field { def: field::DESTINATION, value: FieldValue::AccountID(dest_id()) },
        ];
    ...
    fn test_signing_hash_matches_and_signature_verifies() {
        ...
        assert!(ok, "signature in blob must verify against recomputed signing hash");
    }
    ...
    fn test_tampered_blob_fails_verification() {
        ...
        // If parse failed that's also fine — we just can't accept the tx
    }
