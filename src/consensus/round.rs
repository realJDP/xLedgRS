...
//! | >= 200 %         | 95 %      |
//!
//! Any tx set hash held by >= threshold of UNL is adopted as our new position.
//!
//! # Consensus state machine (`check_consensus`)
//!
//! Returns one of four states each establish iteration:
//! - **No** — keep waiting.
//! - **Yes** — 80 %+ of current proposers agree with us.
//! - **MovedOn** — 80 %+ of peers validated a later ledger without us.
...
pub fn convergence_threshold(converge_percent: u32) -> f64 {
    if      converge_percent < 50  { 0.50 }
    else if converge_percent < 85  { 0.65 }
    else if converge_percent < 200 { 0.70 }
    else                           { 0.95 }
}
...
pub enum ConsensusState {
    ...
    No,
    /// We reached 80 %+ agreement with current proposers.
    Yes,
    /// Network validated past this ledger without us (80 %+ finished).
    MovedOn,
...
pub struct ConsensusRound {
    ...
    /// Our own current position (tx set hash we're proposing).
    pub our_position:  Option<[u8; 32]>,
    /// Parent ledger hash we're building on.
    ...
    /// Set of *master* pubkeys we trust (our UNL).
    unl:               Vec<Vec<u8>>,
...
impl ConsensusRound {
    pub fn new(
        ...
        Self {
            ...
            mode:            if proposing { ConsensusMode::Proposing } else { ConsensusMode::Observing },
            our_position:    None,
            prev_ledger,
    ...
    pub fn unl_size(&self) -> usize { self.unl.len() }
    pub fn proposal_count(&self) -> usize { self.proposals.len() }
    pub fn validation_count(&self) -> usize { self.validations.len() }
    ...
    pub fn add_proposal(&mut self, prop: Proposal) -> bool {
        if !self.is_trusted(&prop.node_pubkey) { return false; }
        if prop.ledger_seq != self.ledger_seq  { return false; }
        if !prop.verify_signature()            { return false; }
        let key = hex::encode(&prop.node_pubkey);
        // Only accept if prop_seq is newer
        if let Some(existing) = self.proposals.get(&key) {
            if prop.prop_seq <= existing.prop_seq { return false; }
        }
    ...
    /// Compare our tx set hash with a peer's and create disputes for differences.
    /// Called when a peer proposes a different tx_set_hash than ours.
    ///
    /// `our_txs`: transaction hashes in our proposed set.
    /// `peer_txs`: transaction hashes in the peer's proposed set.
    /// `peer_id`: hex-encoded peer node pubkey.
    pub fn create_disputes(
        &mut self,
        our_txs: &[[u8; 32]],
        peer_txs: &[[u8; 32]],
        peer_id: &str,
        ...
        // Txs in ours but not peer's → dispute (we say yes, they say no)
        for tx_id in our_set.difference(&peer_set) {
            self.add_dispute(*tx_id, true);
            self.set_dispute_vote(tx_id, peer_id, false);
        }

        // Txs in peer's but not ours → dispute (we say no, they say yes)
    ...
    pub fn converge_percent(&self) -> u32 {
        let elapsed_ms = self.establish_start
    ...
    pub fn establish_elapsed(&self) -> Duration {
        self.establish_start.map(|s| s.elapsed()).unwrap_or(Duration::ZERO)
    }

    /// Try to converge: adopt the most popular position if it meets the
    /// current threshold. Returns `Some(new_position)` if we changed.
    pub fn try_converge(&mut self) -> Option<[u8; 32]> {
        if self.phase != ConsensusPhase::Establish { return None; }
        let unl_size = self.unl.len();
        if unl_size == 0 { return None; }
        ...
        if ratio >= threshold {
            ...
            if changed { return Some(best_hash); }
        }
        None
    }
    ...
    pub fn check_consensus(&mut self) -> ConsensusState {
        ...
        let (agree, total) = if let Some(pos) = self.our_position {
            let agree = self.proposals.values()
        ...
        // Check: have we reached consensus?
        ...
        // Check: has the network moved on without us?
        // Count peers who validated a *later* ledger (seq > our current).
        let finished = self.validations.values()
    ...
    /// Declare consensus reached on our current position.
    pub fn accept(&mut self) -> Option<RoundResult> {
        if self.phase != ConsensusPhase::Establish { return None; }
        let tx_set_hash = self.our_position?;
        let agree_count = self.proposals.values()
    ...
    /// Accepts validations for our ledger seq (for quorum) AND later seqs
    /// (for MovedOn detection — network validated past us).
    /// Returns `true` if it was from a trusted node with a valid signature.
    pub fn add_validation(&mut self, val: Validation) -> bool {
        if !self.is_trusted(&val.node_pubkey) { return false; }
        if val.ledger_seq < self.ledger_seq   { return false; }
        if !val.verify_signature()            { return false; }
    ...
    pub fn check_validated(&mut self) -> Option<[u8; 32]> {
        if self.unl.is_empty() { return None; }
    ...
    pub fn add_dispute(&mut self, tx_id: [u8; 32], our_vote: bool) {
        self.disputes.entry(tx_id).or_insert_with(|| DisputedTx::new(tx_id, our_vote));
    }
    ...
    /// Returns list of tx_ids where our vote changed.
    ...
    /// Get the current set of transactions we'd include (our_vote == true).
    ...
    pub fn dispute_count(&self) -> usize { self.disputes.len() }

    // ── Bow-out / seqLeave ──────────────────────────────────────────────────

    /// Bow out of consensus — stop proposing and broadcast seqLeave.
    /// Returns the bow-out proposal to broadcast, if we were proposing.
    pub fn leave_consensus(&mut self) -> Option<[u8; 32]> {
        if self.mode != ConsensusMode::Proposing { return None; }
    ...
    /// Returns `Some(correct_parent)` if we should switch, None if we're OK.
    pub fn check_wrong_ledger(&self) -> Option<[u8; 32]> {
        ...
        let (best_parent, best_count) = parent_counts.into_iter()
            .max_by_key(|(_, c)| *c)?;
        // If majority (>50%) builds on a different parent, we're wrong
        let total = self.proposals.len();
        if total > 0 && best_parent != self.prev_ledger
            && best_count * 2 > total
        {
            Some(best_parent)
        } else {
    ...
    pub fn check_close_time_consensus(&mut self) -> Option<u64> {
        ...
        let (&best_time, &best_count) = self.close_time_votes.iter()
            .max_by_key(|(_, &c)| c)?;
    ...
    pub fn have_full_consensus(&self) -> bool {
        // TX consensus: 80%+ agree on our position
        if let Some(pos) = self.our_position {
            let agree = self.proposals.values()
                .filter(|p| p.tx_set_hash == pos)
                .count();
            let unl_size = self.unl.len();
            if unl_size == 0 { return self.have_close_time_consensus; }
...
mod tests {
    ...
    fn test_ephemeral_proposal_accepted_via_manifest() {
        ...
        assert!(r.add_proposal(prop), "ephemeral key with manifest should be trusted");
    }
    ...
    fn test_revoked_master_key_not_trusted() {
        ...
        assert!(!r.add_proposal(prop1), "revoked master's ephemeral must be rejected");

        let prop2 = Proposal::new_signed(1, tx_hash(1), [0u8; 32], 0, 0, &masters[0]);
        assert!(!r.add_proposal(prop2), "revoked master key directly must be rejected");
    }
    ...
    fn test_no_manifest_uses_direct_master() {
        ...
        let mut r = new_round(1, unl_from(&validators), [0u8; 32], true);
        let prop = make_proposal(&validators[0], tx_hash(1), 0);
        assert!(r.add_proposal(prop), "direct master key must still work without manifest");
    }
    ...
    fn test_converge_percent_relative_to_prev_round() {
        ...
        let mut r = ConsensusRound::new(
            1, unl_from(&validators), [0u8; 32], true,
        ...
        // We can't easily test time-dependent values, but we can test the formula
        // by checking the denominator logic: max(prev_round_time, AV_MIN_CONSENSUS_TIME)
    }
    ...
    fn test_check_consensus_returns_no_before_min_consensus() {
        ...
        // But we just started — should return No (< 1950ms)
        // The establish_start was just set, so elapsed ~ 0ms
    ...
    fn test_check_consensus_returns_expired_on_very_long_round() {
        ...
        r.close_ledger(tx_hash(1));
        // Add disagreeing proposals so agreement is below 80%
        for v in &validators {
        ...
        // With self: agree=1 (us), total=6 → 16% < 80%. Expired fires.
    ...
    fn test_check_consensus_moved_on() {
        ...
        r.close_ledger(tx_hash(1));
        r.establish_start = Some(Instant::now() - Duration::from_secs(3));
        // No proposals agreeing with us — so Yes won't fire.
        // But 4 of 5 peers validated seq 2 (later than our seq 1) → MovedOn.
        for v in &validators[..4] {
            let mut val = Validation::new_signed(2, tx_hash(99), 0, true, v);
            val.ledger_seq = 2; // later than our seq=1
            r.add_validation(val);
        }
        // 4 validated past us, 0 current proposers → consensus_reached(4, 0, false, false)
        // total=0 with reached_max=false → false. But wait — we need proposers.
        // Let's add some proposals too so total > 0.
        for v in &validators {
