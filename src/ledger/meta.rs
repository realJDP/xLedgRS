...
//! their raw binary field data. This is used by the ledger follower to update
//! the SHAMap with exact binary SLE payloads.
//!
//! Metadata is treated as a generic binary STObject. The parser walks the
//! encoded fields directly instead of relying on hard-coded offsets.
...
pub fn patch_sle(
    ...
    while pos < existing_sle.len() {
        ...
        if header_end > existing_sle.len() {
            break;
        }
        ...
        if data_end <= pos {
            tracing::warn!(
                "patch_sle: forward-progress stall at pos={pos} tc={tc} fc={fc}, breaking"
            );
    ...
    // Add any new fields from FinalFields that were absent from the original
    // SLE. Each field is inserted at the canonical position sorted by
    // `(type_code, field_code)`.
    for f in new_fields {
        if seen_fields.contains(&(f.type_code, f.field_code)) {
            continue;
        }
...
pub fn build_pre_tx_sle(
    ...
    let mut merged: Vec<ParsedField> = final_fields
        .iter()
        .map(|f| ParsedField {
    ...
    for pf in previous_fields {
        if let Some(existing) = merged
            .iter_mut()
            .find(|f| f.type_code == pf.type_code && f.field_code == pf.field_code)
        {
...
pub fn build_sle(
    ...
    let mut fields: Vec<ParsedField> = new_fields
        .iter()
        .map(|f| ParsedField {
            type_code: f.type_code,
            field_code: f.field_code,
    ...
    // Add common required fields with default values when they are missing.
    // rippled always serializes required fields even when the value is zero.
...
pub fn diff_previous_fields(pre: &[ParsedField], post: &[ParsedField]) -> Vec<ParsedField> {
    ...
    let post_map: HashMap<(u16, u16), &ParsedField> = post
        .iter()
    ...
    pre.iter()
        .filter(|field| {
            post_map
                .get(&(field.type_code, field.field_code))
...
mod tests {
    ...
    fn test_patch_sle_malformed_input_no_hang() {
        ...
        assert!(
            ...
            "patch_sle hung on truncated VL"
        );
        ...
        assert!(
            start.elapsed().as_millis() < 100,
            "patch_sle hung on unknown type"
        );
