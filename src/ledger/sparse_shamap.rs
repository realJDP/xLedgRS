...
fn nibble(key: &[u8; 32], depth: usize) -> usize {
    ...
    if depth % 2 == 0 { (byte >> 4) as usize } else { (byte & 0x0F) as usize }
}

impl SparseSHAMap {
    pub fn new() -> Self {
        Self { root: InnerNode::new(), count: 0 }
    }
...
mod tests {
    ...
    fn test_same_hash_as_full_shamap() {
        ...
        use crate::ledger::shamap::{SHAMap, Key};
