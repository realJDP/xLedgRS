...
pub async fn run_diagnose(
    ...
    // Object/leaf-hash iteration was removed with RocksDB.
    // State objects now live in NuDB nodestore — use --verify-hash for hash verification.
    println!("NOTE: Object-level diagnostics removed (RocksDB removed).");
    ...
    if let Some(ref endpoint) = rpc_endpoint {
                ...
                Ok(body) => {
                    if let Ok(resp) = serde_json::from_str::<serde_json::Value>(&body) {
                        ...
                        if let Some(v) = validated {
                            ...
                            println!("  our sync is {} ledgers behind", diff);
                        }
                    }
                }
        ...
        println!("No --rpc-sync endpoint specified, skipping network verification.");
        println!("Re-run with --rpc-sync <host:port> to compare against rippled.");
    }

    println!("\n=== Summary ===");
    println!("Storage stats: {} ledgers, {} transactions", stats.ledgers, stats.transactions);

    Ok(())
}

/// Definitive hash verification: stream ledger_data from RPC, compute root hash,
/// compare against the ledger's account_hash. This proves whether our leaf_hash
/// and SparseSHAMap code is correct — no storage involved.
pub async fn verify_hash_live(host: &str, port: u16) -> anyhow::Result<()> {
    ...
    let mut ledger_seq = result["ledger_index"].as_str()
        .and_then(|s| s.parse::<u64>().ok())
        .ok_or_else(|| anyhow::anyhow!("no ledger_index"))?;
    let mut expected_hash = result["account_hash"].as_str()
    ...
    loop {
        ...
        if result["status"].as_str() != Some("success") {
            ...
            if err == "lgrNotFound" || err == "internal" {
                println!("  Ledger {} pruned — restarting with fresh validated ledger...", ledger_seq);
                // Get new validated ledger
                let req = r#"{"method":"ledger","params":[{"ledger_index":"validated"}]}"#;
                let body = crate::rpc_sync::http_post(host, port, req).await?;
                let resp: serde_json::Value = serde_json::from_str(&body)?;
                let r = &resp["result"]["ledger"];
                ledger_seq = r["ledger_index"].as_str()
                    .and_then(|s| s.parse::<u64>().ok())
                    .ok_or_else(|| anyhow::anyhow!("no ledger_index on retry"))?;
                expected_hash = r["account_hash"].as_str()
        ...
        for obj in state {
            let index_hex = match obj["index"].as_str() { Some(s) => s, None => continue };
            let data_hex = match obj["data"].as_str() { Some(s) => s, None => continue };
    ...
    if hex::encode_upper(our_hash) == expected_hash {
        ...
        println!("\n*** MISMATCH — our hash computation has a bug. ***");
    }
