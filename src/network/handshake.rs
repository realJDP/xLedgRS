...
use crate::crypto::base58::{encode, decode, PREFIX_NODE_PUBLIC};
...
pub fn build_request_simple(node_pubkey: &[u8], session_signature: &[u8]) -> String {
    build_request(node_pubkey, session_signature, 0, &"0".repeat(64), &"0".repeat(64))
}
...
fn validate_and_extract(
    ...
    let upgrade = headers.get("upgrade").ok_or(HandshakeError::MissingHeader("Upgrade"))?;
    ...
    // Validate Connect-As
    let connect_as = headers.get("connect-as").ok_or(HandshakeError::MissingHeader("Connect-As"))?;
    ...
    let pubkey_b58 = headers.get("public-key").ok_or(HandshakeError::MissingHeader("Public-Key"))?;
    let (prefix, node_pubkey) = decode(pubkey_b58)
        .map_err(|_| HandshakeError::InvalidPublicKey(pubkey_b58.clone()))?;
    if prefix != PREFIX_NODE_PUBLIC {
        return Err(HandshakeError::InvalidPublicKey(pubkey_b58.clone()));
    }

    // Decode Session-Signature (base64)
    let sig_b64 = headers.get("session-signature").ok_or(HandshakeError::MissingHeader("Session-Signature"))?;
    let session_signature = B64.decode(sig_b64)
    ...
    let network_id = headers.get("network-id").and_then(|v| v.parse::<u32>().ok());
    let network_time = headers.get("network-time").and_then(|v| v.parse::<u64>().ok());
