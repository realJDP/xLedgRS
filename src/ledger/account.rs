...
/// The fields of an XRPL AccountRoot that we care about for `account_info`.
...
impl AccountRoot {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        // Actually: type=16 → top nibble would be 16 which is >=16, so we need extended type encoding
    ...
    pub fn to_sle_binary(&self) -> Vec<u8> {
        ...
        // Use SLE to patch only the fields we model
    ...
    pub fn decode(data: &[u8]) -> Result<Self, DecodeError> {
        ...
        while pos < data.len() {
            ...
            let (type_code, field_code) = if top == 0 && bot == 0 {
                // Both extended (we don't emit this, skip cleanly)
            ...
            match (type_code, field_code) {
                (1, 1) => {
                    // LedgerEntryType (UInt16) — skip, we already know it's AccountRoot
                    if pos + 2 > data.len() { break; }
                    pos += 2;
                }
                (2, 2) => {
                ...
                (2, 5) => {
                    ...
                    if pos + 4 > data.len() { break; }
                    previous_txn_lgr_seq = u32::from_be_bytes(data[pos..pos + 4].try_into().unwrap());
                    pos += 4;
                }
                (5, 5) => {
                ...
                (1, _) => { if pos + 2 > data.len() { break; } pos += 2; }       // UInt16
                (2, _) => { if pos + 4 > data.len() { break; } pos += 4; }       // UInt32
                (3, _) => { if pos + 8 > data.len() { break; } pos += 8; }       // UInt64
                (4, _) => { if pos + 16 > data.len() { break; } pos += 16; }     // Hash128
                (5, _) => { if pos + 32 > data.len() { break; } pos += 32; }     // Hash256
                (6, _) => {                                                        // Amount
                    if pos >= data.len() { break; }
                    if (data[pos] & 0x80) != 0 {
                        pos += 48; // IOU
                ...
                (7, _) | (8, _) | (19, _) => {                                    // VL types
                    if pos >= data.len() { break; }
                    let (vl_len, vl_bytes) = crate::transaction::serialize::decode_length(&data[pos..]);
                    pos += vl_bytes + vl_len;
                }
                (16, _) => { if pos >= data.len() { break; } pos += 1; }         // UInt8
                (17, _) => { if pos + 20 > data.len() { break; } pos += 20; }    // Hash160
                _ => { break; }                                                    // truly unknown
            }
        }
        ...
        Ok(AccountRoot {
            ...
            regular_key, minted_nftokens, burned_nftokens,
            transfer_rate, domain, tick_size, ticket_count,
...
mod tests {
    ...
    fn test_balance_encoding_xrp_bit() {
        let acct = AccountRoot {
            account_id: [0u8; 20], balance: 1_000_000, sequence: 1,
        ...
        let pos = bytes.iter().position(|&b| b == 0x62).expect("Balance field present");
        let raw = u64::from_be_bytes(bytes[pos + 1..pos + 9].try_into().unwrap());
        assert_ne!(raw & 0x4000_0000_0000_0000, 0, "positive XRP bit must be set");
