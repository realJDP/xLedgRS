...
impl PeerState {
    pub fn is_active(&self)  -> bool { matches!(self, PeerState::Active) }
    pub fn is_closed(&self)  -> bool { matches!(self, PeerState::Closed { .. }) }
    pub fn is_open(&self)    -> bool { !self.is_closed() }
}
...
pub enum PeerEvent {
    ...
    TlsEstablished,
    /// We sent our HTTP upgrade request (outbound) or received one (inbound).
    HandshakeSent,
    ...
    DisconnectRequested,
    /// Outbound queue is empty and we can close the socket.
    OutboundDrained,
...
pub struct Peer {
    ...
    /// When we last received any data from this peer.
    pub last_rx:      Instant,
    /// How long without traffic before we time out.
...
/// A unique peer identifier (for now just a counter; later a node public key).
    ...
    pub fn handle(&mut self, event: PeerEvent) -> PeerAction {
        match (&self.state, event) {
            ...
            (PeerState::Handshaking, PeerEvent::HandshakeAccepted(info)) => {
                ...
                match self.direction {
                    // Inbound: we received the request, send 101 back
                    Direction::Inbound  => PeerAction::SendHandshakeResponse,
                    // Outbound: we already sent the request, peer accepted
            ...
            (PeerState::Active, PeerEvent::DisconnectRequested) => {
                self.state = PeerState::Closing { reason: CloseReason::LocalDisconnect };
            ...
            (_, PeerEvent::RemoteClosed) => {
                self.state = PeerState::Closed { reason: CloseReason::RemoteDisconnect };
                PeerAction::CloseSocket
            }
            ...
            (_, PeerEvent::IdleTimeout) => {
                self.state = PeerState::Closed { reason: CloseReason::Timeout };
                PeerAction::CloseSocket
            }
...
mod tests {
    ...
    fn peer(direction: Direction) -> Peer {
        ...
        Peer::new(PeerId(1), addr, direction)
    }
    ...
    fn test_handshake_rejected() {
        ...
        assert!(matches!(
            p.state,
            PeerState::Closed { reason: CloseReason::HandshakeRejected(_) }
        ));
    }
    ...
    fn test_clean_disconnect() {
        ...
        assert!(matches!(
            p.state,
            PeerState::Closed { reason: CloseReason::LocalDisconnect }
        ));
    }
    ...
    fn test_error_closes_connection() {
        ...
        assert!(matches!(
            p.state,
            PeerState::Closed { reason: CloseReason::ProtocolError(_) }
        ));
    }
    ...
    fn test_message_received_increments_counter() {
        ...
        p.handle(PeerEvent::MessageReceived(MessageType::Transaction, vec![1, 2, 3]));
    ...
    fn test_idle_timeout_closes_active_peer() {
        ...
        assert_eq!(action, PeerAction::CloseSocket);
        assert!(matches!(p.state, PeerState::Closed { reason: CloseReason::Timeout }));
    }
