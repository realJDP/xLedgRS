...
impl Offer {
    ...
    pub fn quality(&self) -> Option<f64> {
        ...
        if gets == 0.0 { None } else { Some(pays / gets) }
    }
}
...
/// XRP: just drops. IOU: mantissa (exponent handled via cross-multiply).
...
pub fn rate_gte(a_pays: &Amount, a_gets: &Amount, b_pays: &Amount, b_gets: &Amount) -> bool {
    ...
    match exp_diff.cmp(&0) {
        ...
        std::cmp::Ordering::Greater => {
            ...
            if exp_diff > 38 { return lhs > 0; } // definitively larger if non-zero
            lhs.saturating_mul(10i128.pow(exp_diff as u32)) >= rhs
        }
        std::cmp::Ordering::Less => {
            let diff = -exp_diff;
            if diff > 38 { return rhs <= 0; } // rhs side definitively larger
...
pub fn amount_to_f64(a: &Amount) -> f64 {
    match a {
        ...
        Amount::Iou { value, .. } => {
            if value.mantissa == 0 { return 0.0; }
...
pub fn scale_amount(a: &Amount, numerator: i128, denominator: i128) -> Amount {
    ...
    match a {
        ...
        Amount::Iou { value, currency, issuer } => {
            ...
            let num_iou = IouValue { mantissa: numerator.clamp(i64::MIN as i128, i64::MAX as i128) as i64, exponent: 0 };
            let den_iou = IouValue { mantissa: denominator.clamp(i64::MIN as i128, i64::MAX as i128) as i64, exponent: 0 };
...
pub fn subtract_amount(a: &Amount, b: &Amount) -> Amount {
    match (a, b) {
        ...
        (Amount::Iou { value: va, currency, issuer },
         Amount::Iou { value: vb, .. }) => {
            ...
            let (ma, mb, exp) = if va.exponent == vb.exponent {
                ...
                (va.mantissa as i128 * 10i128.pow(shift), vb.mantissa as i128, vb.exponent)
            } else {
                // Scale vb up to match va's exponent
                let shift = exp_diff as u32;
                (va.mantissa as i128, vb.mantissa as i128 * 10i128.pow(shift), va.exponent)
            };
            let diff = (ma - mb).max(0).clamp(0, i64::MAX as i128) as i64;
            let mut result = IouValue { mantissa: diff, exponent: exp };
            result.normalize();
            Amount::Iou {
                value: result,
                currency: currency.clone(),
...
impl BookKey {
    pub fn from_amounts(pays: &Amount, gets: &Amount) -> Self {
        ...
        Self { pays_currency: pc, pays_issuer: pi, gets_currency: gc, gets_issuer: gi }
    }
...
fn currency_issuer(a: &Amount) -> ([u8; 20], [u8; 20]) {
    match a {
        ...
        Amount::Iou { currency, issuer, .. } => (currency.code, *issuer),
...
impl OrderBook {
    ...
    pub fn len(&self) -> usize { self.offers.len() }
    pub fn is_empty(&self) -> bool { self.offers.is_empty() }
}
...
mod tests {
    ...
    fn xrp(drops: u64) -> Amount { Amount::Xrp(drops) }
    ...
    fn test_offer_quality() {
        let offer = Offer {
            account: acct(1), sequence: 1,
            taker_pays: xrp(1_000_000), // wants 1 XRP
            taker_gets: usd(10.0, 2),    // giving 10 USD
            flags: 0,
            book_directory: [0u8; 32], book_node: 0, owner_node: 0,
    ...
    fn test_order_book_sorted_by_quality() {
        ...
        let cheap = Offer {
            account: acct(1), sequence: 1,
            taker_pays: xrp(1_000_000), taker_gets: usd(100.0, 2), // quality = 10000
            flags: 0,
            book_directory: [0u8; 32], book_node: 0, owner_node: 0,
            expiration: None, domain_id: None, additional_books: Vec::new(),
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0,
        ...
        let expensive = Offer {
            account: acct(3), sequence: 2,
            taker_pays: xrp(1_000_000), taker_gets: usd(10.0, 2), // quality = 100000
            flags: 0,
            book_directory: [0u8; 32], book_node: 0, owner_node: 0,
            expiration: None, domain_id: None, additional_books: Vec::new(),
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0,
    ...
    fn test_order_book_remove() {
        ...
        let offer = Offer {
            account: acct(1), sequence: 1,
            taker_pays: xrp(1_000_000), taker_gets: usd(10.0, 2),
            flags: 0,
            book_directory: [0u8; 32], book_node: 0, owner_node: 0,
            expiration: None, domain_id: None, additional_books: Vec::new(),
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0,
