...
pub struct DisputedTx {
    ...
    pub tx_id: [u8; 32],
    /// Whether the local position includes this transaction.
    pub our_vote: bool,
...
impl DisputedTx {
    ...
    /// Update the local vote using the avalanche threshold.
    /// `percent_time`: progress through consensus round (0-100+).
    /// `proposing`: whether the local node is actively proposing (affects threshold).
    /// Returns true if the local vote changed.
    pub fn update_vote(&mut self, percent_time: u32, proposing: bool) -> bool {
        ...
        let new_vote = if proposing {
            // Proposing mode uses a weighted average that includes the local vote.
            ...
            if self.yays + self.nays == 0 {
                ...
                let weight = ((self.yays as u32 * 100) + self_weight)
                    / (self.nays as u32 + self.yays as u32 + 1);
