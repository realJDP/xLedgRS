...
use crate::ledger::ter::{self, TxResult, ApplyFlags};
...
static FEATURE_PERMISSIONED_DOMAINS: LazyLock<[u8; 32]> = LazyLock::new(|| amendment_hash("PermissionedDomains"));
...
#[allow(dead_code)]
static FEATURE_PERMISSIONED_DEX: LazyLock<[u8; 32]> = LazyLock::new(|| amendment_hash("PermissionedDEX"));
static FEATURE_NFT_PAGE_LINKS: LazyLock<[u8; 32]> = LazyLock::new(|| amendment_hash("fixNFTokenPageLinks"));
static FEATURE_DELEGATION: LazyLock<[u8; 32]> = LazyLock::new(|| amendment_hash("PermissionDelegationV1_1"));
...
pub(crate) fn bridge_metadata_only_tx(
    ...
    if let Some(validated) = ctx.validated_result {
        ...
        tracing::warn!(
            "apply_tx: tx type {} ({}) reached without validated metadata; returning {}",
            tx_type,
            label,
            fallback_token,
        );
...
pub fn run_tx(
    ...
    // Full ticket consumption is deferred to handler — we just gate here.
    ...
    if needs_reset {
        ...
        let touched = state.commit_tx();
        return TxRunResult { ter, applied: true, touched };
    }
    ...
    if ter.is_tec_claim() && !ter.is_tec_claim_hard_fail(flags) {
        ...
        state.discard_tx();
        return TxRunResult { ter, applied: false, touched: Vec::new() };
    }
    ...
    // We need the touched entries for invariant checking.
    ...
    let fee_inv = invariants::check_invariants(state, &fee_touched, ter::TEC_INVARIANT_FAILED, tx.fee, tx);

    if fee_inv == InvariantResult::Ok {
        let touched = state.commit_tx();
        TxRunResult { ter: ter::TEC_INVARIANT_FAILED, applied: true, touched }
    } else {
        ...
        state.discard_tx();
        TxRunResult { ter: ter::TEF_INVARIANT_FAILED, applied: false, touched: Vec::new() }
    }
}
...
fn parse_majority_entries(raw: &[u8]) -> Vec<MajorityEntry> {
    ...
    let mut pos = 0;
    while pos < raw.len() {
        if raw[pos] == 0xF1 { break; } // array end marker
        // Expect sfMajority object header (type=14, field=18)
        // Skip the object header — we just need to find the fields inside.
        let start = pos;
        // Skip field header (1-3 bytes)
        let byte = raw[pos]; pos += 1;
        let hi = (byte >> 4) & 0x0F;
        let lo = byte & 0x0F;
        if hi == 0 || lo == 0 {
            if hi == 0 && lo != 0 { if pos < raw.len() { pos += 1; } }
            else if hi != 0 && lo == 0 { if pos < raw.len() { pos += 1; } }
            else { pos += 2.min(raw.len() - pos); }
        }
        ...
        while pos < raw.len() && raw[pos] != 0xE1 {
            ...
            let (tc, fc) = if tc != 0 && fc != 0 {
                ...
                let t = raw[pos]; pos += 1; (t, fc)
            } else if tc != 0 && fc == 0 {
                if pos >= raw.len() { break; }
                let f = raw[pos]; pos += 1; (tc, f)
            } else {
                if pos + 1 >= raw.len() { break; }
                let t = raw[pos]; pos += 1;
                let f = raw[pos]; pos += 1;
                (t, f)
            };
            match tc {
                2 => { // UInt32
                    ...
                    if fc == 7 { close_time = v; } // sfCloseTime
                    pos += 4;
                }
                5 => { // Hash256
                    if pos + 32 > raw.len() { break; }
                    if fc == 19 { // sfAmendment
                ...
                _ => {
                    ...
                    while pos < raw.len() && raw[pos] != 0xE1 { pos += 1; }
                    break;
                }
            }
        }
        if pos < raw.len() && raw[pos] == 0xE1 { pos += 1; } // skip object end
        if amendment != [0u8; 32] || close_time != 0 || pos > start + 3 {
            entries.push(MajorityEntry { amendment, close_time });
        }
    }
    entries
}
...
fn extract_majorities_raw(sle_data: &[u8]) -> Option<Vec<u8>> {
    ...
    while pos < sle_data.len() {
        ...
        let lo = byte & 0x0F;
        if hi != 0 && lo != 0 {
            type_code = hi; field_code = lo;
        } else if hi == 0 && lo != 0 {
            if pos >= sle_data.len() { break; }
            type_code = sle_data[pos]; pos += 1; field_code = lo;
        } else if hi != 0 && lo == 0 {
            if pos >= sle_data.len() { break; }
            type_code = hi; field_code = sle_data[pos]; pos += 1;
        } else {
            if pos + 1 >= sle_data.len() { break; }
            type_code = sle_data[pos]; pos += 1;
            field_code = sle_data[pos]; pos += 1;
        }
        match type_code {
            1 => { pos += 2; } // UInt16
            2 => { pos += 4; } // UInt32
            ...
            15 => {
                ...
                if field_code == 16 {
                    let start = pos;
                    while pos < sle_data.len() && sle_data[pos] != 0xF1 { pos += 1; }
                    if pos < sle_data.len() { pos += 1; } // include 0xF1
                    return Some(sle_data[start..pos].to_vec());
                } else {
                    while pos < sle_data.len() && sle_data[pos] != 0xF1 { pos += 1; }
                    if pos < sle_data.len() { pos += 1; }
                }
            }
            19 => { // VECTOR256
                if pos >= sle_data.len() { break; }
...
pub fn apply_tx(state: &mut LedgerState, tx: &ParsedTx, ctx: &TxContext) -> ApplyResult {
    ...
    // account — just skip.  (rippled: Change.cpp — ttAMENDMENT, ttFEE, ttUNL_MODIFY)
    match tx.tx_type {
        100 => {
        ...
        101 => {
            ...
            use crate::ledger::{fees_key, Fees, serialize_fee_settings};
            let key = fees_key();
            let fees = Fees {
        ...
        102 => {
            ...
            // For now, we write the pending disable/re-enable field.
            ...
            tracing::info!(
                "UNLModify: {} validator {}",
                if disabling { "disabling" } else { "re-enabling" },
    ...
    // ── Amendment gate ────────────────────────────────────────────────────────
    // Log (but don't block) tx types whose amendment isn't in our local set.
    ...
    // until we are independently computing ledgers as a trusted validator.
    ...
    let result = match tx.tx_type {
        2 | 4 | 7 | 8 | 15 | 17 | 18 | 26 | 28 | 29 | 30 | 31 | 35 | 36 | 37 | 38 | 39 | 40 | 59 | 60 | 65 | 67 | 68 | 69 | 74 | 75 | 80 | 81 | 84 => {
        ...
        _ => {
            let r = match tx.tx_type {
                ...
                62 => permissioned_domain::apply_permissioned_domain_set(state, tx, &mut new_sender),    // PermissionedDomainSet
                63 => permissioned_domain::apply_permissioned_domain_delete(state, tx, &mut new_sender), // PermissionedDomainDelete
                ...
                _ => bridge_metadata_only_tx(ctx, tx.tx_type, "unknown/future tx type", "temUNKNOWN"),
            };
...
mod tests {
    ...
    use crate::transaction::{Amount, builder::TxBuilder, parse_blob};
    ...
    fn state_with_genesis(balance: u64) -> LedgerState {
        ...
        state.insert_account(AccountRoot {
            account_id: genesis_id(), balance, sequence: 1,
    ...
    fn test_apply_credits_existing_destination() {
        ...
        state.insert_account(AccountRoot {
            account_id: dest_id(), balance: 5_000_000, sequence: 1,
            owner_count: 0, flags: 0, regular_key: None, minted_nftokens: 0, burned_nftokens: 0,
            transfer_rate: 0, domain: Vec::new(), tick_size: 0, ticket_count: 0,
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0, raw_sle: None,
        });
    ...
    fn test_state_hash_changes_after_apply() {
        ...
        assert_ne!(hash_before, hash_after, "state hash must change after applying a tx");
    }
    ...
    fn state_with_two_accounts() -> LedgerState {
        let mut state = state_with_genesis(50_000_000);
        state.insert_account(AccountRoot {
            account_id: dest_id(), balance: 50_000_000, sequence: 1,
            owner_count: 0, flags: 0, regular_key: None, minted_nftokens: 0, burned_nftokens: 0,
            transfer_rate: 0, domain: Vec::new(), tick_size: 0, ticket_count: 0,
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0, raw_sle: None,
        });
    ...
    fn test_trustset_zero_limit_deletes() {
        ...
        let usd = crate::transaction::amount::Currency::from_code("USD").unwrap();
        assert!(state.get_trustline_for(&genesis_id(), &dest_id(), &usd).is_none());
    ...
    fn test_iou_payment_through_trust_line() {
        ...
        // Step 2: Directly manipulate the trust line to simulate the issuer funding the line
        {
            ...
            if genesis_id() < dest_id() {
                tl.balance = IouValue { mantissa: 500_000_000_000_000_0, exponent: -15 };
            } else {
                tl.balance = IouValue { mantissa: -500_000_000_000_000_0, exponent: -15 };
            }
        ...
        let iou_amount = Amount::Iou {
            value:    IouValue { mantissa: 100_000_000_000_000_0, exponent: -15 },
        ...
        let usd = Currency::from_code("USD").unwrap();
        let tl = state.get_trustline_for(&genesis_id(), &dest_id(), &usd).unwrap();
        let bal = tl.balance_for(&genesis_id());
        assert!(bal.mantissa > 0, "genesis should still have a positive balance");
    }
    ...
    fn test_offer_create_appears_in_book() {
        ...
        let dir = state.get_directory(&expected_dir).expect("book dir should exist");
    ...
    fn test_offer_crossing_full_fill() {
        ...
        state.insert_account(AccountRoot {
            account_id: acct2_id, balance: 100_000_000, sequence: 1,
            owner_count: 0, flags: 0, regular_key: None, minted_nftokens: 0, burned_nftokens: 0,
            transfer_rate: 0, domain: Vec::new(), tick_size: 0, ticket_count: 0,
            previous_txn_id: [0u8; 32], previous_txn_lgr_seq: 0, raw_sle: None,
        });
    ...
    fn test_offer_no_crossing_when_prices_dont_overlap() {
        ...
        apply_tx(&mut state, &trustset, &ctx(0));
        {
            ...
            if genesis_id() < dest_id() {
                tl.balance = IouValue { mantissa: 1000_000_000_000_000_0, exponent: -15 };
            } else {
                tl.balance = IouValue { mantissa: -1000_000_000_000_000_0, exponent: -15 };
            }
            state.insert_trustline(tl);
        ...
        // happen. Previously our engine deleted tx1 unconditionally when it
    ...
    fn test_offer_crossing_partial_fill() {
        ...
        let offers = state.offers_by_account(&genesis_id());
        assert!(offers.len() <= 2, "at most one remaining offer plus possibly a partial");
    }
    ...
    fn test_offer_replace_reuses_released_iou_liquidity() {
        ...
        let trustset = sign_trustset(1, "USD", "rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe", 10000.0);
        assert_eq!(apply_tx(&mut state, &trustset, &ctx(0)), ApplyResult::Success);
        {
            let usd = Currency::from_code("USD").unwrap();
            let key = crate::ledger::trustline::shamap_key(&genesis_id(), &dest_id(), &usd);
            let mut tl = state.get_trustline(&key).unwrap().clone();
        ...
        assert_eq!(apply_tx(&mut state, &old_offer, &ctx(0)), ApplyResult::Success);
        ...
        assert_eq!(apply_tx(&mut state, &replacement, &ctx(0)), ApplyResult::Success);

        let old_key = crate::ledger::offer::shamap_key(&genesis_id(), 2);
        let new_key = crate::ledger::offer::shamap_key(&genesis_id(), 3);

        assert!(state.get_offer(&old_key).is_none(), "old offer should be canceled");
        assert!(state.get_offer(&new_key).is_some(), "replacement offer should be placed");
        assert!(crate::ledger::directory::owner_dir_contains_entry(&state, &genesis_id(), &new_key.0));
        assert!(!crate::ledger::directory::owner_dir_contains_entry(&state, &genesis_id(), &old_key.0));
    }
    ...
    fn sign_escrow_create(seq: u32, amount: u64, dest: &str, finish: u32, cancel: u32) -> ParsedTx {
        ...
        let mut b = TxBuilder::escrow_create()
            ...
            .fee(12)
            .sequence(seq);
        if finish > 0 { b = b.finish_after(finish); }
        if cancel > 0 { b = b.cancel_after(cancel); }
    ...
    fn test_escrow_create_locks_funds() {
        let mut state = state_with_genesis(50_000_000);
        let tx = sign_escrow_create(1, 10_000_000, "rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe", 1000, 2000);
        let result = apply_tx(&mut state, &tx, &ctx(0));
        assert_eq!(result, ApplyResult::Success);
    ...
    fn test_escrow_finish_after_time() {
        ...
        let tx2 = {
            ...
            let signed = TxBuilder::escrow_finish()
                .account(&kp)
                .destination(&genesis_addr).unwrap()
                .offer_sequence(1)
                .fee(12)
                .sequence(2)
                .sign(&kp).unwrap();
    ...
    fn test_paychan_claim_with_signature() {
        ...
        let claim_tx = ParsedTx {
            tx_type: 15, flags: 0, sequence: 2, fee: 12,
            account: genesis_id(), destination: None,
            amount_drops: Some(claimed), amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, offer_sequence: None,
            finish_after: None, cancel_after: None, settle_delay: None,
            expiration: None, channel: Some(chan_key.0), public_key: None,
            deliver_min: None, nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None, uri: None, nftoken_taxon: None, transfer_fee_field: None, asset_scale: None, maximum_amount: None, mutable_flags: None, mptoken_metadata: None, paychan_sig: Some(claim_sig), owner: None, regular_key: None,
            issuer: None, subject: None, credential_type: None, oracle_document_id: None, signer_quorum: None, signer_entries_raw: None, domain_id: None, ledger_fix_type: None, accepted_credentials_raw: None, authorize: None, permissions_raw: None, did_document: None, did_data: None,
            holder: None, mptoken_issuance_id: None, set_flag: None, clear_flag: None, transfer_rate: None, tick_size: None, last_ledger_seq: None, ticket_count: None, ticket_sequence: None, domain: None, asset: None, asset2: None, vault_id: None, amendment: None, base_fee_field: None, reserve_base_field: None, reserve_increment_field: None, unl_modify_disabling: None, unl_modify_validator: None,
            signing_pubkey: vec![0x02; 33], signature: vec![],
            signing_hash: [0u8; 32], signing_payload: vec![], send_max: None, paths: vec![], signers: vec![],
        };
    ...
    fn test_paychan_close_and_delete() {
        ...
        let close_tx = ParsedTx {
            tx_type: 15, flags: 0x00010000, sequence: 2, fee: 12,
            account: genesis_id(), destination: None,
            amount_drops: None, amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, offer_sequence: None,
            finish_after: None, cancel_after: None, settle_delay: None,
            expiration: None, channel: Some(chan_key.0), public_key: None,
            deliver_min: None, nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None, uri: None, nftoken_taxon: None, transfer_fee_field: None, asset_scale: None, maximum_amount: None, mutable_flags: None, mptoken_metadata: None, paychan_sig: None, owner: None, regular_key: None,
            issuer: None, subject: None, credential_type: None, oracle_document_id: None, signer_quorum: None, signer_entries_raw: None, domain_id: None, ledger_fix_type: None, accepted_credentials_raw: None, authorize: None, permissions_raw: None, did_document: None, did_data: None,
            holder: None, mptoken_issuance_id: None, set_flag: None, clear_flag: None, transfer_rate: None, tick_size: None, last_ledger_seq: None, ticket_count: None, ticket_sequence: None, domain: None, asset: None, asset2: None, vault_id: None, amendment: None, base_fee_field: None, reserve_base_field: None, reserve_increment_field: None, unl_modify_disabling: None, unl_modify_validator: None,
            signing_pubkey: vec![0x02; 33], signature: vec![],
            signing_hash: [0u8; 32], signing_payload: vec![], send_max: None, paths: vec![], signers: vec![],
        ...
        let delete_tx = ParsedTx {
            tx_type: 15, flags: 0, sequence: 3, fee: 12,
            account: genesis_id(), destination: None,
            amount_drops: None, amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, offer_sequence: None,
            finish_after: None, cancel_after: None, settle_delay: None,
        ...
        assert!(state.get_paychan(&chan_key).is_some(), "channel shouldn't be deleted yet");

        apply_tx(&mut state, &delete_tx, &ctx(2061));
        assert!(state.get_paychan(&chan_key).is_none(), "channel should be deleted after expiration");
    ...
    fn sign_check_create(seq: u32, amount: u64, dest: &str, expiration: u32) -> ParsedTx {
        ...
        let mut b = TxBuilder::check_create()
            ...
            .amount(Amount::Xrp(amount))
            .fee(12).sequence(seq);
        if expiration > 0 { b = b.expiration(expiration); }
    ...
    fn test_check_cash_exact() {
        ...
        let cash_tx = ParsedTx {
            tx_type: 17, flags: 0, sequence: 1, fee: 12,
            account: dest_id(), destination: None,
            amount_drops: Some(5_000_000), amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, deliver_min: None,
            offer_sequence: Some(1),
            finish_after: None, cancel_after: None, settle_delay: None,
            expiration: None, channel: None, public_key: None,
            nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None, uri: None, nftoken_taxon: None, transfer_fee_field: None, asset_scale: None, maximum_amount: None, mutable_flags: None, mptoken_metadata: None, paychan_sig: None, owner: Some(genesis_id()), regular_key: None,
            issuer: None, subject: None, credential_type: None, oracle_document_id: None, signer_quorum: None, signer_entries_raw: None, domain_id: None, ledger_fix_type: None, accepted_credentials_raw: None, authorize: None, permissions_raw: None, did_document: None, did_data: None,
            holder: None, mptoken_issuance_id: None, set_flag: None, clear_flag: None, transfer_rate: None, tick_size: None, last_ledger_seq: None, ticket_count: None, ticket_sequence: None, domain: None, asset: None, asset2: None, vault_id: None, amendment: None, base_fee_field: None, reserve_base_field: None, reserve_increment_field: None, unl_modify_disabling: None, unl_modify_validator: None,
            signing_pubkey: vec![0x02; 33], signature: vec![],
            signing_hash: [0u8; 32], signing_payload: vec![], send_max: None, paths: vec![], signers: vec![],
    ...
    fn test_check_cash_insufficient_balance() {
        ...
        let cash_tx = ParsedTx {
            ...
            amount_drops: Some(5_000_000), amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, deliver_min: None,
            offer_sequence: Some(1), finish_after: None, cancel_after: None,
            settle_delay: None, expiration: None, channel: None, public_key: None,
            nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None, uri: None, nftoken_taxon: None, transfer_fee_field: None, asset_scale: None, maximum_amount: None, mutable_flags: None, mptoken_metadata: None, paychan_sig: None, owner: Some(genesis_id()), regular_key: None,
            issuer: None, subject: None, credential_type: None, oracle_document_id: None, signer_quorum: None, signer_entries_raw: None, domain_id: None, ledger_fix_type: None, accepted_credentials_raw: None, authorize: None, permissions_raw: None, did_document: None, did_data: None,
            holder: None, mptoken_issuance_id: None, set_flag: None, clear_flag: None, transfer_rate: None, tick_size: None, last_ledger_seq: None, ticket_count: None, ticket_sequence: None, domain: None, asset: None, asset2: None, vault_id: None, amendment: None, base_fee_field: None, reserve_base_field: None, reserve_increment_field: None, unl_modify_disabling: None, unl_modify_validator: None,
            signing_pubkey: vec![0x02; 33], signature: vec![],
    ...
    fn test_check_cash_wrong_account() {
        ...
        let cash_tx = ParsedTx {
            tx_type: 17, flags: 0, sequence: 2, fee: 12,
            account: genesis_id(), destination: None,
            amount_drops: Some(5_000_000), amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, deliver_min: None,
            offer_sequence: Some(1), finish_after: None, cancel_after: None,
    ...
    fn run_tx_wrong_sequence_returns_ter_pre_seq() {
        ...
        assert_eq!(state.get_account(&genesis_id()).unwrap().balance, 10_000_000);
    ...
    fn ctx_at_seq(ledger_seq: u32) -> TxContext {
        TxContext { ledger_seq, ..TxContext::default() }
    }
    ...
    fn ticket_create_tx(seq: u32, count: u32) -> ParsedTx {
        ParsedTx {
            tx_type: 10, flags: 0, sequence: seq, fee: 12,
            account: genesis_id(), destination: None,
            amount_drops: None, amount: None, limit_amount: None,
            taker_pays: None, taker_gets: None, offer_sequence: None,
            finish_after: None, cancel_after: None, settle_delay: None,
            ...
            deliver_min: None, nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None, uri: None, nftoken_taxon: None, transfer_fee_field: None, asset_scale: None, maximum_amount: None, mutable_flags: None, mptoken_metadata: None, paychan_sig: None, owner: None, regular_key: None,
            issuer: None, subject: None, credential_type: None, oracle_document_id: None, signer_quorum: None, signer_entries_raw: None, domain_id: None, ledger_fix_type: None, accepted_credentials_raw: None, authorize: None, permissions_raw: None, did_document: None, did_data: None,
            holder: None, mptoken_issuance_id: None, set_flag: None, clear_flag: None, transfer_rate: None, tick_size: None, last_ledger_seq: None, ticket_count: Some(count), ticket_sequence: None, domain: None, asset: None, asset2: None, vault_id: None, amendment: None, base_fee_field: None, reserve_base_field: None, reserve_increment_field: None, unl_modify_disabling: None, unl_modify_validator: None,
            signing_pubkey: vec![0x02; 33], signature: vec![],
            signing_hash: [0u8; 32], signing_payload: vec![], send_max: None, paths: vec![], signers: vec![],
        }
    }
    ...
    fn ticket_create_starts_at_next_sequence() {
        ...
        assert!(state.get_raw_owned(&wrong_first).is_none(), "must not create a ticket at the tx Sequence itself");
        assert!(state.get_raw_owned(&first).is_some(), "first ticket should use Sequence + 1");
        assert!(state.get_raw_owned(&second).is_some(), "second ticket should use Sequence + 2");
    }
    ...
    fn ticket_create_adds_next_sequence_ticket_to_owner_dir() {
        ...
        assert!(crate::ledger::directory::owner_dir_contains_entry(&state, &genesis_id(), &first.0));
        assert!(!crate::ledger::directory::owner_dir_contains_entry(&state, &genesis_id(), &wrong_first.0));
    }
    ...
    fn ticket_create_advances_account_sequence_past_reserved_range() {
        ...
        let sender = state.get_account(&genesis_id()).unwrap();
        assert_eq!(sender.sequence, 4, "Sequence should advance past tx seq and reserved tickets");
    }
    ...
    fn ticketed_offer_create_uses_ticket_sequence_proxy() {
        ...
        let offer = state.get_offer(&ticket_offer_key).expect("ticket-based offer should use ticket sequence");
        assert_eq!(offer.sequence, 42);
        assert!(state.get_offer(&zero_offer_key).is_none(), "must not create a sequence-0 offer");
    ...
    fn offer_create_preserves_expiration_on_created_sle() {
        ...
        let raw = state.get_raw_owned(&offer_key).expect("offer raw SLE should exist");
        let decoded = crate::ledger::Offer::decode_from_sle(&raw).expect("offer raw SLE should decode");
    ...
    fn batch_without_validated_result_fails_cleanly() {
        ...
        let result = run_tx(&mut state, &tx, &ctx(0), ApplyFlags::NONE);
        assert_eq!(result.ter, ter::TEF_FAILURE);
        assert!(!result.applied);
