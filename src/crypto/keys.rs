...
impl Secp256k1KeyPair {
    ...
    pub fn verify(&self, message: &[u8], signature: &[u8]) -> bool {
        ...
        let msg_hash = crate::crypto::sha512_first_half(message);
        let Ok(msg) = secp256k1::Message::from_digest_slice(&msg_hash) else {
            return false;
        };
...
fn derive_secp256k1_account(entropy: &[u8; 16]) -> Result<secp256k1::SecretKey> {
    ...
    let account_key_material = derive_secp256k1_scalar_with_public(&root_pub_bytes, account_index)?;
