...
use crate::ledger::state_table::{ApplyAction, ApplyStateTable};
use crate::ledger::views::{ApplyFlags, ApplyView, LedgerInfo, ReadView};
...
pub struct ApplyViewImpl<'a> {
    /// The open-ledger view that receives staged changes.
...
impl<'a> ApplyViewImpl<'a> {
    ...
    pub fn discard(self) {
        // Dropping the view without applying it leaves the backing state
        // unchanged.
    }
...
impl<'a> ReadView for ApplyViewImpl<'a> {
    fn read(&self, keylet: &Keylet) -> Option<Arc<SLE>> {
        // Consult the staged buffer before falling back to the base view.
    ...
    fn succ(&self, key: &Key, last: Option<&Key>) -> Option<Key> {
        // Delegate successor traversal to OpenView. ApplyStateTable does not
        // provide its own successor view.
