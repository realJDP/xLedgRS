...
use crate::ledger::apply_view_impl::{ApplyViewImpl, AffectedNodeInfo};
...
mod tests {
    ...
    fn test_unknown_tx_type_is_rejected_without_mutation() {
        ...
        let tx_hash = [0x04; 32];
        let result = apply_transaction(&mut open, &tx, &tx_hash, &UnsupportedHandler, ApplyFlags::NONE);
    ...
    fn test_legacy_unsupported_handlers_fail_without_mutation() {
        ...
        for tx_type in unsupported_types {
            ...
            assert!(result.metadata.is_none(), "tx_type {tx_type} claimed metadata");

            let alice_sle = open.read(&keylet::account(&alice)).unwrap();
            assert_eq!(alice_sle.balance_xrp(), Some(100_000_000), "tx_type {tx_type}");
