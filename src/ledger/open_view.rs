...
use crate::ledger::views::{ReadView, RawView, TxsRawView, LedgerInfo};
use crate::ledger::state_table::{RawStateTable, RawAction};
...
pub struct OpenView {
    ...
    tx_map: SHAMap,
    /// Ledger info (copy from base, modified as we go).
    info: LedgerInfo,
...
impl ReadView for OpenView {
    ...
    fn succ(&self, key: &Key, last: Option<&Key>) -> Option<Key> {
        // Merge our buffer's successor with the base's successor
        ...
        match (buf_succ, base_succ) {
            (Some(a), Some(b)) => {
                ...
                if let Some(last) = last {
                    if &candidate >= last { None } else { Some(candidate) }
                } else {
                    Some(candidate)
                }
            }
...
mod tests {
    ...
    fn test_open_view_read_through() {
        ...
        base.raw_insert(Arc::new(SLE::new(keylet.key, LedgerEntryType::AccountRoot, data)));
