...
fn scramble_taxon(taxon: u32, sequence: u32) -> u32 {
    let scramble = 384160001u64.wrapping_mul(sequence as u64).wrapping_add(2459) as u32;
    taxon ^ scramble
}
...
impl NFToken {
    pub fn shamap_key(&self) -> Key {
        Key(sha512_first_half(&[&NFTOKEN_SPACE[..], &self.nftoken_id].concat()))
    }
}
    ...
    pub fn decode_from_sle(data: &[u8]) -> Option<Self> {
        ...
        for f in &parsed.fields {
            match (f.type_code, f.field_code) {
                (2, 2) if f.data.len() >= 4 => flags = u32::from_be_bytes(f.data[..4].try_into().ok()?),
                (2, 4) if f.data.len() >= 4 => sequence = u32::from_be_bytes(f.data[..4].try_into().ok()?),
                (2, 10) if f.data.len() >= 4 => expiration = Some(u32::from_be_bytes(f.data[..4].try_into().ok()?)),
                (3, 4) if f.data.len() >= 8 => owner_node = u64::from_be_bytes(f.data[..8].try_into().ok()?),
                (3, 12) if f.data.len() >= 8 => nft_offer_node = u64::from_be_bytes(f.data[..8].try_into().ok()?),
        ...
        if let Some(id) = parsed.prev_txn_id { previous_txn_id = id; }
...
mod tests {
    ...
    fn test_taxon_scrambling() {
        ...
        assert_ne!(s1, s2, "different sequences should produce different scrambled taxons");
    }

    #[test]
    fn test_sle_round_trip_preserves_sf_sequence() {
        // Build a minimal NFTokenOffer SLE WITH sfSequence (type=2, field=4)
        use crate::ledger::meta::{ParsedField, build_sle};
        let seq: u32 = 1875; // 0x0753
        let fields = vec![
            ParsedField { type_code: 2, field_code: 2, data: 0x0001u32.to_be_bytes().to_vec() }, // sfFlags
            ParsedField { type_code: 2, field_code: 4, data: seq.to_be_bytes().to_vec() },        // sfSequence
            ParsedField { type_code: 6, field_code: 1, data: vec![0x40, 0,0,0,0,0,0,0] },        // sfAmount (0 XRP)
            ParsedField { type_code: 5, field_code: 10, data: vec![0xAA; 32] },                   // sfNFTokenID
            ParsedField { type_code: 3, field_code: 4, data: 0u64.to_be_bytes().to_vec() },       // sfOwnerNode
            ParsedField { type_code: 8, field_code: 2, data: vec![0xBB; 20] },                    // sfOwner
        ];
        ...
        let has_seq = parsed.fields.iter().any(|f| f.type_code == 2 && f.field_code == 4);
        ...
        let seq_field = parsed2.fields.iter().find(|f| f.type_code == 2 && f.field_code == 4);
        assert!(seq_field.is_some(), "sfSequence must survive SLE round-trip");
    ...
    fn test_to_sle_binary_preserves_sf_sequence_from_raw() {
        ...
        let seq: u32 = 1875;
        let fields = vec![
            ParsedField { type_code: 2, field_code: 2, data: 0x0001u32.to_be_bytes().to_vec() },
            ParsedField { type_code: 2, field_code: 4, data: seq.to_be_bytes().to_vec() },
            ParsedField { type_code: 6, field_code: 1, data: vec![0x40, 0,0,0,0,0,0,0] },
            ParsedField { type_code: 5, field_code: 10, data: vec![0xAA; 32] },
            ParsedField { type_code: 3, field_code: 4, data: 0u64.to_be_bytes().to_vec() },
            ParsedField { type_code: 8, field_code: 2, data: vec![0xBB; 20] },
        ];
        let raw = build_sle(0x0037, &fields, Some([0xCC; 32]), Some(100));
        ...
        let seq_field = parsed.fields.iter().find(|f| f.type_code == 2 && f.field_code == 4);
        assert!(seq_field.is_some(), "sfSequence must survive to_sle_binary round-trip");
        assert_eq!(
