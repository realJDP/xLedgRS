...
impl TxHandler for CheckCancelHandler {
    fn do_apply(&self, tx: &ParsedTx, view: &mut dyn ApplyView) -> TER {
        ...
        let check_keylet = crate::ledger::keylet::from_raw(check_id, 0x0043)
            .unwrap_or(keylet::Keylet::new(crate::ledger::Key(check_id), LedgerEntryType::Check));
...
impl TxHandler for CheckCashHandler {
    fn do_apply(&self, tx: &ParsedTx, view: &mut dyn ApplyView) -> TER {
        ...
        let check_keylet = keylet::Keylet::new(
            crate::ledger::Key(check_id),
            LedgerEntryType::Check,
        );
        let check_sle = match view.peek(&check_keylet) {
            Some(s) => s,
        ...
        // Get the SendMax amount (XRP for now)
        let send_max_drops = check_sle.get_field_xrp_drops(6, 9).unwrap_or(0);
        let cash_drops = tx.amount_drops.unwrap_or(send_max_drops).min(send_max_drops);
