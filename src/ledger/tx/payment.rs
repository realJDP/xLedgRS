...
pub(crate) fn apply_payment(
    ...
    if use_ripple {
        ...
        if let Some(result) =
            apply_validated_replay_self_payment_hint(tx, &dest_id, &deliver, new_sender, ctx)
        {
            return result;
        }
...
fn apply_validated_replay_self_payment_hint(
    ...
    let is_cross_currency = match (deliver, tx.send_max.as_ref()) {
        (Amount::Iou { currency, .. }, Some(Amount::Iou { currency: send_max_currency, .. })) => {
    ...
    let delivered = ctx.validated_delivered_amount.clone().unwrap_or_else(|| deliver.clone());
    match (&delivered, tx.send_max.as_ref()) {
        (Amount::Xrp(drops), _) => {
            new_sender.balance = new_sender.balance.saturating_add(*drops);
        }
        // Replay bundles only give us authoritative DeliveredAmount. For the inverse
