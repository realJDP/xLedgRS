//! Ledger sync — download and verify ledger state from peers.
//!
//! When a validation arrives for a ledger sequence ahead of ours, we request
//! the ledger header (liBASE) from the peer.  The response contains the raw
//! header which we parse to update our displayed ledger info.
//!
//! State tree download via TMGetLedger(liAS_NODE) / TMLedgerData.
//! Follows the same approach as rippled's InboundLedger:
//!   1. Build a local inner node store (HashMap) incrementally
//!   2. get_missing_nodes() walks from root, finds children not yet received
//!   3. Request missing nodes, receive response, insert, repeat
//!   4. When get_missing_nodes() returns empty → sync complete

use std::collections::{HashMap, VecDeque};
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::Arc;

/// Global cookie counter for matching TMGetLedger requests to TMLedgerData
/// responses.
...
/// ```
///
/// We need at least 118 bytes after skipping the optional prefix.
pub fn parse_ledger_header_from_base(data: &[u8]) -> Option<crate::ledger::LedgerHeader> {
    // Skip the "LWR\0" hash prefix if present.
...
pub fn storage_to_wire(data: &[u8]) -> Option<Vec<u8>> {
    ...
    if prefix == STORAGE_PREFIX_INNER {
        ...
        // Wire: 512 child hashes + wireTypeInner
        let mut wire = Vec::with_capacity(payload.len() + 1);
        wire.extend_from_slice(payload);
        wire.push(WIRE_TYPE_INNER);
        Some(wire)
    } else if prefix == STORAGE_PREFIX_LEAF {
        // Leaf node: MLN\0 + SLE data + 32-byte key = variable
        // Wire: SLE data + 32-byte key + wireTypeAccountState
        let mut wire = Vec::with_capacity(payload.len() + 1);
        wire.extend_from_slice(payload);
        ...
        // (could be tx nodes SND\0/SMD\0, but state sync doesn't need those)
        None
    }
}

/// Maximum SHAMapNodeIDs to request per TMGetLedger batch (matches rippled's missingNodesFind).
...
/// A SHAMap inner node: 16 child hashes + a bitmask of which children
/// have been received (inner or leaf).
#[derive(Clone)]
pub struct InnerNode {
    /// Child hashes — zero means empty branch.
    children: [[u8; 32]; 16],
    /// Bitmask: bit `i` set means child `i` has been received.
    received: u16,
    /// This node's own hash (from parent's child hash array).
    self_hash: [u8; 32],
    /// True when entire subtree below this node is complete.
    /// Checked inline during tree walk — avoids HashSet lookup.
    is_full_below: bool,
}
...
impl InnerNode {
    fn from_full(data: &[u8]) -> Option<Self> {
        ...
        let mut node = InnerNode { children: [[0u8; 32]; 16], received: 0, self_hash: [0u8; 32], is_full_below: false };
        for i in 0..16 {
...
/// Apply rippled's depthMask to a 32-byte path: zero out all nibbles beyond depth.
/// Matches rippled's SHAMapNodeID constructor invariant: id == (id & depthMask(depth)).
fn apply_depth_mask(path: &mut [u8; 32], depth: u8) {
    let d = depth as usize;
    // Full bytes used: d/2 (each byte holds 2 nibbles)
    let full_bytes = d / 2;
...
/// Applies depthMask to match rippled's canonicalization — all nibbles beyond
/// child_depth are zeroed. Without this, rippled rejects the node ID in
/// deserializeSHAMapNodeID() and silently drops the request.
...
pub fn parent_and_branch(child_id: &[u8; 33]) -> Option<([u8; 33], u8)> {
    ...
    if depth == 0 { return None; } // root has no parent
...
impl PeerSyncManager {
    pub fn new(seq: u32, hash: [u8; 32], account_hash: [u8; 32]) -> Self {
        ...
        Self {
            ledger_seq: seq, ledger_hash: hash, account_hash, active: true,
            in_flight: 0,
            ...
            last_response: now, last_type2: now,
            stalled_retries: 0, tail_stuck_hash: [0u8; 32], tail_stuck_retries: 0,
            pass_number: 0, pass_start_leaf_count: 0, last_new_nodes: now,
            inner_count: 0, leaf_count: 0,
        }
    }

    pub fn accept_response(&mut self, resp_hash: &[u8], cookie: Option<u32>) -> bool {
        let ltclosed = self.ledger_hash == [0u8; 32];
        if !ltclosed && (resp_hash.len() < 8 || resp_hash[..8] != self.ledger_hash[..8]) {
            return false;
        }
        if ltclosed && resp_hash.len() != 32 {
            return false;
        }
        if let Some(c) = cookie {
            let c32 = c as u32;
            if self.outstanding_cookies.remove(&c32) && !self.responded_cookies.contains(&c32) {
    ...
    pub fn accept_object_response(&mut self, resp_hash: &[u8], seq: Option<u32>) -> bool {
        ...
        let seq_known = self.outstanding_object_queries.contains(&s32)
        ...
        if self.outstanding_object_queries.remove(&s32) && !self.responded_object_queries.contains(&s32) {
    ...
    pub fn clear_recent(&mut self) { self.recent_nodes.clear(); }
    ...
    pub fn has_cookie(&self, cookie: u32) -> bool { self.outstanding_cookies.contains(&cookie) }
    pub fn outstanding_cookie_count(&self) -> usize { self.outstanding_cookies.len() }
    pub fn outstanding_object_query_count(&self) -> usize { self.outstanding_object_queries.len() }
    pub fn recent_node_count(&self) -> usize { self.recent_nodes.len() }
    pub fn new_objects_this_pass(&self) -> usize { self.leaf_count.saturating_sub(self.pass_start_leaf_count) }
    ...
    pub fn knows_object_query(&self, seq: u32) -> bool {
        self.outstanding_object_queries.contains(&seq) || self.responded_object_queries.contains(&seq)
    }
    ...
    pub fn build_requests_from_missing(
        ...
        // Filter out recently requested nodes for Reply triggers.
        // Timeout triggers bypass the filter entirely — they must always
        // produce a request when missing > 0 to prevent sync stalls.
        let filtered: Vec<&(crate::ledger::shamap_id::SHAMapNodeID, [u8; 32])> =
            if matches!(reason, SyncRequestReason::Timeout) {
                missing.iter().collect()
            } else {
                let f: Vec<_> = missing.iter()
                    .filter(|(_, hash)| !self.recent_nodes.contains(hash))
                    .collect();
        ...
        // query_depth: 1 for bulk (get node + children), 0 for tail/retry
        let query_depth = match reason {
            SyncRequestReason::Reply if filtered.len() > 16 => 1,
            _ => 0,
        };
        ...
        let per_request = match reason {
            SyncRequestReason::Reply => 128,
            SyncRequestReason::Timeout => 12,
        };
        // For timeout (stall recovery): use ltCLOSED (zero hash) instead of
        // specific ledger hash. Peers only keep the current ledger's SHAMap
        // in memory — by the time our request for a specific ledger arrives,
        // that ledger's SHAMap has been evicted. ltCLOSED serves from the
        // peer's current tree, which shares 99.99% of nodes with our target.
        let request_hash = if matches!(reason, SyncRequestReason::Timeout) && self.stalled_retries > 0 {
            [0u8; 32] // triggers ltCLOSED on the peer side
        } else {
            self.ledger_hash
        };
        let mut reqs = Vec::new();
        for chunk in filtered.chunks(per_request) {
            let batch: Vec<Vec<u8>> = chunk.iter()
                .map(|(nid, _)| nid.to_wire().to_vec())
                .collect();
            reqs.push(crate::network::relay::encode_get_ledger_state(
                &request_hash, &batch, 0, query_depth, query_type, self.ledger_seq,
            ));
        }
        reqs
    }
    ...
    pub fn parse_response(
        ...
        for node in &ld.nodes {
            if node.nodedata.is_empty() { continue; }

            // Extract SHAMapNodeID from nodeid field
            let node_id = match &node.nodeid {
                Some(nid) if nid.len() >= 33 => {
                    crate::ledger::shamap_id::SHAMapNodeID::from_wire(nid)
                }
                Some(nid) if nid.is_empty() => {
                    Some(crate::ledger::shamap_id::SHAMapNodeID::root())
                }
            ...
            let Some(nid) = node_id else {
                tracing::debug!("sync: skipping node with unparseable nodeid len={:?}", node.nodeid.as_ref().map(|n| n.len()));
                continue;
            };

            // Log first few nodes per response for debugging
            if nodes.len() < 3 && self.inner_count < 100 {
                tracing::info!("sync node: depth={} nodeid_len={:?} data_len={} wire_type={:#04x}",
                    nid.depth(), node.nodeid.as_ref().map(|n| n.len()), node.nodedata.len(),
                    node.nodedata.last().copied().unwrap_or(0));
            }

            // Wire data includes type byte at end
            let wire_type = node.nodedata[node.nodedata.len() - 1];
            let data = &node.nodedata[..node.nodedata.len() - 1];

            // Extract leaf data for NuDB persistence
            if wire_type == 0x01 && data.len() > 32 {
                // Account state leaf: [sle_data || key(32)]
                ...
                tracing::debug!("sync: unknown wire_type={:#04x} data_len={}", wire_type, data.len());
            }
...
pub struct StateSyncer {
    ...
    /// Batch buffer for RocksDB writes — flushed at WRITE_BUFFER_FLUSH_SIZE or on process_response return.
    /// Retained as empty Vec for API compat (flush_write_buffer clears it).
...
impl StateSyncer {
    ...
    pub fn new(seq: u32, hash: [u8; 32], account_hash: [u8; 32], storage: Option<Arc<crate::storage::Storage>>) -> Self {
        Self {
            ledger_seq: seq,
            ledger_hash: hash,
            account_hash,
    ...
    /// Update a node in the cache (no-op — cache is already mutated in place).
    fn update_node(&mut self, _id: &[u8; 33]) {
        // No persistence — inner nodes live in memory only.
    }
    ...
    /// Flush the write buffer — no-op, inner nodes are in-memory only.
    ...
    /// Pre-warm the cache — no-op, all inner nodes are in-memory only.
    pub fn warm_walk_cache(&mut self) {
        // With RocksDB removed, all inner nodes live in node_cache already.
    }
    ...
    pub fn build_batch_requests(&mut self, count: usize, storage: Option<&crate::storage::Storage>) -> Vec<crate::network::message::RtxpMessage> {
    ...
    fn build_requests(
        ...
        let mut reqs = Vec::new();
        // queryDepth: 1 for bulk fetching (get node + children), 0 for tail/retry.
        // queryDepth=1 can cascade-fail in rippled's getNodeFat: if any child
        // of the requested node is missing from the peer's NodeStore, the entire
        // node is omitted from the response. Use 0 when few nodes remain to
        // avoid this, and always 0 for timeout retries.
        let query_depth = match reason {
            SyncRequestReason::Reply if missing.len() > 16 => 1,
            _ => 0,
        };
        ...
        // Check if the tail-stuck nodes are in the batch we're about to send
        if missing.len() <= 20 {
            let current_stuck = self.get_missing_nodes(5, 0, None);
            let stuck_set: std::collections::HashSet<[u8; 33]> = current_stuck.iter().cloned().collect();
            let in_batch = missing.iter().filter(|id| stuck_set.contains(*id)).count();
            tracing::info!(
                "BATCH CHECK: sending {} nodes, {} of {} stuck are in batch",
                missing.len(), in_batch, current_stuck.len(),
            );
        }
    ...
    pub fn build_next_request(&mut self, storage: Option<&crate::storage::Storage>) -> Option<crate::network::message::RtxpMessage> {
        self.build_request(storage, SyncRequestReason::Reply)
    }

    pub fn build_timeout_request(&mut self, storage: Option<&crate::storage::Storage>) -> Option<crate::network::message::RtxpMessage> {
    ...
    pub fn build_timeout_object_request(
        ...
        // If we have a tail-stuck node, include it first so it's always requested.
        ...
        for entry in others {
            if needed.len() >= MAX_OBJECT_BY_HASH_BATCH { break; }
    ...
    pub fn process_response(&mut self, ld: &crate::proto::TmLedgerData) -> SyncProgress {
        ...
        let response_depths: Vec<u8> = ld.nodes.iter()
            .filter_map(|n| n.nodeid.as_ref().and_then(|id| if id.len() == 33 { Some(id[32]) } else { None }))
            .collect();
        if response_node_count > 0 && response_node_count <= 10 {
            tracing::info!(
                ...
                response_node_count,
                response_depths,
                ld.nodes.iter().map(|n| n.nodedata.len()).collect::<Vec<_>>(),
            );
        }
        ...
        if !current_missing.is_empty() && current_missing.len() <= 5 && ld.nodes.len() <= 20 {
            let missing_set: std::collections::HashSet<[u8; 33]> = current_missing.iter().cloned().collect();
            ...
            if !found_missing {
                let resp_nids: Vec<String> = ld.nodes.iter()
                    .map(|n| {
                    ...
                    .collect();
                let missing_nids: Vec<String> = current_missing.iter()
            ...
            match wire_type {
                WIRE_TYPE_INNER | WIRE_TYPE_COMPRESSED_INNER => {
                    ...
                    if let Some(mut inner) = parsed {
                        ...
                        for b in 0..16 {
                            ...
                            if child_hash.iter().all(|&x| x == 0) { continue; }
                        ...
                        for (branch, _old_hash) in dels {
                            diff_deletions.push(DiffDeletion { parent_nid: nid, branch });
                        }
                        ...
                        // "known" before we've walked their subtrees.
                ...
                WIRE_TYPE_ACCOUNT_STATE => {
                    ...
                    if body.len() > 32 {
                        ...
                        let computed_hash = crate::ledger::sparse_shamap::leaf_hash(&stobject, &key32);
        ...
        // Flush any remaining buffered writes to RocksDB
    ...
    fn diff_detect_deletions(&self, _nid: &[u8; 33], _new_node: &mut InnerNode) -> Vec<(u8, [u8; 32])> {
    ...
    fn update_full_below(&mut self, child_id: &[u8; 33]) {
        ...
        loop {
            ...
            let all_complete = {
                ...
                for b in 0..16 {
                    if parent.is_empty_branch(b) { continue; }
                    if !parent.is_received(b) {
                        complete = false;
                        break;
                    }
                    let child = build_child_id(&parse_node_id(&parent_id).0, parent_id[32], b as u8);
    ...
    pub fn get_missing_nodes(&self, max: usize, _random_start: usize, _storage: Option<&crate::storage::Storage>) -> Vec<[u8; 33]> {
        let root_id = [0u8; 33];
        let root = match self.get_node_or_load_readonly(&root_id) {
            Some(n) => n,
            None => return vec![root_id],
        };
        if root.is_full_below { return vec![]; }

        let mut missing = Vec::new();
        let mut stack: Vec<[u8; 33]> = vec![root_id];

        while let Some(node_id) = stack.pop() {
            if missing.len() >= max { break; }
            ...
            for i in 0..16u8 {
                ...
                if inner.is_empty_branch(branch as usize) { continue; }

                let child_hash = inner.children[branch as usize];
                if self.known_hashes.contains(&child_hash) { continue; }

                if inner.is_received(branch as usize) {
                    let child_id = build_child_id(&path, depth, branch);
                    let child_full_below = self.get_node_or_load_readonly(&child_id)
    ...
    pub fn is_truly_complete(&self, storage: Option<&crate::storage::Storage>) -> bool {
        ...
        self.has_node(&[0u8; 33])
            && self.get_missing_nodes(1, 0, storage).is_empty()
    }
    ...
    pub fn parse_account_root(stobject: &[u8]) -> Option<crate::ledger::AccountRoot> {
        if stobject.len() < 3 { return None; }
        if stobject[0] != 0x11 { return None; }
        let entry_type = u16::from_be_bytes([stobject[1], stobject[2]]);
        if entry_type != 0x0061 { return None; }
...
mod tests {
    ...
    fn test_get_missing_nodes_with_root() {
        ...
        let mut root = InnerNode { children: [[0u8; 32]; 16], received: 0, self_hash: [0u8; 32], is_full_below: false };
    ...
    fn test_depth_mask_depth_52() {
        ...
        for i in 0..26 { assert_eq!(path[i], 0xFF, "depth 52: byte {} should be FF", i); }
        for i in 26..32 { assert_eq!(path[i], 0x00, "depth 52: byte {} should be 00", i); }
    }
    ...
    fn test_depth_mask_depth_53() {
        ...
        for i in 0..26 { assert_eq!(path[i], 0xFF); }
        assert_eq!(path[26], 0xF0, "depth 53: byte 26 upper nibble only");
        for i in 27..32 { assert_eq!(path[i], 0x00); }
    }
    ...
    fn test_depth_mask_depth_63() {
        ...
        for i in 0..31 { assert_eq!(path[i], 0xFF); }
