...
pub struct LedgerStore {
    ...
    /// Lowest sequence retained locally.
    min_seq: u32,
    /// Highest sequence retained locally.
    max_seq: u32,
...
impl LedgerStore {
    ...
    pub fn insert_ledger(&mut self, header: LedgerHeader, tx_records: Vec<TxRecord>) {
        ...
        if let Some(max_history) = self.max_history {
            if self.max_seq.saturating_sub(self.min_seq) > max_history {
                ...
                let pruned_tx_hashes: Vec<[u8; 32]> = self
                    .ledgers
                    .iter()
    ...
    /// Return the highest-sequence ledger retained in memory.
    ...
    /// Return a reverse-chronological page of transactions across stored ledgers.
    pub fn tx_history_page(&self, start: usize, limit: usize) -> Vec<TxRecord> {
        let mut seqs: Vec<u32> = self.ledgers.keys().copied().collect();
        seqs.sort_unstable_by(|a, b| b.cmp(a));

        let mut all = Vec::new();
        for seq in seqs {
            all.extend(self.ledger_txs(seq));
        }
