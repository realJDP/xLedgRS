    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, ManifestError> {
        ...
        while pos < data.len() {
            ...
            let (type_code, field_code) = if top == 0 && bot == 0 {
                if pos + 2 > data.len() { return Err(ManifestError::Truncated); }
                let t = data[pos] as u16; let f = data[pos+1] as u16;
                pos += 2;
                (t, f)
            } else if top == 0 {
                if pos >= data.len() { return Err(ManifestError::Truncated); }
                let t = data[pos] as u16; pos += 1;
                (t, bot)
            } else if bot == 0 {
                if pos >= data.len() { return Err(ManifestError::Truncated); }
                let f = data[pos] as u16; pos += 1;
                (top, f)
            } else {
                (top, bot)
            };
            ...
            match (type_code, field_code) {
                ...
                (2, 4) => {
                    // sfSequence (UInt32)
                    if pos + 4 > data.len() { return Err(ManifestError::Truncated); }
                ...
                (7, _) => {
                    ...
                    pos += 1;
                    if pos + vl > data.len() { return Err(ManifestError::Truncated); }
...
impl ManifestCache {
    ...
    pub fn add(&mut self, manifest: Manifest) -> bool {
        ...
        let current_seq = self.master_seq
            .get(&manifest.master_pubkey)
            .copied()
            .unwrap_or(0);

        // Reject replays and out-of-order sequences.
        // Allow exactly REVOKE_SEQ only if not already revoked.
        ...
        self.master_seq.insert(manifest.master_pubkey.clone(), manifest.sequence);
        ...
        if manifest.is_revocation() {
            ...
            self.by_signing_key.insert(manifest.signing_pubkey.clone(), manifest);
        }
...
mod tests {
    ...
    fn kp() -> Secp256k1KeyPair { Secp256k1KeyPair::generate() }
    ...
    fn test_cache_key_rotation() {
        ...
        let mut cache = ManifestCache::new();
        assert!(cache.add(m1));
        assert!(cache.add(m2), "higher-sequence manifest must replace old one");

        // Old signing key must be gone
        assert!(cache.signing_key_to_master(&pub1).is_none(),
