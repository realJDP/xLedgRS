...
TARGET_DIR="${1:-$(cd "$ROOT_DIR/.." && pwd)/XLedgRS-V1Beta}"
...
chmod +x "$TARGET_DIR/deploy-xledgrs.sh"
...
: > /tmp/xledgrs-release-seed-scan.txt
seed_error=0
while IFS= read -r -d '' file; do
    if awk '
        ...
        END { exit bad ? 0 : 1 }
    ' "$file" >> /tmp/xledgrs-release-seed-scan.txt; then
        seed_error=1
    fi
done < <(find "$TARGET_DIR/cfg" -name '*.cfg' -print0)

if [ "$seed_error" -ne 0 ]; then
    cat /tmp/xledgrs-release-seed-scan.txt
...
if rg -n -P '^\s*(?:ssh|scp|rsync)\b.*\b[A-Za-z0-9._-]+@[A-Za-z0-9._-]+\b' \
    "${RELEASE_SURFACES[@]}" > /tmp/xledgrs-release-host-scan.txt; then
    cat /tmp/xledgrs-release-host-scan.txt
...
if rg -n -P '\\b(?!(?:127\\.0\\.0\\.1|0\\.0\\.0\\.0)\\b)(?:\\d{1,3}\\.){3}\\d{1,3}\\b' \
    "${RELEASE_SURFACES[@]}" > /tmp/xledgrs-release-ip-scan.txt; then
    cat /tmp/xledgrs-release-ip-scan.txt
