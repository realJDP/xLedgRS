...
//! - the original lightweight TOML format used by this project
...
pub struct ConfigFile {
    ...
    /// When present the node signs validations & proposals with this key
    /// instead of the random node identity key — making it a validator.
...
impl ConfigFile {
    ...
    fn from_toml(toml_cfg: TomlConfigFile, path: &Path) -> Self {
        let validators = toml_cfg.validators.into_iter()
            .filter_map(|v| decode_validator_key(&v.public_key))
            .map(|public_key| ValidatorEntry { public_key })
            .collect();

        let runtime = RuntimeConfig {
            peer_addr: toml_cfg.node.peer_addr
                .and_then(|s| s.parse().ok()),
            rpc_addr: toml_cfg.node.rpc_addr
                .and_then(|s| s.parse().ok()),
            ws_addr: toml_cfg.node.ws_addr
                .and_then(|s| s.parse().ok()),
        ...
        Self {
            ...
            amendments: AmendmentConfig { enabled: toml_cfg.amendments.enabled },
            runtime,
    ...
    fn from_rippled_cfg(contents: &str, path: &Path) -> Result<Self> {
        ...
        for name in server_names {
            if let Some(addr) = parse_server_port(&sections, &server_defaults.values, &name, &["peer"]) {
                runtime.peer_addr.get_or_insert(addr);
            }
            if let Some(addr) = parse_server_port(&sections, &server_defaults.values, &name, &["http", "https"]) {
                runtime.rpc_addr.get_or_insert(addr);
            }
            if let Some(addr) = parse_server_port(&sections, &server_defaults.values, &name, &["ws", "wss"]) {
        ...
        if let Some(section) = sections.get("node_db") {
            runtime.online_delete = section.values.get("online_delete").and_then(|v| v.parse().ok());
        ...
        if let Some(section) = sections.get("xledgrs").or_else(|| sections.get("xrpl_rs")) {
            runtime.rpc_sync = section.values.get("rpc_sync").cloned();
            runtime.enable_consensus_close_loop = section.values.get("enable_consensus_close_loop")
        ...
        if let Some(section) = sections.get("validators") {
            validators.extend(section.items.iter().filter_map(|v| decode_validator_key(v)).map(|public_key| ValidatorEntry { public_key }));
        }
        if let Some(section) = sections.get("validators_file") {
            if let Some(rel_path) = first_scalar(section) {
                ...
                validator_lists.publisher_keys.extend(file_cfg.validator_lists.publisher_keys);
        ...
        let validation_seed = sections.get("validation_seed")
...
fn parse_sections(contents: &str) -> HashMap<String, SectionConfig> {
    ...
    for raw_line in contents.lines() {
        ...
        let Some(section_name) = current.as_ref() else { continue };
        let section = sections.entry(section_name.clone()).or_default();
        if let Some((key, value)) = line.split_once('=') {
            section.values.insert(key.trim().to_ascii_lowercase(), value.trim().to_string());
        } else {
...
fn parse_server_port(
    ...
    let ip = section.values.get("ip")
        .or_else(|| server_defaults.get("ip"))?;
    let port = section.values.get("port")
        .or_else(|| server_defaults.get("port"))?;
    let protocols = section.values.get("protocol")
    ...
    let protocol_set: Vec<String> = protocols
        ...
        .filter(|p| !p.is_empty())
        .collect();
    if !wanted_protocols.iter().any(|wanted| protocol_set.iter().any(|p| p == wanted)) {
        return None;
    }
...
fn resolve_path(config_path: &Path, raw: &str) -> PathBuf {
    ...
    if candidate.is_absolute() {
        ...
        config_path.parent().unwrap_or_else(|| Path::new(".")).join(candidate)
    }
}
...
fn load_validators_file(path: &Path) -> Result<ConfigFile> {
    ...
    let sites = sections.get("validator_list_sites")
        .map(|s| s.items.clone())
        .unwrap_or_default();
    let publisher_keys: Vec<String> = sections.get("validator_list_keys")
        .map(|s| s.items.iter()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty())
            .collect())
        .unwrap_or_default();
    let threshold = sections.get("validator_list_threshold")
        .and_then(first_scalar)
...
"#;
        ...
        assert_eq!(cfg.runtime.bootstrap_peers, vec!["r.ripple.com:51235".to_string()]);
...
"#;
    ...
    fn test_parse_validators_file_with_node_public_keys() {
        ...
        let encoded = crate::crypto::base58::encode(crate::crypto::base58::PREFIX_NODE_PUBLIC, &pubkey);
...
"#;
    ...
    fn test_parse_validators_file_with_publisher_lists_only() {
        let validators_path = write_temp(r#"
...
"#, "txt");
    ...
    fn test_parse_validators_file_with_validators_and_publishers() {
        ...
        let encoded = crate::crypto::base58::encode(crate::crypto::base58::PREFIX_NODE_PUBLIC, &pubkey);
        let validators_path = write_temp(&format!(r#"
[validators]
{}
...
"#, encoded), "txt");
        ...
        assert_eq!(cfg.validators.len(), 1);
        assert_eq!(cfg.validators[0].public_key, pubkey);
        assert_eq!(cfg.validator_lists.sites, vec!["https://vl.ripple.com".to_string()]);
