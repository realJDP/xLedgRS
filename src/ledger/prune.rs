...
pub fn collect_live_leaf_hashes(
...
fn collect_from_inner(
    node: &crate::ledger::shamap::InnerNode,
...
pub fn compact_from_shamap(
    ...
    info!("prune: complete — {} copied, {} missing out of {} live",
        copied, missing, live_hashes.len());
...
pub fn compact_in_place(
    ...
    // The source NodeStore is still open — caller must drop it before swap.
    // For now, just leave the compact dir and let the caller handle the swap.
    info!(
