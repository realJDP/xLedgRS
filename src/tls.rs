...
//!
//! Because peers verify identity via Session-Signature (not TLS certificate chain),
//! we skip TLS cert chain verification in `AcceptAnyCert`. TLS still provides
//! encryption and the session binding that makes signatures meaningful.
...
/// Rustls-based TLS configs. Currently unused for peer connections (OpenSslConfig
/// is used instead for rippled-compatible session hash computation), but kept
/// for potential non-peer TLS uses (e.g., WebSocket TLS).
...
impl TlsConfig {
    ...
    pub fn new_self_signed() -> Result<Self> {
        ...
        let key_der = PrivateKeyDer::Pkcs8(
            PrivatePkcs8KeyDer::from(cert_gen.serialize_private_key_der()),
        );
        ...
        // Client config — skip cert chain verification; identity proven via Session-Signature
...
//
// Used for peer-to-peer connections so we can compute the same session hash
// (via TLS Finished messages) that rippled uses.
