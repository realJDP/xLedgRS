...
pub(super) fn build_li_base_nodes(
    ...
    if let Some(root_wire) = tx_root_wire {
        nodes.push(crate::proto::TmLedgerNode {
            nodedata: root_wire,
            nodeid: Some(
                crate::ledger::shamap_id::SHAMapNodeID::root()
...
mod tests {
    #[test]
    fn reply_followup_requires_useful_data_like_rippled() {
        assert!(super::should_issue_reply_followup("Continue", true));
        assert!(super::should_issue_reply_followup("PassComplete", true));
        assert!(!super::should_issue_reply_followup("Continue", false));
        assert!(!super::should_issue_reply_followup("PassComplete", false));
        assert!(!super::should_issue_reply_followup("Inactive", true));
    }
}

pub(super) fn should_refuse_get_ledger_for_load(
    load: &crate::network::load::LoadSnapshot,
    peer_send_capacity: Option<usize>,
    info_type: i32,
) -> bool {
    if info_type == crate::proto::TmLedgerInfoType::LiTsCandidate as i32 {
        return false;
    }
    if peer_send_capacity == Some(0) {
        return true;
    }
    load.is_queue_overloaded()
        || load
            .load_factor_local()
            .is_some_and(|fee| fee > load.load_base.saturating_mul(2))
}
