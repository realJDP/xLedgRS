...
//! - getMissingNodes: walk the tree to find nodes we still need
//!
...
use crate::ledger::shamap::{MapType, Key, InnerNode, LeafNode, Node};
use crate::ledger::shamap::PREFIX_INNER_NODE;
...
pub(crate) fn add_known_node(
    ...
    while current_id.depth() < node_id.depth() {
        ...
        if current_id.depth() + 1 == node_id.depth() {
            // We're at the parent — insert the new node here
            ...
            if computed_hash != expected_hash {
                tracing::warn!("add_known_node: INVALID hash mismatch depth={} computed={} expected={}",
                    node_id.depth(), hex::encode_upper(&computed_hash[..8]), hex::encode_upper(&expected_hash[..8]));
                return AddNodeResult::Invalid;
            }
        ...
        match &mut inner.children[branch] {
            Some(child) => match child.as_mut() {
                ...
                _ => {
                    tracing::warn!("add_known_node: INVALID child not inner at depth={}", current_id.depth());
                    return AddNodeResult::Invalid;
                }
            },
            None => {
                ...
                if let Some(ref backend) = backend {
                    if let Ok(Some(stored)) = backend.fetch(&expected_hash) {
                        if stored.len() == 16 * 32 {
                            let child_inner = deserialize_inner_from_store(&stored);
...
/// Find missing nodes — walk the tree and return hashes we need from peers.
...
pub(crate) fn get_missing_nodes(
    ...
    // Stack: (node_ptr, node_id, next_child_to_process, full_below_flag)
    // When descending into a child, parent is pushed with current_child
    // pointing to the NEXT branch to process on resume.
    let mut stack: Vec<(*mut InnerNode, SHAMapNodeID, usize, bool)> = Vec::new();
    ...
    loop {
        ...
        while current_child < 16 {
            ...
            let branch = current_child;
            current_child += 1;

            let inner = unsafe { &mut *node };
            if !inner.has_branch(branch) { continue; }

            let child_hash = inner.child_hash(branch);
            if child_hash == [0u8; 32] { continue; }

            // Already known complete in cache?
            if full_below.touch_if_exists(&child_hash) { continue; }
                    ...
                    Some(child) => match child.as_mut() {
                        ...
                        Node::Stub { content_hash, .. } => {
                            ...
                            if let Some(ref be) = backend {
                                if let Ok(Some(_)) = be.fetch(content_hash) {
                                    full_below.insert(*content_hash);
                                } else {
                                    missing.push((child_id, *content_hash));
                    ...
                    None => {
                        ...
                        if let Some(ref be) = backend {
                            if let Ok(Some(stored)) = be.fetch(&child_hash) {
                                if stored.len() == 16 * 32 {
                                    // Inner node from NuDB — load into tree
                                    if let Some(ci) = deserialize_inner_from_store(&stored) {
                                        inner.children[branch] = Some(Box::new(Node::Inner(ci)));
            ...
            // If we have an inner child to descend into, suspend parent
            if let Some(child_ptr) = descend_target {
                stack.push((node, node_id, current_child, full_below_flag));
                node = child_ptr;
                node_id = child_id;
                current_child = 0;
        ...
        if let Some((parent_ptr, parent_id, parent_child, parent_full)) = stack.pop() {
...
mod tests {
    ...
    fn add_root_node_works() {
        ...
        let result = add_known_node(&mut root, &root_id, &wire, MapType::AccountState, None, &mut fb);
    ...
    fn add_child_node_works() {
        ...
        let leaf_node = parse_wire_node(&leaf_wire[..leaf_wire.len()-1], WIRE_LEAF_ACCOUNT_STATE, MapType::AccountState).unwrap();
        ...
        let root_id = SHAMapNodeID::root();
        add_known_node(&mut root, &root_id, &root_wire, MapType::AccountState, None, &mut fb);

        // Now add the leaf at depth 1, branch 0xA
        let leaf_id = root_id.child_id(0xA);
        let result = add_known_node(&mut root, &leaf_id, &leaf_wire, MapType::AccountState, None, &mut fb);
        assert_eq!(result, AddNodeResult::Useful);
    ...
    fn get_missing_finds_gaps() {
        ...
        let missing = get_missing_nodes(
            &mut root, 256, MapType::AccountState, None, &mut fb,
        );
