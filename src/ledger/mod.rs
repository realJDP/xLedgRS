//! Ledger module — headers, state, and the SHAMap hash tree.
//!
//! The view stack (matching rippled's architecture):
//!   ClosedLedger (ReadView + RawView) → OpenView (TxsRawView) → ApplyViewImpl (ApplyView)

pub mod account;
pub mod tx;

// ── View stack (rippled architecture) ───────────────────────────────────────
pub mod sle;
pub mod keylet;
pub mod fees;
pub mod rules;
...
pub mod prune;
pub mod sfield_meta;
// Backward compatibility — old imports via `crate::ledger::apply::*` still work.
pub mod apply {
    pub use super::tx::{ApplyResult, TxContext, apply_tx};
}
pub mod check;
pub mod diff_sync;
pub mod close;
...
pub use account::{AccountRoot, shamap_key};
pub use check::Check;
pub use close::{close_ledger, CloseResult, replay_ledger, ReplayResult, extract_tx_blobs_from_tx_tree};
...
impl Default for Fees {
    fn default() -> Self {
        Self { base: 10, reserve: 10_000_000, increment: 2_000_000 }
    }
}
...
pub fn parse_fee_settings(data: &[u8]) -> Fees {
    ...
    // We scan for known field codes rather than assuming a fixed offset.
    let mut pos = 0;
    while pos < data.len() {
        if data.len() - pos < 1 { break; }
        let byte = data[pos];
        pos += 1;
        ...
        if hi != 0 && lo != 0 {
            ...
            type_code = data[pos] as u16; pos += 1;
            field_code = lo as u16;
        } else if hi != 0 && lo == 0 {
            if pos >= data.len() { break; }
            type_code = hi as u16;
            field_code = data[pos] as u16; pos += 1;
        } else {
        ...
        match type_code {
            ...
            15 => {
                ...
                if pos < data.len() { pos += 1; } // skip end marker
            }
            _ => {
    ...
    while pos < data.len() {
        ...
        match type_code {
            ...
            15 => {
                ...
                if pos < data.len() { pos += 1; } // skip 0xF1 end marker
            }
            19 => {
...
impl LedgerState {
    ...
    fn record_preimage(&mut self, key: &Key) {
        let should_record = self.tx_journal.as_ref()
    ...
    pub fn peek_tx_journal(&self) -> Vec<(Key, Option<Vec<u8>>)> {
        self.tx_journal.as_ref()
            .map(|journal| {
                journal.order.iter()
                    .filter_map(|key| journal.before.get(key).cloned().map(|snapshot| (*key, snapshot)))
                    .collect()
            })
            .unwrap_or_default()
    }
    ...
    pub fn discard_tx(&mut self) {
        ...
        if let Some(journal) = self.tx_journal.take() {
            ...
            // multiple times we restore the original pre-tx state.
        ...
        if let Some(snap) = self.tx_snapshot.take() {
            // First, collect all keys that were touched so we can remove
            // any typed entries that were CREATED during this tx.
            let touched_keys: Vec<Key> = snap.typed_before.iter()
                .map(|(k, _)| *k)
                .collect();
    ...
    fn remove_typed_entry_silent(&mut self, key: &Key) {
        ...
        let acct_id = self.accounts.iter()
        ...
        if self.trustlines.remove(key).is_some() {
            ...
            // (will be rebuilt when we restore the before-state entry)
            self.account_trustlines.values_mut().for_each(|v| v.retain(|k| k != key));
            return;
        }
        if self.checks.remove(key).is_some() { return; }
        if self.deposit_preauths.remove(key).is_some() { return; }
        if self.dids.remove(key).is_some() { return; }
        if self.escrows.remove(key).is_some() { return; }
        if self.paychans.remove(key).is_some() { return; }
        if self.tickets.remove(key).is_some() { return; }
        // NFTokens — scan by shamap_key
        let nft_id = self.nftokens.iter()
        ...
        if self.nft_offers.remove(key).is_some() { return; }
        if let Some(off) = self.offers.remove(key) {
            // Clean up secondary indexes
            let bk = offer::BookKey::from_amounts(&off.taker_pays, &off.taker_gets);
            if let Some(book) = self.order_books.get_mut(&bk) { book.remove(&off); }
            if let Some(v) = self.account_offers_idx.get_mut(&off.account) { v.retain(|k| k != key); }
            return;
        }
    ...
    fn restore_typed_entry(&mut self, entry: TypedEntry) {
        match entry {
            ...
            TypedEntry::Trustline(key, tl) => {
                ...
                if !low_idx.contains(&key) { low_idx.push(key); }
                let high_idx = self.account_trustlines.entry(tl.high_account).or_default();
                if !high_idx.contains(&key) { high_idx.push(key); }
                self.trustlines.insert(key, tl);
            }
            TypedEntry::Check(key, chk) => { self.checks.insert(key, chk); }
            TypedEntry::DepositPreauth(key, dp) => { self.deposit_preauths.insert(key, dp); }
            TypedEntry::Did(key, d) => { self.dids.insert(key, d); }
            TypedEntry::Escrow(key, esc) => { self.escrows.insert(key, esc); }
            TypedEntry::PayChannel(key, pc) => { self.paychans.insert(key, pc); }
            TypedEntry::Ticket(key, tkt) => { self.tickets.insert(key, tkt); }
            TypedEntry::NFToken(id, nft) => { self.nftokens.insert(id, nft); }
            TypedEntry::NFTokenOffer(key, off) => { self.nft_offers.insert(key, off); }
            TypedEntry::Offer(key, off) => {
                ...
                if !idx.contains(&key) { idx.push(key); }
                self.offers.insert(key, off);
            }
            TypedEntry::Directory(key, dir) => { self.directories.insert(key, dir); }
        }
    }
    ...
    pub fn remove_trustline(&mut self, key: &Key) -> bool {
        if self.trustlines.contains_key(key) {
            ...
            if let Some(v) = self.account_trustlines.get_mut(&tl.low_account) { v.retain(|k| k != key); }
            if let Some(v) = self.account_trustlines.get_mut(&tl.high_account) { v.retain(|k| k != key); }
            true
        } else {
            false
        }
    ...
    pub fn trustlines_for_account(&self, account: &[u8; 20]) -> Vec<&RippleState> {
        self.account_trustlines.get(account)
    ...
    pub fn get_nftoken_from_pages(&self, id: &[u8; 32]) -> Option<&nft_page::PageToken> {
        ...
        // Since we don't know the owner from the ID alone, scan all pages.
    ...
    pub fn insert_nftoken_paged(&mut self, owner: &[u8; 20], token_id: [u8; 32], uri: Option<Vec<u8>>) {
        use nft_page::*;

        let token = PageToken { nftoken_id: token_id, uri: uri.clone() };
        ...
        if needs_split {
            ...
            let first_upper = self.nft_pages[&new_page_key].tokens
                .first().map(|t| t.nftoken_id).unwrap_or([0xFF; 32]);
        ...
        self.nftokens.insert(token_id, NFToken {
            nftoken_id: token_id, owner: *owner, issuer: *owner,
            uri, flags: 0, transfer_fee: 0, taxon: 0,
        });
    }
    ...
    pub fn remove_nftoken_paged(&mut self, owner: &[u8; 20], token_id: &[u8; 32]) -> bool {
        ...
        if page.is_empty() {
            ...
            let merged = self.try_merge_nft_pages(page_key)
                || self.try_merge_nft_pages_next(page_key);
            if !merged {
    ...
    fn encode_nft_page(&self, page: &nft_page::NFTokenPage) -> Vec<u8> {
        ...
        let mut fields = vec![
            ...
            ParsedField { type_code: 15, field_code: 10, data: array_data },
        ];

        // sfPreviousPageMin (Hash256=5, field=26 per rippled sfields.macro)
        if let Some(ref prev) = page.prev_page {
            fields.push(ParsedField { type_code: 5, field_code: 26, data: prev.0.to_vec() });
        }
        // sfNextPageMin (Hash256=5, field=27 per rippled sfields.macro)
        if let Some(ref next) = page.next_page {
            fields.push(ParsedField { type_code: 5, field_code: 27, data: next.0.to_vec() });
        }
    ...
    pub fn iter_nft_pages_for(&self, owner: &[u8; 20]) -> impl Iterator<Item = (Key, &nft_page::NFTokenPage)> {
        let min = nft_page::page_min(owner);
        let max = nft_page::page_max(owner);
    ...
    pub fn offers_by_account(&self, account: &[u8; 20]) -> Vec<&Offer> {
        self.account_offers_idx.get(account)
    ...
    pub fn mark_all_dirty(&mut self) {
        for id in self.accounts.keys() { self.dirty_accounts.insert(*id); }
        for k in self.trustlines.keys() { self.dirty_trustlines.insert(*k); }
        for k in self.checks.keys() { self.dirty_checks.insert(*k); }
        for k in self.deposit_preauths.keys() { self.dirty_deposit_preauths.insert(*k); }
        for k in self.dids.keys() { self.dirty_dids.insert(*k); }
        for k in self.escrows.keys() { self.dirty_escrows.insert(*k); }
        for k in self.paychans.keys() { self.dirty_paychans.insert(*k); }
        for k in self.tickets.keys() { self.dirty_tickets.insert(*k); }
        for k in self.offers.keys() { self.dirty_offers.insert(*k); }
        for k in self.directories.keys() { self.dirty_directories.insert(*k); }
        for (key, _) in self.state_map.iter_leaves() { self.dirty_raw.insert(key); }
    }
    ...
    pub fn inspect_raw_lookup(&self, key: &Key) -> (bool, bool, bool, bool) {
        ...
        let nudb_present = self.nudb_map_guard()
    ...
    pub fn enable_sparse(&mut self) {
        ...
        if migrated > 0 {
            tracing::info!("enable_sparse: migrated {} state_map entries to sparse", migrated);
        }
    ...
    fn typed_overlay_key_sets(&self) -> (std::collections::BTreeSet<Key>, std::collections::BTreeSet<Key>) {
    ...
    fn overlay_hash_from_snapshot(
        snapshot: &mut shamap::SHAMap,
        state: &LedgerState,
    ) -> [u8; 32] {
    ...
    fn apply_current_overlay_to_snapshot(
        snapshot: &mut shamap::SHAMap,
        state: &LedgerState,
    ) {
    ...
    pub fn historical_state_map_from_root(
        &self,
        root_hash: [u8; 32],
    ) -> Option<shamap::SHAMap> {
        let backend = {
...
mod tests {
    ...
    fn peer_state_map_snapshot_includes_overlay_inserts() {
        ...
        let wire = snapshot
            .get_wire_node_by_id(&crate::ledger::shamap_id::SHAMapNodeID::from_key(&overlay_key.0))
