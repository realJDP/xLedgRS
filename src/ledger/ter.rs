...
pub fn code_to_message(code: i32) -> &'static str {
    ...
    if code >= 100 {
        return "The transaction failed but consumed the fee.";
    }
    if (-99..0).contains(&code) {
        return "The transaction may be retried.";
    }
    if (-199..-100).contains(&code) {
        return "A required field was missing or invalid.";
    }
    if (-299..-200).contains(&code) {
        return "The transaction was malformed.";
    }
    if (-399..-300).contains(&code) {
        return "A local error occurred.";
    }
    "Unknown result."
}
