...
mod tests {
    ...
    fn test_decode_incomplete_header() {
        ...
        assert!(
            result.is_none(),
            "should return None when header is incomplete"
        );
    }

    #[test]
    fn test_decode_incomplete_payload() {
        // The header advertises a 100-byte payload, but the buffer only
        // contains 10 bytes.
    ...
    fn test_compressed_roundtrip() {
        ...
        assert!(
            wire[0] & COMPRESSED_FLAG != 0,
            "should be a compressed frame"
        );
