...
fn build_vault_sle(
    ...
    let fields = vec![
        ...
        ParsedField { type_code: 2, field_code: 2, data: flags.to_be_bytes().to_vec() },
        ...
        ParsedField { type_code: 21, field_code: 2, data: share_mptid.to_vec() },
        // sfAssetsTotal (NUMBER=9, field=4) — 0 initially
        ParsedField { type_code: 9, field_code: 4, data: 0i64.to_be_bytes().to_vec() },
        // sfAssetsAvailable (NUMBER=9, field=2) — 0 initially
        ParsedField { type_code: 9, field_code: 2, data: 0i64.to_be_bytes().to_vec() },
        // sfLossUnrealized (NUMBER=9, field=5) — 0 initially
        ParsedField { type_code: 9, field_code: 5, data: 0i64.to_be_bytes().to_vec() },
        // sfWithdrawalPolicy (UINT8=16, field=20) — default: FirstComeFirstServe = 0
        ParsedField { type_code: 16, field_code: 20, data: vec![0] },
        // sfScale (UINT8=16, field=4) — 0 for XRP
        ParsedField { type_code: 16, field_code: 4, data: vec![0] },
    ];
...
fn build_share_issuance_sle(pseudo_account: &[u8; 20], sequence: u32) -> Vec<u8> {
    use crate::ledger::meta::ParsedField;
    crate::ledger::meta::build_sle(LT_MPT_ISSUANCE, &[
        // sfIssuer (ACCOUNT=8, field=4)
        ParsedField { type_code: 8, field_code: 4, data: pseudo_account.to_vec() },
        // sfSequence (UINT32=2, field=4)
        ParsedField { type_code: 2, field_code: 4, data: sequence.to_be_bytes().to_vec() },
        ...
        ParsedField { type_code: 3, field_code: 25, data: 0u64.to_be_bytes().to_vec() },
...
fn build_mptoken_sle(account: &[u8; 20], mptid: &[u8; 24], flags: u32) -> Vec<u8> {
    use crate::ledger::meta::ParsedField;
    crate::ledger::meta::build_sle(LT_MPTOKEN, &[
        // sfAccount (ACCOUNT=8, field=1)
        ParsedField { type_code: 8, field_code: 1, data: account.to_vec() },
        // sfMPTokenIssuanceID (UINT192=21, field=1)
        ParsedField { type_code: 21, field_code: 1, data: mptid.to_vec() },
        // sfMPTAmount (UINT64=3, field=26) — 0 initially
        ParsedField { type_code: 3, field_code: 26, data: 0u64.to_be_bytes().to_vec() },
...
fn vault_sle_share_mptid(raw: &[u8]) -> Option<[u8; 24]> {
...
pub(crate) fn apply_vault_delete(
    state: &mut LedgerState,
    tx: &ParsedTx,
) -> ApplyResult {
...
pub(crate) fn apply_vault_deposit(
    state: &mut LedgerState,
    tx: &ParsedTx,
) -> ApplyResult {
    // 1. Find the vault
    ...
    // 2. Get deposit amount (XRP drops for now)
...
pub(crate) fn apply_vault_withdraw(
    state: &mut LedgerState,
    tx: &ParsedTx,
) -> ApplyResult {
    // 1. Find the vault
    ...
    let assets_returned = ((shares_to_burn as u128 * assets_total as u128)
        / outstanding_shares as u128) as u64;
...
pub(crate) fn apply_vault_set(
    ...
    // For now, return Success and let metadata handle the actual field changes.
    ...
    // (sfVaultID field), we defer full implementation to when the amendment activates.
    bridge_metadata_only_tx(ctx, 66, "VaultSet", "tefFAILURE")
}
...
pub(crate) fn apply_vault_clawback(
    ...
    // For validator mode, we need the full VaultID → vault SLE → asset lookup chain.
    // Since this amendment is not yet active on mainnet, we return Success and
    // the amendment gate will block it until the amendment passes.
    tracing::debug!("VaultClawback: applying for account {:?}", hex::encode(tx.account));
    bridge_metadata_only_tx(ctx, 70, "VaultClawback", "tefFAILURE")
}
...
mod tests {
    ...
    fn vault_sle_round_trip() {
        ...
        for f in &parsed.fields {
            eprintln!("  field: type={} code={} len={}", f.type_code, f.field_code, f.data.len());
        }
        ...
        assert_eq!(extracted_pseudo, Some(pseudo), "should extract sfAccount (pseudo)");
