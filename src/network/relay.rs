#![allow(deprecated)]
//! Peer message relay — serialization helpers for broadcasting consensus and
//! transaction messages to connected peers over RTXP.
//!
//! Messages are encoded using Protocol Buffers (protobuf) for compatibility
//! with rippled nodes.  The proto types live in `crate::proto`.
...
/// Serialize a `Proposal` into an RTXP message (type = ProposeLedger) using protobuf.
...
/// Deserialize a `Proposal` from an RTXP ProposeLedger protobuf payload.
pub fn decode_proposal(data: &[u8]) -> Option<Proposal> {
    ...
    Some(Proposal {
        ledger_seq: 0, // TmProposeSet doesn't carry ledger_seq; caller may fill it in
        tx_set_hash,
...
/// Serialize a `Validation` into an RTXP message (type = Validation) using protobuf.
///
/// The `validation` field in TmValidation is the serialized validation object.
/// We pack our fields into a simple blob for now (similar to the old approach).
pub fn encode_validation(val: &Validation) -> RtxpMessage {
    // Build a blob containing our validation fields
...
/// Deserialize a `Validation` from an RTXP Validation protobuf payload.
pub fn decode_validation(data: &[u8]) -> Option<Validation> {
    ...
    // Parse it by walking field type/code prefixes.
    let mut pos = 0;
    ...
    while pos < blob.len() {
        ...
        match type_code {
            ...
            2 => {
                ...
                match field_code {
                    2 => { flags = val; }              // sfFlags
                    6 => { ledger_seq = val; }         // sfLedgerSequence
                    9 => { sign_time = val; }          // sfSigningTime
                    _ => {}
                }
                pos += 4;
            }
            3 => {
                // UInt64 — 8 bytes
                if pos + 8 > blob.len() { break; }
                pos += 8;
            }
            4 => {
                // Hash128 — 16 bytes
                if pos + 16 > blob.len() { break; }
                pos += 16;
            }
            5 => {
                // Hash256 — 32 bytes
                if pos + 32 > blob.len() { break; }
                match field_code {
                    1 => { ledger_hash.copy_from_slice(&blob[pos..pos+32]); } // LedgerHash
                    23 => { /* ConsensusHash — sfConsensusHash is field 23, not 9 */ }
                    _ => {}
                }
                pos += 32;
            }
            6 => {
                // Amount — variable (XRP = 8 bytes, IOU = 48 bytes)
                if pos + 8 > blob.len() { break; }
                let first = blob[pos];
            ...
            7 => {
                ...
                pos += vl_bytes;
                if pos + vl_len > blob.len() { break; }
                match field_code {
                    3 => { node_pubkey = blob[pos..pos+vl_len].to_vec(); }   // sfSigningPubKey (field 3)
                    6 => { signature = blob[pos..pos+vl_len].to_vec(); }     // sfSignature (field 6)
                    _ => {}
                }
                pos += vl_len;
            }
            ...
            14 => {
                // STObject — skip inner object
                // Read until end-of-object marker (0xE1)
                while pos < blob.len() && blob[pos] != 0xE1 { pos += 1; }
                if pos < blob.len() { pos += 1; } // skip 0xE1
            }
            15 => {
                // STArray — skip inner array
                while pos < blob.len() && blob[pos] != 0xF1 { pos += 1; }
                if pos < blob.len() { pos += 1; } // skip 0xF1
            }
            16 => {
                // UInt8 — 1 byte
                if pos >= blob.len() { break; }
            ...
            17 => {
                // Hash160 — 20 bytes
                if pos + 20 > blob.len() { break; }
                pos += 20;
            }
            18 => {
                // PathSet — variable, skip to end marker 0x00
                while pos < blob.len() && blob[pos] != 0x00 { pos += 1; }
                if pos < blob.len() { pos += 1; }
            }
            19 => {
...
pub fn decode_manifests(data: &[u8]) -> Vec<Manifest> {
    let Ok(pb) = proto::TmManifests::decode(data) else { return vec![] };
    pb.list.iter()
...
pub fn decode_validator_list_collection(data: &[u8]) -> Option<(Vec<u8>, Vec<PeerValidatorListBlob>)> {
    let pb = proto::TmValidatorListCollection::decode(data).ok()?;
    let blobs = pb.blobs.into_iter().map(|b| PeerValidatorListBlob {
        manifest:  b.manifest,
        blob:      b.blob,
...
pub fn encode_endpoints(addrs: &[SocketAddr]) -> RtxpMessage {
    let endpoints_v2: Vec<proto::tm_endpoints::TmEndpointv2> = addrs.iter().map(|addr| {
        proto::tm_endpoints::TmEndpointv2 {
...
pub fn decode_endpoints(data: &[u8]) -> Vec<SocketAddr> {
    ...
    pb.endpoints_v2.iter().filter_map(|ep| {
        ep.endpoint.parse::<SocketAddr>().ok()
    }).collect()
}
...
pub fn encode_get_ledger_state(
    ...
    let pb = proto::TmGetLedger {
        ...
        ltype: if has_hash { None } else { Some(proto::TmLedgerType::LtClosed as i32) },
        ledger_hash: if has_hash { Some(ledger_hash.to_vec()) } else { None },
        ...
        ledger_seq: if has_hash && ledger_seq > 0 { Some(ledger_seq) } else { None },
...
pub fn encode_get_ledger_leaves(
    ...
    let node_ids: Vec<Vec<u8>> = shamap_keys.iter().map(|key| {
...
pub fn encode_get_state_nodes_by_hash(
    ...
    let objects = nodes
        .iter()
        .map(|(hash, node_id)| crate::proto::TmIndexedObject {
            hash: Some(hash.to_vec()),
            index: None,
            data: None,
            ledger_seq: Some(ledger_seq),
            node_id: Some(node_id.to_vec()),
        })
        .collect();
...
pub fn encode_get_ledger_txs_for_hash(
    ledger_hash: &[u8; 32],
    cookie: u64,
) -> RtxpMessage {
    let pb = proto::TmGetLedger {
...
pub fn decode_history_ledger(data: &[u8]) -> Option<(crate::ledger::LedgerHeader, Vec<crate::ledger::history::TxRecord>)> {
    if data.len() < 8 { return None; }
    let mut pos = 0;
    let hdr_len = u32::from_be_bytes(data[pos..pos+4].try_into().ok()?) as usize;
    pos += 4;
    if pos + hdr_len > data.len() { return None; }
    let header: crate::ledger::LedgerHeader = bincode::deserialize(&data[pos..pos+hdr_len]).ok()?;
    pos += hdr_len;
    ...
    for _ in 0..tx_count {
        ...
        if let Ok(rec) = bincode::deserialize::<crate::ledger::history::TxRecord>(&data[pos..pos+rec_len]) {
            tx_records.push(rec);
        }
        pos += rec_len;
    }
...
pub fn encode_squelch(validator_pubkey: &[u8], squelch: bool, duration_secs: Option<u32>) -> RtxpMessage {
    use prost::Message;
...
mod tests {
    ...
    // We only relay validations from rippled, never encode our own.
    ...
    fn test_validation_decode_field_mapping() {
        ...
        assert_eq!(decoded.node_pubkey, pubkey, "node_pubkey should be field 3 (sfSigningPubKey)");
        assert_eq!(decoded.signature, sig, "signature should be field 6 (sfSignature)");
