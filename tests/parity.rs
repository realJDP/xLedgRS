...
use xrpl::ledger::tx::{run_tx, TxContext, ApplyOutcome, classify_result};
use xrpl::ledger::{AccountRoot, LedgerState};
use xrpl::transaction::{Amount, builder::TxBuilder, parse_blob};
use xrpl::transaction::parse::ParsedTx;

fn pctx(close_time: u64) -> TxContext {
    TxContext { close_time, ..TxContext::default() }
}
...
fn alice_id() -> [u8; 20] { xrpl::crypto::account_id(&kp_alice().public_key_bytes()) }
fn bob_id() -> [u8; 20] { xrpl::crypto::account_id(&kp_bob().public_key_bytes()) }
fn carol_id() -> [u8; 20] { xrpl::crypto::account_id(&kp_carol().public_key_bytes()) }
fn bob_addr() -> String { xrpl::crypto::base58::encode_account(&bob_id()) }
...
fn payment_xrp_basic() {
    ...
    assert!(result.ter.is_tes_success(), "expected tesSUCCESS, got {}", result.ter);
...
fn payment_sequence_increments() {
    ...
    for seq in 1..=3 {
        ...
        let tx = parse_blob(&signed.blob).unwrap();
        let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
        assert!(result.ter.is_tes_success(), "tx seq={} failed: {}", seq, result.ter);
    }
...
fn escrow_create_locks_xrp() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "escrow create failed: {}", result.ter);
...
fn escrow_finish_releases_xrp() {
    ...
    assert!(result.ter.is_tes_success(), "escrow finish failed: {}", result.ter);
...
fn trustset_creates_trust_line() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "trustset failed: {}", result.ter);
    ...
    assert!(state.get_trustline_for(&alice_id(), &bob_id(), &usd).is_some());
}
...
fn trustset_zero_limit_removes_trust_line() {
    ...
    let tx = parse_blob(&signed.blob).unwrap();
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "trustset zero failed: {}", result.ter);

    let a = state.get_account(&alice_id()).unwrap();
    assert_eq!(a.owner_count, 0);

    let usd = xrpl::transaction::amount::Currency::from_code("USD").unwrap();
    assert!(state.get_trustline_for(&alice_id(), &bob_id(), &usd).is_none());
}
...
fn offer_create_places_offer() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "offer create failed: {}", result.ter);
...
fn offer_cancel_removes_offer() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "offer cancel failed: {}", result.ter);
...
fn check_create_increases_owner_count() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "check create failed: {}", result.ter);
...
fn paychan_create_locks_xrp() {
    ...
    let result = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "paychan create failed: {}", result.ter);
...
fn offer_crossing_full_fill() {
    ...
    let signed = TxBuilder::trust_set()
        .account(&alice)
        .limit_amount(iou_usd(10_000.0, bob_id()))
        .fee(BASE_FEE).sequence(1).sign(&alice).unwrap();
    run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);

    // Carol trust line to bob for USD
    let signed = TxBuilder::trust_set()
        .account(&carol)
        .limit_amount(iou_usd(10_000.0, bob_id()))
        .fee(BASE_FEE).sequence(1).sign(&carol).unwrap();
    run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);

    // Give Carol 50 USD via IOU payment from bob
    // (Bob is the issuer, so he can "create" USD by paying Carol)
    // Bob doesn't need a trust line to himself — just credit Carol's line
    // We need to directly set Carol's trust line balance instead
    {
        let usd = xrpl::transaction::amount::Currency::from_code("USD").unwrap();
        let key = xrpl::ledger::trustline::shamap_key(&carol_id(), &bob_id(), &usd);
        if let Some(tl) = state.get_trustline(&key) {
            let mut tl = tl.clone();
            // Bob is the issuer, so seed Carol's balance via the trust-line
            // transfer helper instead of assuming a raw low/high sign.
            tl.transfer(&bob_id(), &xrpl::transaction::amount::IouValue::from_f64(50.0));
    ...
    let signed = TxBuilder::offer_create()
        ...
        .taker_pays(iou_usd(50.0, bob_id()))
        .taker_gets(Amount::Xrp(100 * XRP))
        .fee(BASE_FEE).sequence(2).sign(&alice).unwrap();
    let r = run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);
    ...
    let signed = TxBuilder::offer_create()
        ...
        .fee(BASE_FEE).sequence(2).sign(&carol).unwrap();
    let r = run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tes_success(), "carol crossing offer failed: {}", r.ter);

    // After crossing: Alice gave 100 XRP, received 50 USD
    // Carol gave 50 USD, received 100 XRP
    let a = state.get_account(&alice_id()).unwrap();
    assert!(a.balance < alice_pre.balance, "alice should have less XRP after crossing");

    let c = state.get_account(&carol_id()).unwrap();
    assert!(c.balance > 5_000 * XRP - 2 * BASE_FEE, "carol should have gained XRP");
}
...
fn escrow_finish_too_early_fails() {
    ...
    let signed = TxBuilder::escrow_create()
        ...
        .destination(&bob_addr()).unwrap()
        .amount(Amount::Xrp(1_000 * XRP))
        .fee(BASE_FEE).sequence(1).finish_after(500)
        .sign(&alice).unwrap();
    run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);
    ...
    let signed = TxBuilder::escrow_finish()
        .account(&bob)
        .fee(BASE_FEE).sequence(1).offer_sequence(1)
        .sign(&bob).unwrap();
    ...
    assert!(result.ter.is_tec_claim() || result.ter.token().contains("PERMISSION"),
        "expected failure for early finish, got {}", result.ter);
...
fn escrow_cancel_after_expiry() {
    ...
    let signed = TxBuilder::escrow_create()
        ...
        .destination(&bob_addr()).unwrap()
        .amount(Amount::Xrp(1_000 * XRP))
        .fee(BASE_FEE).sequence(1)
        .finish_after(200).cancel_after(300)
        .sign(&alice).unwrap();
    let r = run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);
    ...
    let signed = TxBuilder::escrow_cancel()
        .account(&alice)
        .fee(BASE_FEE).sequence(2).offer_sequence(1)
        .sign(&alice).unwrap();
    let mut tx = parse_blob(&signed.blob).unwrap();
    tx.owner = Some(alice_id());

    let result = run_tx(&mut state, &tx, &pctx(400), ApplyFlags::NONE);
    assert!(result.ter.is_tes_success(), "escrow cancel failed: {}", result.ter);

    // XRP should be returned to alice (minus fees)
    let a = state.get_account(&alice_id()).unwrap();
    assert!(a.balance > alice_after_create, "alice should have gotten XRP back");
    assert_eq!(a.owner_count, 0);
}
...
fn close_skips_unfunded_tx() {
    ...
    // Balances unchanged
    assert_eq!(state.get_account(&alice_id()).unwrap().balance, 10_000 * XRP);
...
fn offer_no_cross_places_standing() {
    // Offer with no matching book → just placed as standing offer
    let mut state = LedgerState::new();
    let alice = kp_alice();
    let bob = kp_bob();
    fund(&mut state, &alice, 5_000);
...
fn offer_partial_cross() {
    ...
    for (kp, seq) in [(&alice, 1u32), (&carol, 1u32)] {
        let signed = TxBuilder::trust_set()
            .account(kp)
            .limit_amount(iou_usd(10_000.0, bob_id()))
            .fee(BASE_FEE).sequence(seq).sign(kp).unwrap();
        run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(100), ApplyFlags::NONE);
    }
    ...
    // Give Carol 25 USD
    {
        ...
        if let Some(tl) = state.get_trustline(&key) {
            let mut tl = tl.clone();
            tl.transfer(&bob_id(), &xrpl::transaction::amount::IouValue::from_f64(25.0));
            state.insert_trustline(tl);
        }
    }
    ...
    let c = state.get_account(&carol_id()).unwrap();
    assert!(c.balance > 5_000 * XRP - 2 * BASE_FEE, "carol should have more XRP");

    // Alice should have lost 50 XRP and her offer should still be standing (partially filled)
    let a = state.get_account(&alice_id()).unwrap();
    assert!(a.balance < alice_pre, "alice should have less XRP after partial cross");
    // Alice still owns trust line + remaining offer
    assert!(a.owner_count >= 2, "alice should still own trust line + partial offer");
}
...
fn vault_delete_empty() {
    ...
    assert!(state.get_raw(&vkey).is_none(), "vault SLE should be removed");
...
fn vault_deposit_xrp() {
    ...
    // Vault should have 100 XRP in pseudo-account
    // (we can't easily find pseudo-account address from test, but account_count > 1)
...
fn vault_delete_nonempty_fails() {
    ...
    assert_eq!(r.ter, xrpl::ledger::ter::TEC_HAS_OBLIGATIONS,
        "expected tecHAS_OBLIGATIONS, got {}", r.ter);
}
...
fn vault_withdraw_to_empty_then_delete() {
    ...
    assert!(r.ter.is_tes_success(), "delete after full withdraw failed: {}", r.ter);
...
fn loan_broker_create_basic() {
    ...
    assert!(r.ter.is_tes_success(), "loan broker create failed: {}", r.ter);
...
fn loan_broker_create_no_vault_fails() {
    ...
    let r = run_tx(&mut state, &broker_tx, &pctx(100), ApplyFlags::NONE);
    assert_eq!(r.ter, xrpl::ledger::ter::TEC_NO_ENTRY, "expected tecNO_ENTRY, got {}", r.ter);
}
...
fn test_amm_key(issue1: &xrpl::transaction::amount::Issue, issue2: &xrpl::transaction::amount::Issue) -> xrpl::ledger::Key {
    ...
    let (min_acct, min_cur, max_acct, max_cur) = match a_cur.cmp(&b_cur) {
        ...
        std::cmp::Ordering::Equal => {
            if a_acct <= b_acct { (a_acct, a_cur, b_acct, b_cur) }
            else { (b_acct, b_cur, a_acct, a_cur) }
        }
    };
...
fn amm_create_basic() {
    ...
    use xrpl::transaction::amount::{Issue, Currency};
...
fn amm_create_same_asset_fails() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tec_claim() || r.ter.token().contains("BAD_AMM"),
        "expected failure for same asset pair, got {}", r.ter);
}
...
fn amm_create_duplicate_fails() {
    ...
    let xrp = Issue::Xrp;
    let usd = Issue::Iou { currency: Currency::from_code("USD").unwrap(), issuer: bob_id() };
    ...
    assert_eq!(r.ter, xrpl::ledger::ter::TEC_DUPLICATE, "expected tecDUPLICATE, got {}", r.ter);
}
...
fn amm_delete_nonempty_fails() {
    ...
    let akey = test_amm_key(&xrp, &usd);
    assert!(state.get_raw(&akey).is_some(), "AMM SLE should exist before delete");

    let mut del = mpt_tx(40, alice_id(), 3, BASE_FEE);
    del.asset = Some(xrp.clone());
    del.asset2 = Some(usd.clone());
    let r = run_tx(&mut state, &del, &pctx(100), ApplyFlags::NONE);
    assert_eq!(r.ter.to_string(), "tecHAS_OBLIGATIONS", "expected tecHAS_OBLIGATIONS, got {}", r.ter);

    assert!(state.get_raw(&akey).is_some(), "AMM SLE should remain after failed delete");
}
...
fn nft_page_sle_exists_in_shamap() {
    ...
    assert!(raw.is_some(), "page SLE should exist in state_map at page_max key");
}
...
fn nft_page_split_creates_two_sles() {
    ...
    assert!(page_count >= 2, "should have at least 2 pages after split, got {}", page_count);
    ...
    assert!(sle_count >= 2, "should have at least 2 page SLEs in state_map");
}
...
fn nft_page_empty_page_deleted_from_shamap() {
    ...
    let max_key = xrpl::ledger::nft_page::page_max(&alice_id());
    assert!(state.get_raw(&max_key).is_some(), "page should exist after mint");
    ...
    assert!(state.get_raw(&max_key).is_none(), "empty page SLE should be deleted");
...
fn nft_page_transfer_moves_between_owners() {
    ...
    assert_eq!(state.nft_page_count(&alice_id()), 0, "alice should have no pages");
    assert!(state.nft_page_count(&bob_id()) >= 1, "bob should have a page");
}
...
fn nft_page_merge_on_removal() {
    ...
    assert!(state.nft_page_count(&alice_id()) >= 2, "should have 2+ pages after 33 mints");

    // Burn tokens until we're back to <=32 total (should trigger merge)
    let ids: Vec<[u8; 32]> = state.iter_nftokens()
    ...
    let page_count = state.nft_page_count(&alice_id());
    assert_eq!(page_count, 1, "should have merged to 1 page, got {}", page_count);
...
fn nft_burn_nonexistent_fails() {
    ...
    let r = run_tx(&mut state, &burn_tx, &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tec_claim(), "expected tec for nonexistent NFT, got {}", r.ter);
}
...
fn mpt_tx(tx_type: u16, account: [u8; 20], seq: u32, fee: u64) -> ParsedTx {
    ParsedTx {
        tx_type, account, sequence: seq, fee, flags: 0,
        amount_drops: None, destination: None, amount: None,
        limit_amount: None, taker_pays: None, taker_gets: None,
        deliver_min: None, offer_sequence: None, finish_after: None,
        cancel_after: None, settle_delay: None, expiration: None,
        set_flag: None, clear_flag: None, transfer_rate: None,
        tick_size: None, last_ledger_seq: None, ticket_count: None,
        ticket_sequence: None, domain: None,
        channel: None, public_key: None, paychan_sig: None,
        nftoken_id: None, nft_sell_offer: None, nft_buy_offer: None,
        uri: None, did_document: None, did_data: None,
        nftoken_taxon: None, transfer_fee_field: None,
        asset_scale: None, maximum_amount: None, mutable_flags: None,
        mptoken_metadata: None, owner: None, regular_key: None,
        issuer: None, subject: None, credential_type: None,
        oracle_document_id: None, signer_quorum: None,
        signer_entries_raw: None, domain_id: None, ledger_fix_type: None,
        accepted_credentials_raw: None, authorize: None,
        permissions_raw: None, holder: None,
        mptoken_issuance_id: None, asset: None, asset2: None,
        vault_id: None, amendment: None, base_fee_field: None,
        reserve_base_field: None, reserve_increment_field: None,
        unl_modify_disabling: None, unl_modify_validator: None,
        send_max: None, paths: Vec::new(),
        signing_pubkey: vec![0x02; 33], signature: vec![0; 64],
        signing_hash: [0u8; 32], signing_payload: vec![],
        signers: Vec::new(),
    }
}
...
fn sle_flags(raw: &[u8]) -> u32 {
    xrpl::ledger::meta::parse_sle(raw)
        .and_then(|sle| {
            sle.fields.into_iter().find_map(|field| {
                (field.type_code == 2 && field.field_code == 2 && field.data.len() >= 4)
...
fn mpt_issuance_create_basic() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tes_success(), "mpt issuance create failed: {}", r.ter);
...
fn mpt_issuance_destroy_basic() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tes_success(), "mpt issuance destroy failed: {}", r.ter);
...
fn mpt_issuance_destroy_nonexistent() {
    ...
    assert!(!r.ter.is_tes_success(), "expected failure for nonexistent issuance");
}
...
fn mpt_authorize_holder_creates_mptoken() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(r.ter.is_tes_success(), "mpt authorize (holder create) failed: {}", r.ter);
...
fn mpt_payment_direct_creates_holder_and_tracks_outstanding() {
    ...
    assert!(r.ter.is_tes_success(), "direct mpt payment failed: {}", r.ter);

    let issuance = state.get_raw(&mpt_issuance_key_for(&mptid)).unwrap().to_vec();
    assert_eq!(sle_u64(&issuance, 4), 100, "outstanding should increase when issuer pays holder");

    let bob_token = state.get_raw(&mpt_holder_key_for(&mptid, &bob_id())).unwrap().to_vec();
    assert_eq!(sle_u64(&bob_token, 26), 100, "destination holder should receive mpt balance");
    assert_eq!(state.get_account(&bob_id()).unwrap().owner_count, 1, "payment should auto-create bob's MPToken entry");
}
...
fn mpt_payment_secondary_sale_charges_transfer_fee() {
    ...
    create.maximum_amount = Some(1_000);
    assert!(run_tx(&mut state, &create, &pctx(100), ApplyFlags::NONE).ter.is_tes_success());
    ...
    assert!(run_tx(&mut state, &issue_to_bob, &pctx(100), ApplyFlags::NONE).ter.is_tes_success());
    ...
    assert!(r.ter.is_tes_success(), "secondary transfer failed: {}", r.ter);

    let issuance = state.get_raw(&mpt_issuance_key_for(&mptid)).unwrap().to_vec();
    let bob_token = state.get_raw(&mpt_holder_key_for(&mptid, &bob_id())).unwrap().to_vec();
    let carol_token = state.get_raw(&mpt_holder_key_for(&mptid, &carol_id())).unwrap().to_vec();

    assert_eq!(sle_u64(&bob_token, 26), 95, "sender should pay delivered amount plus 5% fee");
    assert_eq!(sle_u64(&carol_token, 26), 100, "recipient should receive the full delivered amount");
    assert_eq!(sle_u64(&issuance, 4), 195, "secondary transfer fee should reduce outstanding by the issuer-collected fee");
}
...
fn mpt_payment_require_auth_blocks_unauthorized_recipient() {
    ...
    assert!(run_tx(&mut state, &holder_create, &pctx(100), ApplyFlags::NONE).ter.is_tes_success());

    let mut payment = mpt_tx(0, alice_id(), 2, BASE_FEE);
    payment.destination = Some(bob_id());
    payment.amount = Some(Amount::from_mpt_value(50, mptid));
    let r = run_tx(&mut state, &payment, &pctx(100), ApplyFlags::NONE);
    assert_eq!(r.ter, xrpl::ledger::ter::TEC_NO_AUTH, "unauthorized holder should not receive MPT");

    let bob_token = state.get_raw(&mpt_holder_key_for(&mptid, &bob_id())).unwrap().to_vec();
    assert_eq!(sle_u64(&bob_token, 26), 0, "failed payment must not credit destination");
}
...
fn mpt_clawback_reduces_holder_and_outstanding() {
    ...
    assert!(run_tx(&mut state, &payment, &pctx(100), ApplyFlags::NONE).ter.is_tes_success());
...
fn mpt_global_lock_blocks_secondary_transfer_but_allows_return_to_issuer() {
    ...
    assert!(run_tx(&mut state, &lock, &pctx(100), ApplyFlags::NONE).ter.is_tes_success());

    let issuance = state.get_raw(&mpt_issuance_key_for(&mptid)).unwrap().to_vec();
    assert_ne!(sle_flags(&issuance) & 0x0000_0001, 0, "global lock should set issuance locked flag");
    ...
    assert!(r.ter.is_tes_success(), "locked holder should still be able to return value to issuer");
...
fn ter_payment_self_payment_rejected() {
    ...
    let signed = TxBuilder::payment()
        .account(&alice)
        .destination(&alice_addr_str).unwrap()
        .amount(Amount::Xrp(100 * XRP))
        .fee(BASE_FEE).sequence(1)
        .sign(&alice).unwrap();
    let tx = parse_blob(&signed.blob).unwrap();
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "self-payment should fail, got {}", r.ter);
}
...
fn ter_nft_mint_bad_transfer_fee() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "bad transfer fee should fail, got {}", r.ter);
}
...
fn ter_nft_mint_transfer_fee_without_transferable() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "fee without transferable flag should fail, got {}", r.ter);
}
...
fn ter_set_regular_key_self_rejected() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "regular key == account should fail, got {}", r.ter);
}
...
fn ter_account_set_bad_transfer_rate() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "bad transfer rate should fail, got {}", r.ter);
}
...
fn ter_account_set_bad_tick_size() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "bad tick size should fail, got {}", r.ter);
}
...
fn ter_account_set_conflicting_flags() {
    ...
    let r = run_tx(&mut state, &tx, &pctx(100), ApplyFlags::NONE);
    assert!(!r.ter.is_tes_success(), "conflicting flags should fail, got {}", r.ter);
}
...
fn pool_payment(
    pool: &mut TxPool, kp: &KeyPair, dest_addr: &str,
    amount: u64, fee: u64, seq: u32,
) {
    let signed = TxBuilder::payment()
        .account(kp).destination(dest_addr).unwrap()
        .amount(Amount::Xrp(amount)).fee(fee).sequence(seq)
        .sign(kp).unwrap();
    let mut data = Vec::with_capacity(4 + signed.blob.len());
    data.extend_from_slice(&[0x54, 0x58, 0x4E, 0x00]);
    data.extend_from_slice(&signed.blob);
...
fn do_close(prev: &LedgerHeader, state: &mut LedgerState, pool: &mut TxPool, ct: u64) -> CloseResult {
...
fn replay_payment_chain_across_closes() {
    ...
    let mut header = LedgerHeader {
        sequence: 1, hash: [0xAA; 32], parent_hash: [0u8; 32],
        close_time: 0, total_coins: 11_000 * XRP,
        account_hash: state.state_hash(), transaction_hash: [0u8; 32],
    ...
    for ledger in 1..=3u32 {
        ...
        assert_eq!(result.applied_count, 1, "ledger {} should apply 1 tx", ledger + 1);
...
fn replay_offer_create_then_cancel() {
    ...
    // Trust line first
    let signed = TxBuilder::trust_set()
        .account(&alice).limit_amount(iou_usd(10_000.0, bob_id()))
        .fee(BASE_FEE).sequence(1).sign(&alice).unwrap();
    run_tx(&mut state, &parse_blob(&signed.blob).unwrap(), &pctx(0), ApplyFlags::NONE);

    let header = LedgerHeader {
        sequence: 1, hash: [0xBB; 32], parent_hash: [0u8; 32],
        close_time: 0, total_coins: 10_000 * XRP,
        account_hash: state.state_hash(), transaction_hash: [0u8; 32],
        parent_close_time: 0, close_time_resolution: 10, close_flags: 0,
    };
    ...
    // Ledger 3: cancel offer
    {
        let mut pool = TxPool::new();
        let signed = TxBuilder::offer_cancel()
            .account(&alice).offer_sequence(2)
            .fee(BASE_FEE).sequence(3).sign(&alice).unwrap();
        let mut d = Vec::with_capacity(4 + signed.blob.len());
        d.extend_from_slice(&[0x54, 0x58, 0x4E, 0x00]);
        d.extend_from_slice(&signed.blob);
        pool.insert(xrpl::crypto::sha512_first_half(&d), signed.blob.clone(),
...
fn replay_vault_lifecycle() {
    ...
    let header = LedgerHeader {
        sequence: 1, hash: [0xCC; 32], parent_hash: [0u8; 32],
        close_time: 0, total_coins: 10_000 * XRP,
        account_hash: state.state_hash(), transaction_hash: [0u8; 32],
        parent_close_time: 0, close_time_resolution: 10, close_flags: 0,
    };
    ...
    let r = run_tx(&mut state, &mpt_tx(65, alice_id(), 1, BASE_FEE), &ctx(&header, 200), ApplyFlags::NONE);
    assert!(r.ter.is_tes_success());
    assert_eq!(state.get_account(&alice_id()).unwrap().owner_count, 2);
    ...
    assert!(run_tx(&mut state, &dep, &ctx(&header, 300), ApplyFlags::NONE).ter.is_tes_success());

    let mut wd = mpt_tx(69, alice_id(), 3, BASE_FEE);
    wd.vault_id = Some(vkey.0);
    wd.amount_drops = Some(500 * XRP);
    assert!(run_tx(&mut state, &wd, &ctx(&header, 400), ApplyFlags::NONE).ter.is_tes_success());

    let mut del = mpt_tx(67, alice_id(), 4, BASE_FEE);
    del.vault_id = Some(vkey.0);
    assert!(run_tx(&mut state, &del, &ctx(&header, 500), ApplyFlags::NONE).ter.is_tes_success());
    assert!(state.get_raw(&vkey).is_none());
    assert_eq!(state.get_account(&alice_id()).unwrap().owner_count, 0);
}
...
fn replay_loan_lifecycle() {
    ...
    let header = LedgerHeader {
        sequence: 1, hash: [0xDD; 32], parent_hash: [0u8; 32],
        close_time: 0, total_coins: 15_000 * XRP,
        account_hash: state.state_hash(), transaction_hash: [0u8; 32],
        parent_close_time: 0, close_time_resolution: 10, close_flags: 0,
    };
    ...
    assert!(run_tx(&mut state, &mpt_tx(65, alice_id(), 1, BASE_FEE), &ctx(100), ApplyFlags::NONE).ter.is_tes_success());
    let vkey = test_vault_key(&alice_id(), 1);
    let mut dep = mpt_tx(68, alice_id(), 2, BASE_FEE);
    dep.vault_id = Some(vkey.0); dep.amount_drops = Some(1_000 * XRP);
    assert!(run_tx(&mut state, &dep, &ctx(100), ApplyFlags::NONE).ter.is_tes_success());
    let mut bt = mpt_tx(74, alice_id(), 3, BASE_FEE);
    bt.vault_id = Some(vkey.0);
    assert!(run_tx(&mut state, &bt, &ctx(100), ApplyFlags::NONE).ter.is_tes_success());
    let bkey = test_loan_broker_key(&alice_id(), 3);

    // Bob: loan → repay → delete
    let mut lt = mpt_tx(80, bob_id(), 1, BASE_FEE);
    lt.vault_id = Some(bkey.0); lt.amount_drops = Some(100 * XRP);
    assert!(run_tx(&mut state, &lt, &ctx(200), ApplyFlags::NONE).ter.is_tes_success());
    let lkey = test_loan_key(&bkey.0, 1);

    let mut pay = mpt_tx(84, bob_id(), 2, BASE_FEE);
    pay.vault_id = Some(lkey.0); pay.amount_drops = Some(100 * XRP);
    assert!(run_tx(&mut state, &pay, &ctx(300), ApplyFlags::NONE).ter.is_tes_success());

    let mut dl = mpt_tx(81, bob_id(), 3, BASE_FEE);
    dl.vault_id = Some(lkey.0);
    assert!(run_tx(&mut state, &dl, &ctx(400), ApplyFlags::NONE).ter.is_tes_success());
    assert!(state.get_raw(&lkey).is_none());
    assert_eq!(state.get_account(&bob_id()).unwrap().owner_count, 0);
    ...
    assert!(run_tx(&mut state, &db, &ctx(500), ApplyFlags::NONE).ter.is_tes_success());
    assert!(state.get_raw(&bkey).is_none());
}
...
fn replay_amm_lifecycle() {
    ...
    let header = LedgerHeader {
        sequence: 1, hash: [0xEE; 32], parent_hash: [0u8; 32],
        close_time: 0, total_coins: 20_000 * XRP,
        account_hash: state.state_hash(), transaction_hash: [0u8; 32],
        parent_close_time: 0, close_time_resolution: 10, close_flags: 0,
    };
    let ctx = |ct: u64| TxContext::from_parent(&header, ct);
    ...
    // Create → deposit → withdraw → delete
    let mut tx = mpt_tx(35, alice_id(), 1, BASE_FEE);
    tx.asset = Some(xrp.clone()); tx.asset2 = Some(usd.clone());
    assert!(run_tx(&mut state, &tx, &ctx(100), ApplyFlags::NONE).ter.is_tes_success());

    let mut dep = mpt_tx(36, alice_id(), 2, BASE_FEE);
    dep.asset = Some(xrp.clone()); dep.asset2 = Some(usd.clone());
    dep.amount_drops = Some(200 * XRP);
    assert!(run_tx(&mut state, &dep, &ctx(200), ApplyFlags::NONE).ter.is_tes_success());

    let bal = state.get_account(&alice_id()).unwrap().balance;
    let mut wd = mpt_tx(37, alice_id(), 3, BASE_FEE);
    wd.asset = Some(xrp.clone()); wd.asset2 = Some(usd.clone());
    wd.amount_drops = Some(200 * XRP);
    assert!(run_tx(&mut state, &wd, &ctx(300), ApplyFlags::NONE).ter.is_tes_success());
    assert_eq!(state.get_account(&alice_id()).unwrap().balance, bal + 200 * XRP - BASE_FEE);

    let mut del = mpt_tx(40, alice_id(), 4, BASE_FEE);
    del.asset = Some(xrp.clone()); del.asset2 = Some(usd.clone());
    assert!(run_tx(&mut state, &del, &ctx(400), ApplyFlags::NONE).ter.is_tes_success());
