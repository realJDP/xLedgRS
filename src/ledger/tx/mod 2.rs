...
pub fn apply_tx(state: &mut LedgerState, tx: &ParsedTx, ctx: &TxContext) -> ApplyResult {
    ...
    // TODO: actually consume the ticket SLE and decrement owner_count
    if tx.sequence != 0 {
    ...
    let result = match tx.tx_type {
        2 | 4 | 7 | 8 | 15 | 17 | 18 | 26 | 28 | 29 | 30 | 31 | 35 | 36 | 37 | 38 | 39 | 40 | 59 | 60 | 65 | 67 | 68 | 69 | 74 | 75 | 80 | 81 | 84 => {
            ...
            match tx.tx_type {
                ...
                7  => offer::apply_offer_create(state, tx),                        // OfferCreate
                ...
                30 => clawback::apply_clawback(state, tx),                         // Clawback
        ...
        _ => {
            let r = match tx.tx_type {
                0  => payment::apply_payment(state, tx, &mut new_sender),                    // Payment
                ...
                41 | 42 | 43 | 44 | 45 | 46 | 47 | 48 => xchain::apply_xchain(),          // XChain*
                ...
                61 => nftoken_modify::apply_nftoken_modify(),                                 // NFTokenModify
                ...
                // ── Vault types ──────────────────────────────────────────────
                66 => vault::apply_vault_set(state, tx),                         // VaultSet
                70 => vault::apply_vault_clawback(state, tx),                    // VaultClawback
                71 => batch::apply_batch_stub(),                                             // Batch (not active)
                // ── Loan types ──────────────────────────────────────────────
                76 | 77 | 78 | 82 => loan::apply_loan_modify(),                       // LoanBrokerCover*/LoanManage/LoanPay
                // ── Unknown/future tx types — fee+seq only ───────────────────
                _ => {
                    tracing::warn!("apply_tx: unknown tx type {} — fee+seq only", tx.tx_type);
                    ApplyResult::Success
                }
            };
...
mod tests {
    ...
    fn test_offer_no_crossing_when_prices_dont_overlap() {
        ...
        let tx1 = sign_offer_create(1, usd(100.0), Amount::Xrp(1_000_000));
        apply_tx(&mut state, &tx1, &ctx(0));

        let tx2 = sign_offer_create(2, Amount::Xrp(1_000_000), usd(1.0));
        apply_tx(&mut state, &tx2, &ctx(0));

        assert_eq!(state.offers_by_account(&genesis_id()).len(), 2);
    }
