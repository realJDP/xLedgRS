...
use crate::transaction::serialize::{Field, FieldValue, serialize_fields, signing_hash, PREFIX_TX_SIGN};
...
impl TxBuilder {
    ...
    pub fn payment() -> Self { Self::new(TransactionType::Payment) }
    pub fn account_set() -> Self { Self::new(TransactionType::AccountSet) }
    pub fn trust_set() -> Self { Self::new(TransactionType::TrustSet) }
    pub fn offer_create() -> Self { Self::new(TransactionType::OfferCreate) }
    pub fn offer_cancel() -> Self { Self::new(TransactionType::OfferCancel) }
    pub fn escrow_create() -> Self { Self::new(TransactionType::EscrowCreate) }
    pub fn escrow_finish() -> Self { Self::new(TransactionType::EscrowFinish) }
    pub fn escrow_cancel() -> Self { Self::new(TransactionType::EscrowCancel) }
    pub fn paychan_create() -> Self { Self::new(TransactionType::PaymentChannelCreate) }
    pub fn paychan_fund() -> Self { Self::new(TransactionType::PaymentChannelFund) }
    pub fn paychan_claim() -> Self { Self::new(TransactionType::PaymentChannelClaim) }
    pub fn check_create() -> Self { Self::new(TransactionType::CheckCreate) }
    pub fn check_cash() -> Self { Self::new(TransactionType::CheckCash) }
    pub fn check_cancel() -> Self { Self::new(TransactionType::CheckCancel) }
    ...
    pub fn build_fields(&self, signing_pubkey: Vec<u8>, signature: Option<Vec<u8>>) -> Result<Vec<Field>> {
        let account = self.account.ok_or_else(|| anyhow::anyhow!("account not set"))?;

        let mut fields = vec![
            Field { def: field::TRANSACTION_TYPE, value: FieldValue::UInt16(self.tx_type as u16) },
            Field { def: field::FLAGS,            value: FieldValue::UInt32(self.flags) },
            Field { def: field::SEQUENCE,         value: FieldValue::UInt32(self.sequence) },
            Field { def: field::FEE,              value: FieldValue::Amount(Amount::Xrp(self.fee)) },
            Field { def: field::SIGNING_PUB_KEY,  value: FieldValue::Blob(signing_pubkey) },
            Field { def: field::ACCOUNT,          value: FieldValue::AccountID(account) },
        ];

        if let Some(tag) = self.source_tag {
            fields.push(Field { def: field::SOURCE_TAG, value: FieldValue::UInt32(tag) });
        }
        if let Some(os) = self.offer_sequence {
            fields.push(Field { def: field::OFFER_SEQUENCE, value: FieldValue::UInt32(os) });
        }
        if let Some(lls) = self.last_ledger {
            fields.push(Field { def: field::LAST_LEDGER_SEQUENCE, value: FieldValue::UInt32(lls) });
        }
        if let Some(fa) = self.finish_after {
            fields.push(Field { def: field::FINISH_AFTER, value: FieldValue::UInt32(fa) });
        }
        if let Some(ca) = self.cancel_after {
            fields.push(Field { def: field::CANCEL_AFTER, value: FieldValue::UInt32(ca) });
        }
        if let Some(sd) = self.settle_delay {
            fields.push(Field { def: field::SETTLE_DELAY, value: FieldValue::UInt32(sd) });
        }
        if let Some(exp) = self.expiration {
            fields.push(Field { def: field::EXPIRATION, value: FieldValue::UInt32(exp) });
        }
        if let Some(ref ch) = self.channel {
            fields.push(Field { def: field::CHANNEL, value: FieldValue::Hash256(*ch) });
        }
        if let Some(ref pk) = self.public_key_field {
            fields.push(Field { def: field::PUBLIC_KEY, value: FieldValue::Blob(pk.clone()) });
        }
        if let Some(ref amt) = self.amount {
            fields.push(Field { def: field::AMOUNT, value: FieldValue::Amount(amt.clone()) });
        }
        if let Some(ref lmt) = self.limit_amount {
            fields.push(Field { def: field::LIMIT_AMOUNT, value: FieldValue::Amount(lmt.clone()) });
        }
        if let Some(ref tp) = self.taker_pays {
            fields.push(Field { def: field::TAKER_PAYS, value: FieldValue::Amount(tp.clone()) });
        }
        if let Some(ref tg) = self.taker_gets {
            fields.push(Field { def: field::TAKER_GETS, value: FieldValue::Amount(tg.clone()) });
        }
        if let Some(dest) = self.destination {
            fields.push(Field { def: field::DESTINATION, value: FieldValue::AccountID(dest) });
        }
        if let Some(tag) = self.dest_tag {
            fields.push(Field { def: field::DESTINATION_TAG, value: FieldValue::UInt32(tag) });
        }
        if let Some(sig) = signature {
            fields.push(Field { def: field::TXN_SIGNATURE, value: FieldValue::Blob(sig) });
        }
...
mod tests {
    ...
    fn test_hash_changes_with_different_fields() {
        ...
        assert_ne!(a.hash, b.hash, "different amounts must produce different hashes");
    ...
    fn test_blob_starts_with_transaction_type_field() {
        ...
        assert_eq!(signed.blob[0], 0x12, "first byte must be TransactionType field ID");
