...
async fn test_rpc_submit_real_tx() {
    ...
    use xrpl::transaction::{Amount, builder::TxBuilder};
    ...
    assert_eq!(returned_hash, signed.hash_hex(),
...
async fn test_peer_listener_accepts_connection() {
    ...
    // Just connecting should work — the node accepts it
    let result = timeout(
...
async fn test_peer_receives_handshake_request_on_inbound() {
    // When we connect to a node as outbound, the node (inbound) should
    // wait for our HTTP upgrade request rather than sending one first.
    let (_, peer_addr, _) = start_node().await;
    let mut stream = TcpStream::connect(peer_addr).await.unwrap();

    // The node should NOT immediately send us anything — it's waiting
    // for our upgrade request (inbound direction)
    let mut buf = [0u8; 256];
    let result = timeout(Duration::from_millis(200), stream.read(&mut buf)).await;
    // Should time out (no data sent yet) — not an error, just silence
    assert!(result.is_err(), "node should not send anything before receiving our request");
}

#[tokio::test]
async fn test_outbound_node_sends_handshake() {
    // Start a plain TCP listener to capture what the node sends
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let target_addr = listener.local_addr().unwrap();

    // Start a node that dials us
    let peer_addr = free_addr().await;
    let rpc_addr  = free_addr().await;
    let config = NodeConfig {
        peer_addr,
    ...
    assert!(received.contains("Connect-As: Peer"),  "must identify as peer");
    assert!(received.contains("Public-Key:"),        "must include public key");
    assert!(received.contains("Session-Signature:"), "must include session signature");
}
