...
use super::{ApplyResult, TxContext, bridge_metadata_only_tx};
...
///
/// For now: fee + sequence are consumed by the outer apply pipeline,
/// and we return Success. If this tx type appears on mainnet before
/// implementation is complete, amendment gating will block it.
pub(crate) fn apply_batch(ctx: &TxContext) -> ApplyResult {
    bridge_metadata_only_tx(ctx, 71, "Batch", "tefFAILURE")
}
