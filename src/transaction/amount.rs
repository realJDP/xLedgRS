...
impl Currency {
    ...
    pub fn from_code(code: &str) -> Result<Self, AmountError> {
        ...
        if bytes.len() != 3 || !bytes.iter().all(|b| b.is_ascii_alphanumeric() || b"<>(){}[]|?!@#$%^&*".contains(b)) {
...
pub enum Issue {
    ...
    Iou { currency: Currency, issuer: [u8; 20] },
    /// MPT: 24-byte MPTID.
    Mpt([u8; 24]),
}
...
impl Issue {
    ...
    /// Is this XRP?
    pub fn is_xrp(&self) -> bool { matches!(self, Issue::Xrp) }
}
...
impl Amount {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<(Self, usize), AmountError> {
        ...
        if is_iou {
            ...
            Ok((Amount::Iou { value, currency: Currency { code: currency_code }, issuer }, 48))
        } else {
...
impl IouValue {
    ...
    pub fn from_f64(v: f64) -> Self {
        ...
        let mut abs = v.abs();
        let mut exp = 0i32;
        while abs < 1e15 { abs *= 10.0; exp -= 1; }
        while abs >= 1e16 { abs /= 10.0; exp += 1; }
        Self { mantissa: sign * abs as i64, exponent: exp }
    }
    ...
    pub fn to_bytes(&self) -> [u8; 8] {
        ...
        if positive { val |= 0x4000_0000_0000_0000; } // bit 62 = sign
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, AmountError> {
        ...
        Ok(Self { mantissa: signed_mantissa, exponent })
    }
}
...
impl IouValue {
    ...
    pub fn normalize(&mut self) {
        ...
        self.mantissa = if self.mantissa < 0 { -(abs as i64) } else { abs as i64 };
    }
    ...
    pub fn negate(&self) -> Self {
        Self { mantissa: -self.mantissa, exponent: self.exponent }
    }

    /// Convert to f64 (lossy, for comparisons and quality calculations).
    pub fn to_f64(&self) -> f64 {
        if self.mantissa == 0 { return 0.0; }
    ...
    pub fn mul(&self, other: &Self) -> Self {
        ...
        Self { mantissa: (sign * abs as i128) as i64, exponent: exp }
    }
    ...
    pub fn div(&self, other: &Self) -> Self {
        ...
        let m = if den == 0 {
            ...
            let sign = if (self.mantissa < 0) != (other.mantissa < 0) { -1i128 } else { 1i128 };
    ...
    pub fn mul_round(&self, other: &Self, round_up: bool) -> Self {
        ...
        let abs_product = (self.mantissa as i128).unsigned_abs()
            * (other.mantissa as i128).unsigned_abs();
        // muldiv_round: add bias for ceiling when rounding away from zero
        let bias: u128 = if result_negative != round_up { TEN14 - 1 } else { 0 };
        ...
        Self { mantissa: (sign * abs as i128) as i64, exponent: e }
    }
    ...
    pub fn div_round(&self, other: &Self, round_up: bool) -> Self {
        ...
        // muldiv_round: add bias for ceiling when rounding away from zero
        let bias: u128 = if result_negative != round_up { den_abs - 1 } else { 0 };
    ...
    pub fn min_abs(&self, other: &Self) -> Self {
        if self.abs().to_f64() <= other.abs().to_f64() { *self } else { *other }
    }
}
...
mod tests {
    ...
    fn test_xrp_encoding_roundtrip() {
        for drops in [0u64, 1, 1_000_000, 100_000_000_000, 99_999_999_999_999_999] {
            ...
            assert_eq!(decoded, Amount::Xrp(drops), "round-trip failed for {drops} drops");
        }
    }
    ...
    fn test_iou_value_roundtrip() {
        let val = IouValue { mantissa: 1_000_000_000_000_000, exponent: -15 };
