...
impl RawStateTable {
    ...
    pub fn insert(&mut self, sle: Arc<SLE>) {
        self.items.insert(*sle.key(), (RawAction::Insert, Some(sle)));
    }

    pub fn replace(&mut self, sle: Arc<SLE>) {
        self.items.insert(*sle.key(), (RawAction::Replace, Some(sle)));
    }
    ...
    pub fn xrp_destroyed(&self) -> u64 { self.xrp_destroyed }
...
impl ApplyStateTable {
    ...
    pub fn insert(&mut self, sle: Arc<SLE>) {
        self.items.insert(*sle.key(), ApplyEntry {
    ...
    pub fn update(&mut self, sle: Arc<SLE>) {
        ...
        if let Some(entry) = self.items.get_mut(&key) {
            ...
            self.items.insert(key, ApplyEntry {
    ...
    pub fn iter_changes(&self) -> impl Iterator<Item = (&Key, &ApplyEntry)> {
        self.items.iter().filter(|(_, e)| e.action != ApplyAction::Cache)
    }
    ...
    pub fn change_count(&self) -> usize {
        self.items.values().filter(|e| e.action != ApplyAction::Cache).count()
    }
}
...
mod tests {
    ...
    fn test_apply_state_table_insert_then_erase() {
        ...
        // Erase something we just inserted — should remove entirely
        table.erase(&key);
    ...
    fn test_apply_state_table_change_count() {
        ...
        table.items.insert(key, ApplyEntry {
            action: ApplyAction::Cache,
