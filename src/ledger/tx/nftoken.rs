...
pub(crate) fn apply_nftoken_burn(
    state: &mut LedgerState,
    tx:    &ParsedTx,
) -> ApplyResult {
    ...
    let offer_keys: Vec<crate::ledger::Key> = state.iter_nft_offers()
...
pub(crate) fn apply_nftoken_cancel_offer(
    state: &mut LedgerState,
    tx:    &ParsedTx,
) -> ApplyResult {
    ...
    if !removed_any {
        tracing::trace!("NFTokenCancelOffer: no offers found via parsed fields, metadata will handle");
    }
...
pub(crate) fn apply_nftoken_create_offer(
    ...
    // NFTs live in NFTokenPage SLEs which we don't track individually.
    ...
    let required = fees.reserve_base
        + ((new_sender.owner_count as u64 + 1) * fees.reserve_inc);
...
pub(crate) fn apply_nftoken_accept_offer(
    ...
    if let Some(sell_hash) = tx.nft_sell_offer {
        ...
        if !sell.is_sell() { return ApplyResult::ClaimedCost("temMALFORMED"); }

        // Check expiration
        if let Some(exp) = sell.expiration {
            if (close_time as u32) >= exp { return ApplyResult::ClaimedCost("tecEXPIRED"); }
        }

        // Check destination restriction
        if let Some(dest) = sell.destination {
            if tx.account != dest { return ApplyResult::ClaimedCost("tecNO_PERMISSION"); }
        }
        ...
        match &sell.amount {
            ...
            Amount::Iou { value, currency, issuer } => {
                ...
                let mut buyer_tl = super::ripple_calc::load_or_create_trustline(state, &buyer_key, &tx.account, issuer, currency);
                buyer_tl.transfer(&tx.account, value);
                state.insert_trustline(buyer_tl);

                let seller_key = crate::ledger::trustline::shamap_key(&sell.account, issuer, currency);
                let mut seller_tl = super::ripple_calc::load_or_create_trustline(state, &seller_key, &sell.account, issuer, currency);
            ...
            _ => {
                // Zero amount or MPT — just decrement seller's owner_count
                if let Some(seller) = state.get_account(&sell.account) {
                    let mut seller = seller.clone();
                    seller.owner_count = seller.owner_count.saturating_sub(1);
                    state.insert_account(seller);
    ...
    if let Some(buy_hash) = tx.nft_buy_offer {
        ...
        if buy.is_sell() { return ApplyResult::ClaimedCost("temMALFORMED"); }
