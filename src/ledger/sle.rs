...
use crate::ledger::meta::{
    read_field_header, skip_field_raw, write_field_header,
    is_vl_type, encode_vl_length, decode_vl_length,
};
...
impl SLE {
            ...
            while pos < data.len() {
                ...
                if tc == 1 && fc == 1 {
                    ...
                    if new_pos + 2 <= data.len() {
                        ...
                        let entry_type = LedgerEntryType::from_u16(code)?;
                        return Some(Self { key, entry_type, data });
                    }
                }
                pos = new_pos;
    ...
    pub fn entry_type(&self) -> LedgerEntryType { self.entry_type }
    ...
    pub fn remove_field(&mut self, type_code: u16, field_code: u16) {
        ...
        while pos < self.data.len() {
            ...
            if header_end > self.data.len() { break; }
