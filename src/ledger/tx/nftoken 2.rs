...
pub(crate) fn apply_nftoken_create_offer(
    ...
    let offer_key = crate::ledger::nftoken::offer_shamap_key(&tx.account, tx.sequence);
...
pub(crate) fn apply_nftoken_accept_offer(
    ...
    if let Some(sell_hash) = tx.nft_sell_offer {
        ...
        let price_drops = match &sell.amount {
            Amount::Xrp(d) => *d,
            _ => 0,
        };
        if price_drops > 0 {
            // Debit buyer (tx.account)
            ...
            if let Some(seller) = state.get_account(&sell.account) {
                ...
                seller.balance = seller.balance.saturating_add(price_drops);
                seller.owner_count = seller.owner_count.saturating_sub(1); // remove offer
                state.insert_account(seller);
            }
        } else {
            // No payment, just decrement seller's owner_count for the offer
            if let Some(seller) = state.get_account(&sell.account) {
                let mut seller = seller.clone();
