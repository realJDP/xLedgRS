...
impl PayChannel {
    pub fn to_sle_binary(&self) -> Vec<u8> {
        ...
        sle.set_field_raw_pub(6, 1, &crate::transaction::Amount::Xrp(self.amount).to_bytes());
        // Balance (Amount, 6, 2) — XRP
        sle.set_field_raw_pub(6, 2, &crate::transaction::Amount::Xrp(self.balance).to_bytes());
...
mod tests {
    ...
    fn test_paychan_key_deterministic() {
        assert_eq!(shamap_key(&acct(1), &acct(2), 5), shamap_key(&acct(1), &acct(2), 5));
    }

    #[test]
    fn test_different_channels_different_keys() {
        assert_ne!(shamap_key(&acct(1), &acct(2), 1), shamap_key(&acct(1), &acct(2), 2));
    }
    ...
    fn test_verify_claim_roundtrip() {
        ...
        let chan = PayChannel {
            account: acct(1), destination: acct(2),
            amount: 10_000_000, balance: 0, settle_delay: 3600,
            public_key: kp.public_key_bytes(), sequence: 1,
            cancel_after: 0, expiration: 0,
