...
pub(crate) fn apply_amm_create(
    ...
    // transfer. The deposit path handles that follow-up work.
...
pub(crate) fn apply_amm_deposit(state: &mut LedgerState, tx: &ParsedTx) -> ApplyResult {
    ...
    // 2. Get deposit amounts (`amount_drops` for asset1/XRP and flags for the
    // asset2 amount).
    // Simplified: for XRP+IOU, amount_drops is the XRP deposit.
    // The IOU side uses a fixed 1:1 ratio in this implementation.
...
/// Apply the voter's proposed fee directly using the single-voter model.
...
pub(crate) fn apply_amm_vote(state: &mut LedgerState, tx: &ParsedTx) -> ApplyResult {
    ...
    // rippled passes `TradingFee` as a field. This implementation reads the
    // lower 16 bits of `Flags`.
...
pub(crate) fn apply_amm_bid(state: &mut LedgerState, tx: &ParsedTx) -> ApplyResult {
    // 1. Find AMM
    let asset1 = match &tx.asset {
        Some(a) => a.clone(),
        None => return ApplyResult::ClaimedCost("temMALFORMED"),
    ...
    // because the SLE field layout is complex.
