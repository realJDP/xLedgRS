...
pub(crate) fn apply_loan_broker_set(
    ...
    let bkey = loan_broker_key(&tx.account, tx.sequence);
    ...
    let broker_sle = build_loan_broker_sle(
        &tx.account, &pseudo_id, &vault_id, tx.sequence, owner_node, vault_node,
    );
...
/// Types 76, 77, 78, 82: Cover/Manage operations (stubs).
pub(crate) fn apply_loan_modify() -> ApplyResult {
    ApplyResult::Success
}
