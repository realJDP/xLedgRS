...
pub fn deposit_preauth(owner: &[u8; 20], authorized: &[u8; 20]) -> Keylet {
    ...
    Keylet::new(Key(sha512_first_half(&buf)), LedgerEntryType::DepositPreauth)
}
...
pub fn fees() -> Keylet {
    Keylet::new(Key(sha512_first_half(&SPACE_FEES)), LedgerEntryType::FeeSettings)
}

/// Amendments singleton keylet: SHA-512-half(0x0066)
pub fn amendments() -> Keylet {
    Keylet::new(Key(sha512_first_half(&SPACE_AMENDMENTS)), LedgerEntryType::Amendments)
}

/// NegativeUNL singleton keylet: SHA-512-half(0x004E)
pub fn negative_unl() -> Keylet {
    Keylet::new(Key(sha512_first_half(&SPACE_NEGATIVE_UNL)), LedgerEntryType::NegativeUNL)
}

/// Short skip list (last 256 ledger hashes): SHA-512-half(0x0073)
/// Updated every ledger. Circular buffer of 256 parent hashes.
pub fn skip() -> Keylet {
    Keylet::new(Key(sha512_first_half(&SPACE_SKIP)), LedgerEntryType::LedgerHashes)
}
