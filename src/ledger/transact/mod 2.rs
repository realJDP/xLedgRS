...
use crate::ledger::views::{ReadView, ApplyView, ApplyFlags};
use crate::ledger::open_view::OpenView;
use crate::ledger::apply_view_impl::{ApplyViewImpl, ApplyResult, AffectedNodeInfo};
...
pub use account_set::{AccountSetHandler, account_set_flag_to_ledger};
pub use set_regular_key::SetRegularKeyHandler;
pub use ticket_create::TicketCreateHandler;
pub use escrow::{EscrowCreateHandler, EscrowFinishHandler, EscrowCancelHandler};
...
pub use check::{CheckCreateHandler, CheckCashHandler, CheckCancelHandler};
pub use paychan::{PayChanCreateHandler, PayChanFundHandler, PayChanClaimHandler};
pub use trust_set::{TrustSetHandler, extract_iou_issuer};
pub use nftoken::{NFTokenMintHandler, NFTokenBurnHandler, NFTokenCreateOfferHandler};
pub use nftoken_accept::{NFTokenCancelOfferHandler, NFTokenAcceptOfferHandler};
pub use nftoken_modify::NFTokenModifyHandler;
pub use clawback::ClawbackHandler;
pub use amm::{AMMCreateHandler, AMMDepositHandler, AMMWithdrawHandler, AMMVoteHandler, AMMBidHandler, AMMDeleteHandler, AMMClawbackHandler};
pub use xchain::XChainHandler;
pub use did::{DIDSetHandler, DIDDeleteHandler};
pub use signer_list::SignerListSetHandler;
pub use oracle::{OracleSetHandler, OracleDeleteHandler};
pub use ledger_state_fix::LedgerStateFixHandler;
pub use mptoken::{MPTokenIssuanceCreateHandler, MPTokenIssuanceDestroyHandler, MPTokenIssuanceSetHandler, MPTokenAuthorizeHandler};
pub use credential::{CredentialCreateHandler, CredentialAcceptHandler, CredentialDeleteHandler};
pub use permissioned_domain::{PermissionedDomainSetHandler, PermissionedDomainDeleteHandler};
pub use delegate::DelegateSetHandler;
pub use vault::{VaultCreateHandler, VaultSetHandler, VaultDeleteHandler, VaultDepositHandler, VaultWithdrawHandler, VaultClawbackHandler};
pub use batch::BatchHandler;
pub use loan::{LoanBrokerSetHandler, LoanBrokerDeleteHandler, LoanSetHandler, LoanDeleteHandler, LoanManageHandler, LoanPayHandler, LoanCoverHandler};
pub use pseudo::{SetFeeHandler, EnableAmendmentHandler, UNLModifyHandler};
...
impl TER {
    ...
    pub fn is_success(&self) -> bool { matches!(self, TER::Success) }
...
pub fn apply_transaction(
    ...
    if let Err(ter) = handler.preflight(tx) {
        return TxApplyResult { ter, metadata: None };
    }
...
// ── Stub handler for unimplemented tx types ─────────────────────────────────

/// Stub handler: claims fee and bumps sequence, no other effects.
/// Used for tx types we haven't implemented yet.
pub struct StubHandler;

impl TxHandler for StubHandler {
...
pub fn check_reserve(
    ...
    let required = fees.reserve_base
        + ((owner_count as u64 + additional as u64) * fees.reserve_inc);
    if balance < required {
...
mod tests {
    ...
    fn setup_ledger_with_account(account_id: &[u8; 20], balance: u64) -> ClosedLedger {
        ...
        ledger.raw_insert(Arc::new(SLE::new(kl.key, LedgerEntryType::AccountRoot, data)));
        ledger
    }
    ...
    fn test_payment_xrp_existing_dest() {
        ...
        base.raw_insert(Arc::new(SLE::new(bob_kl.key, LedgerEntryType::AccountRoot, bob_data)));
    ...
    fn test_insufficient_balance() {
        ...
        let bob_kl = keylet::account(&bob);
        base_mut.raw_insert(Arc::new(SLE::new(bob_kl.key, LedgerEntryType::AccountRoot, bob_data)));
    ...
    fn test_account_set_flags() {
        ...
        let tx_hash = [0x05; 32];
        let result = apply_transaction(&mut open, &tx, &tx_hash, &AccountSetHandler, ApplyFlags::NONE);
        assert!(result.ter.is_success());
    ...
    fn test_escrow_create() {
        ...
        let tx_hash = [0x07; 32];
        let result = apply_transaction(&mut open, &tx, &tx_hash, &EscrowCreateHandler, ApplyFlags::NONE);
        assert!(result.ter.is_success());
    ...
    fn test_account_delete() {
        ...
        base.raw_insert(Arc::new(SLE::new(
            keylet::account(&bob).key, LedgerEntryType::AccountRoot, bob_data,
        )));
        ...
        let tx_hash = [0x08; 32];
        let result = apply_transaction(&mut open, &tx, &tx_hash, &AccountDeleteHandler, ApplyFlags::NONE);
        assert!(result.ter.is_success());
        ...
        assert_eq!(bob_sle.balance_xrp(), Some(10_000_000 + 50_000_000 - 2_000_000));
    }

    #[test]
    fn test_stub_handler_claims_fee() {
        let alice = [0xAA; 20];
        let base = setup_ledger_with_account(&alice, 100_000_000);
        let mut open = OpenView::new(Arc::new(base));
        ...
        let tx_hash = [0x04; 32];
        let result = apply_transaction(&mut open, &tx, &tx_hash, &StubHandler, ApplyFlags::NONE);
        assert!(result.ter.is_success());

        // Fee should be deducted
        let alice_sle = open.read(&keylet::account(&alice)).unwrap();
        assert_eq!(alice_sle.balance_xrp(), Some(100_000_000 - 10));
        // Sequence should be bumped
