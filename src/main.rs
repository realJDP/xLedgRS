...
#[derive(Parser, Debug)]
#[command(name = "xledgrs", about = "XRP Ledger node implemented in Rust", version)]
struct Args {
    ...
    #[arg(long)] test_skip_creates: bool,
    #[arg(long)] test_skip_modifies: bool,
    #[arg(long)] test_skip_deletes: bool,
...
async fn main() -> anyhow::Result<()> {
    ...
    let file_cfg = config_file_path.as_ref().and_then(|path| {
    ...
    let mut bootstrap: Vec<std::net::SocketAddr> = args.bootstrap
        .split(',')
        .filter(|s| !s.is_empty())
        .filter_map(|s| {
            let s = s.trim();
            match s.parse() {
                Ok(addr) => Some(addr),
                Err(e) => { tracing::warn!("ignoring bad bootstrap address '{}': {}", s, e); None }
            }
        })
        .collect();
    ...
    // ips_fixed peers are treated as full-history peers — they get priority-dialed
    // and used for catch-up via peer protocol (no RPC).
    ...
    if !fixed_addrs.is_empty() {
        tracing::info!("ips_fixed: {} peers registered as full-history", fixed_addrs.len());
    }
    // Add to bootstrap too so they're in the known peers set
    ...
    if bootstrap.is_empty() {
        ...
        tracing::info!("using {} configured bootstrap peers; skipping built-in public seed fallback", bootstrap.len());
    }
    ...
    if bootstrap.is_empty() {
            ...
            if let Some(peers) = (|| -> Option<Vec<(String, bool)>> {
                ...
                for peer in active {
                    if let Some(ip) = peer.get("ip").and_then(|v| v.as_str()) {
                        if !ip.contains(':') && !ip.starts_with("10.") && !ip.starts_with("192.168.") {
                            // Deep history = 100K+ ledger span (~4 days).
                            // These peers can serve any ledger we need for catch-up.
                            let is_deep = peer.get("complete_ledgers")
                                .and_then(|v| v.as_str())
                                .map_or(false, |cl| {
                ...
                tracing::info!("crawl {}: {} peers ({} deep-history)", url, peers.len(), deep_count);
                for (ip, is_deep) in peers {
                    all_ips.insert(ip.clone());
                    if is_deep { full_history_ips.insert(ip); }
                }
            }
        }
        ...
        if !crawl_ips.is_empty() {
            ...
            let results: Arc<Mutex<Vec<(Duration, SocketAddr, bool)>>> = Arc::new(Mutex::new(Vec::new()));
                    ...
                    handles.push(std::thread::spawn(move || {
                        ...
                        if TcpStream::connect_timeout(&addr, Duration::from_secs(1)).is_ok() {
                            results
                                ...
                                .push((start.elapsed(), addr, is_deep));
                        }
                    }));
                }
            }
            ...
            if !full_history_peers.is_empty() {
                tracing::info!(
                    ...
                    probed.iter().filter(|(_, _, f)| *f).next().map_or(0, |(d, _, _)| d.as_millis()),
                    probed.iter().filter(|(_, _, f)| *f).last().map_or(0, |(d, _, _)| d.as_millis()),
                );
            } else {
    ...
    let data_dir = args.data_dir
        ...
        .or_else(|| file_cfg.as_ref().and_then(|cfg| cfg.runtime.data_dir.clone()));
    ...
    let ledger_history = file_cfg.as_ref()
        .and_then(|cfg| cfg.runtime.ledger_history)
        .unwrap_or(HistoryRetention::Count(256));
    let fetch_depth = file_cfg.as_ref()
        .and_then(|cfg| cfg.runtime.fetch_depth)
        .unwrap_or(HistoryRetention::Full);
    let online_delete = file_cfg.as_ref()
        .and_then(|cfg| cfg.runtime.online_delete);
    let enable_consensus_close_loop = if args.enable_consensus_close_loop {
        true
    } else {
        file_cfg.as_ref()
    ...
    let config = NodeConfig {
        ...
        rpc_sync: args.rpc_sync.or_else(|| {
            file_cfg.as_ref().and_then(|cfg| cfg.runtime.rpc_sync.clone())
        }),
        full_history_peers: {
        ...
        post_sync_checkpoint_script: file_cfg.as_ref()
            .and_then(|cfg| cfg.runtime.post_sync_checkpoint_script.clone()),
        validation_seed: file_cfg.as_ref().and_then(|cfg| cfg.validation_seed.clone()),
    };
    ...
    let test_skip = (args.test_skip_creates, args.test_skip_modifies, args.test_skip_deletes);
    ...
    if args.verify_hash {
        let endpoint = config.rpc_sync.as_ref().expect("--rpc-sync required for --verify-hash");
        let (host, port) = {
            let parts: Vec<&str> = endpoint.splitn(2, ':').collect();
            (parts[0].to_string(), parts.get(1).and_then(|p| p.parse().ok()).unwrap_or(5005u16))
        };
    ...
    if args.diagnose {
        let data_dir = config.data_dir.as_ref().expect("--data-dir required for --diagnose");
    ...
    if args.compact {
        let data_dir = config.data_dir.as_ref().expect("--data-dir required for --compact");
        ...
        // We need the root hash — load from storage metadata
    ...
    if test_one {
        ...
        let (host, port) = {
            ...
            (parts[0].to_string(), parts.get(1).and_then(|p| p.parse().ok()).unwrap_or(51234))
        };
        ...
        if let Some(ref guard) = sync_guard {
            if let Some(ref syncer) = **guard {
                ...
                if let Some(ref store) = storage {
                    let _ = store.save_meta_kv("leaf_count",
                        &(syncer.leaf_count() as u64).to_le_bytes());
...
mod tests {
    ...
    fn normalize_peer_endpoint_accepts_rippled_style_host_port() {
        assert_eq!(normalize_peer_endpoint("r.ripple.com 51235"), "r.ripple.com:51235");
        assert_eq!(normalize_peer_endpoint("169.55.164.20 51235"), "169.55.164.20:51235");
        assert_eq!(normalize_peer_endpoint("s1.ripple.com:51235"), "s1.ripple.com:51235");
    }
}
