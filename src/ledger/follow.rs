...
fn load_verified_sync_anchor(
    storage: &Storage,
) -> Option<(u32, Option<[u8; 32]>, Option<[u8; 32]>, crate::ledger::LedgerHeader)> {
...
fn validated_reacquired_sync_header(
    ...
    if validate_sync_anchor_header(sync_ledger_seq, Some(expected_hash), sync_account_hash, &hdr) {
        Some(hdr)
    } else {
        None
    }
...
fn remove_typed(
    ...
    match entry_type {
        ...
        0x0072 => { state.remove_trustline(key); }
        0x0064 => { state.remove_directory(key); }
        0x006f => { state.remove_offer(key); }
        _ => {}
    }
}
...
fn build_directory_sle_from_fields(
    ...
    fn as_vector256(data: Option<&[u8]>) -> Vec<[u8; 32]> {
        let Some(bytes) = data else { return Vec::new(); };
        bytes
            .chunks_exact(32)
...
fn build_offer_sle_from_fields(
    ...
    let taker_pays = find_field(fields, 6, 4)
        .and_then(|d| crate::transaction::amount::Amount::from_bytes(d).ok().map(|(a, _)| a))
...
fn build_created_sle_with_state(
    state: &crate::ledger::LedgerState,
    key: &crate::ledger::Key,
    entry_type: u16,
    fields: &[crate::ledger::meta::ParsedField],
...
fn extract_offer_book_sides(
    ...
    let pays = find_field(fields, 6, 4)
        .and_then(|d| crate::transaction::amount::Amount::from_bytes(d).ok().map(|(a, _)| a))?;
    let gets = find_field(fields, 6, 5)
        .and_then(|d| crate::transaction::amount::Amount::from_bytes(d).ok().map(|(a, _)| a))?;
    ...
    let (pays_currency, pays_issuer) = match pays {
        ...
        crate::transaction::amount::Amount::Xrp(_) | crate::transaction::amount::Amount::Mpt(_) => ([0u8; 20], [0u8; 20]),
    };
...
fn related_offer_book_for_directory_node(
    node: &crate::ledger::meta::AffectedNode,
    tx_book_dirs: &std::collections::HashMap<
        [u8; 32],
    ...
    if tx_book_dirs.len() == 1 {
        return tx_book_dirs.values().next().copied();
    }
    None
}
...
fn seed_entries_from_metadata(
    ...
    for (_tx_index, nodes) in &all_meta {
        for node in nodes {
            if seeded_keys.contains(&node.ledger_index) { continue; }
            let sle = match node.action {
                ...
                crate::ledger::meta::Action::Deleted =>
                    crate::ledger::meta::build_sle(
                        node.entry_type, &node.fields,
                        node.prev_txn_id, node.prev_txn_lgrseq),
...
fn apply_metadata_patches(
    ...
    let mut created_seen: std::collections::HashMap<[u8; 32], Vec<crate::ledger::meta::ParsedField>> =
    ...
    for (_tx_index, tx_hash, nodes) in &all_meta {
        let mut tx_ops: Vec<(crate::ledger::Key, u16, Option<Vec<u8>>)> = Vec::new();
        let mut tx_book_dirs: std::collections::HashMap<[u8; 32], ([u8; 20], [u8; 20], [u8; 20], [u8; 20])> =
            std::collections::HashMap::new();
        for n in nodes {
        ...
        for node in nodes {
            ...
            if ledger_seq == 103483090 && node.entry_type == 0x0064 && watch_directory_key(&node.ledger_index) {
                let idx = field_vector256(&node.fields);
                let idx_short: Vec<String> = idx.iter().map(|h| hex::encode_upper(&h[..8])).collect();
                info!(
            ...
            match node.action {
                crate::ledger::meta::Action::Created => {
                    ...
                    let sle = if let Some(override_sle) = created_overrides.get(&key).cloned() {
                        ...
                        build_created_sle_with_state(&state, &key, node.entry_type, &norm_fields, related_offer_book, new_ptid, new_ptseq)
                    };
                    if byte_diff_mode {
                        info!(
                        ...
                        match crate::ledger::DirectoryNode::decode(&sle, key.0) {
                            Ok(dir) => {
                                        ...
                                        || dir.taker_gets_issuer.is_none())
                                {
                                    ...
                                    tx_ops.push((key, node.entry_type, Some(sle)));
                                    continue;
                                }
                            }
                            Err(e) => {
                ...
                crate::ledger::meta::Action::Modified => {
                    if let Some(override_sle) = modified_dir_overrides.get(&key).cloned() {
                        ...
                        tx_ops.push((key, node.entry_type, Some(override_sle)));
                        continue;
                    }
                    let norm_fields = normalize_directory_fields(node.entry_type, &node.fields);
                    ...
                    if node.entry_type == 0x0064 {
                        ...
                        if !has_indexes {
                            if let Some(override_sle) = modified_dir_overrides.get(&key).cloned() {
                                if ledger_seq == 103483090 && watch_directory_key(&node.ledger_index) {
                                    info!(
                                ...
                                tx_ops.push((key, node.entry_type, Some(override_sle)));
                                continue;
                            } else if ledger_seq == 103483090 && watch_directory_key(&node.ledger_index) {
                                warn!(
                    ...
                    let final_keys: std::collections::HashSet<(u16, u16)> =
                        node.fields.iter().map(|f| (f.type_code, f.field_code)).collect();
                    let deleted: Vec<(u16, u16)> = node.previous_fields.iter()
                    ...
                    let final_sle = if let Some(e) = existing {
                        if created_earlier_in_ledger {
                                ...
                                if let Some(creation_fields) = created_seen.get(&node.ledger_index) {
                                    for cf in creation_fields {
                                        if cf.type_code == 17
                                            ...
                                            && !merged.iter().any(|f| f.type_code == cf.type_code && f.field_code == cf.field_code)
                                        {
                            ...
                            let rebuilt = build_created_sle_with_state(&state, &key, node.entry_type, &merged_fields, related_offer_book, new_ptid, new_ptseq);
                            if byte_diff_mode {
                                info!(
                            ...
                            let patched = crate::ledger::meta::patch_sle(&e, &norm_fields, new_ptid, new_ptseq, &deleted);
                            if byte_diff_mode {
                                info!(
                        ...
                        return MetadataPatchStats {
                            applied,
                            missing_modified,
                            created_override_miss_keys: Vec::new(),
                            incomplete_book_dir_keys: Vec::new(),
                            aborted: true,
                        };
                    };
                    // Block incomplete book DirectoryNodes from entering state.
                    ...
                    tx_ops.push((key, node.entry_type, Some(final_sle)));
                }
                crate::ledger::meta::Action::Deleted => {
                    if byte_diff_mode {
                        info!(
                    ...
                    tx_ops.push((key, node.entry_type, None));
                }
            }
        }
        for (key, entry_type, sle_opt) in tx_ops {
            match sle_opt {
                Some(sle) => {
...
fn prune_engine_only_transients(
    ...
    for (_tx_hash, meta_blob) in meta_with_hashes {
        let nodes = match std::panic::catch_unwind(|| crate::ledger::meta::parse_metadata(meta_blob)) {
            Ok(nodes) => nodes,
            Err(_) => continue,
        };
        for node in nodes {
...
async fn fetch_modified_directory_overrides(
    ...
    if ledger_seq == 103483090 {
        ...
        if !watch.is_empty() {
            info!("follower: modified DirectoryNode watch candidates={:?}", watch);
        }
    }
    let mut out = std::collections::HashMap::new();
    let mut join_set = tokio::task::JoinSet::new();
...
pub async fn run_follower(
    ...
    // Determine starting sequence: synced ledger + 1 for consecutive replay.
    // The synced ledger's state is already in NuDB — we need to apply the NEXT
    // ledger's metadata patches to evolve the state forward.
    let sync_ledger_seq = storage.get_sync_ledger().unwrap_or(0) as u32;
    let start_seq = if sync_ledger_seq > 0 {
        info!("ledger follower: NuDB state is from synced ledger {} — starting at {}", sync_ledger_seq, sync_ledger_seq + 1);
        sync_ledger_seq + 1
    } else {
        ...
        loop {
            ...
            if seq > 1 && hash != [0u8; 32] {
                info!("ledger follower: no sync_ledger stored, bootstrapping from peer tip {}", seq);
                break seq;
            }
    ...
    follower_state.current_seq.store(start_seq, Ordering::Relaxed);
    ...
    let mut prev_header: Option<crate::ledger::LedgerHeader> = if sync_ledger_seq > 0 {
        ...
        if let Some(hash) = sync_ledger_hash {
            ...
            info!(
                ...
                hex::encode_upper(&hash[..8]), sync_ledger_seq,
            );
            {
                let mut guard = inbound_ledgers.lock().unwrap_or_else(|e| e.into_inner());
                guard.create(hash, sync_ledger_seq);
            }
        ...
        if hdr.hash != [0u8; 32] { Some(hdr.clone()) } else { None }
    };
    ...
    loop {
        ...
        let (validated, ledger_hash) = if let Some(ref prev) = prev_header {
            ...
            let lookup_deadline = tokio::time::Instant::now()
            ...
            loop {
                {
                    ...
                    if state.ctx.ledger_seq > next_seq + 100 && last_heartbeat.elapsed().as_secs() >= 10 {
                        info!(
                ...
                if last_heartbeat.elapsed().as_secs() >= 2 {
                    ...
                    debug!(
                        ...
                        next_seq, pif, lookup_deadline.elapsed().as_secs_f64(),
                    );
            ...
            if let Some(hash) = found_hash {
                ...
                let seq_deadline = tokio::time::Instant::now()
                ...
                loop {
                    // Check if inbound_ledgers got a header for this seq
                    {
                        ...
                        if let Some(hash) = guard.hash_for_seq(next_seq) {
                            info!("follower: got hash for seq {} from peer: {}", next_seq, hex::encode_upper(&hash[..8]));
                ...
                if let Some(hash) = found_hash {
                    ...
                    warn!("follower: no peer responded with ledger {} header — trying RPC fallback", next_seq);
                    // RPC fallback: fetch ledger hash from s1/s2.ripple.com
                    if let Some((_resp, _acc_hash)) = fetch_ledger_binary(&rpc_host, rpc_port, next_seq).await {
                        // Got a response — extract the hash from the response
                        if let Some(hash_hex) = _resp["result"]["ledger"]["ledger_hash"].as_str()
                            .or_else(|| _resp["result"]["ledger_hash"].as_str())
                        {
                            if let Ok(hash_bytes) = hex::decode(hash_hex) {
                                if hash_bytes.len() == 32 {
                                    ...
                                    info!("follower: got hash for seq {} via RPC fallback: {}", next_seq, hex::encode_upper(&hash[..8]));
                                    (next_seq, hash)
                                } else {
            ...
            loop {
                let (seq, hash) = {
                    ...
                    let hash = hex::decode(&state.ctx.ledger_hash).ok()
                        .and_then(|b| if b.len() == 32 {
                            let mut h = [0u8;32]; h.copy_from_slice(&b); Some(h)
                        } else { None })
                        .unwrap_or([0u8; 32]);
                    (seq, hash)
                };
        ...
        let is_consecutive = prev_header.as_ref()
        ...
        info!(
            ...
            validated, hex::encode_upper(&ledger_hash[..8]),
            if is_consecutive { "consecutive" } else { "bootstrap/fallback" },
        );
        let mut watch_rx = {
            let mut guard = inbound_ledgers.lock().unwrap_or_else(|e| e.into_inner());
            let rx = guard.create(ledger_hash, validated);
        ...
        // Send requests (may be redundant with validation handler's pre-fetch)
        {
            ...
            let tx_req = crate::network::relay::encode_get_ledger_txs_for_hash(&ledger_hash, cookie_tx);
            let mut state = shared_state.write().await;
        ...
        // (pre-fetch beat us), we see it immediately. No lost signals.
        ...
        loop {
            ...
            match tokio::time::timeout_at(deadline, watch_rx.changed()).await {
                Ok(Ok(())) => {
                    if *watch_rx.borrow() { break; } // Complete
                }
        ...
        let result = {
            ...
            if wait_ms > 50 {
                warn!("follower inbound_ledgers.take() lock_wait={}ms seq={}", wait_ms, target_seq);
            }
            r
        };
        ...
        let result = if result.is_none() {
            ...
            if let Some((rpc_resp, rpc_acc_hash)) = fetch_ledger_binary(&rpc_host, rpc_port, target_seq).await {
                ...
                if !rpc_blobs.is_empty() {
                    ...
                    fn decode_h256(hex_str: &str) -> [u8; 32] {
                        hex::decode(hex_str).ok()
                            .and_then(|b| if b.len() == 32 { let mut h = [0u8;32]; h.copy_from_slice(&b); Some(h) } else { None })
                            .unwrap_or([0u8; 32])
                    }
                    // Try local rippled first, then public servers for header
                    let hdr_body_opt = if let Ok(b) = crate::rpc_sync::http_post(&rpc_host, rpc_port, &hdr_req).await {
                        let v: Value = serde_json::from_str(&b).unwrap_or_default();
                        if v["result"]["status"].as_str() == Some("success") { Some(b) } else { None }
                    } else { None };
                    ...
                    let rpc_header: Option<crate::ledger::LedgerHeader> = if let Some(hdr_body) = hdr_body_opt {
                        let hdr: Value = serde_json::from_str(&hdr_body).unwrap_or_default();
                        let lh = &hdr["result"]["ledger"];
                        Some(crate::ledger::LedgerHeader {
                            sequence: lh["ledger_index"].as_u64().unwrap_or(target_seq as u64) as u32,
                            hash: ledger_hash,
                            parent_hash: decode_h256(lh["parent_hash"].as_str().unwrap_or("")),
                            close_time: lh["close_time"].as_u64().unwrap_or(0),
                            total_coins: lh["total_coins"].as_str().and_then(|s| s.parse::<u64>().ok()).unwrap_or(0),
                            account_hash: decode_h256(&rpc_acc_hash),
                            transaction_hash: decode_h256(lh["transaction_hash"].as_str().unwrap_or("")),
                            parent_close_time: lh["parent_close_time"].as_u64().unwrap_or(0) as u32,
                            close_time_resolution: lh["close_time_resolution"].as_u64().unwrap_or(10) as u8,
        ...
        if let Some((seq_header, peer_blobs)) = result {
            ...
            if prev_header.is_none() {
                info!("ledger follower: bootstrapping prev_header from ledger {}", seq_header.sequence);
            ...
            // The acquired header must chain to our prev_header. If not,
            ...
            if seq_header.parent_hash != parent.hash {
                ...
                if false {
                    ...
                    {
                        let mut state = shared_state.write().await;
                        if state.validated_hashes.get(&seq_header.sequence).copied() == Some(seq_header.hash) {
            ...
            let meta_with_hashes: Vec<([u8; 32], Vec<u8>)> = blobs.iter().map(|(tx, meta)| {
            ...
            let mut modified_overrides =
                fetch_modified_directory_overrides(&rpc_host, rpc_port, seq, &meta_with_hashes).await;
            ...
            // the divergence test point. Clone the inputs we'll need for the
            // artifact bundle before `blobs` gets consumed by replay_ledger.
            let byte_diff_capture = first_post_sync_seq == Some(seq);
            let tx_blobs_for_capture: Option<Vec<(Vec<u8>, Vec<u8>)>> =
                if byte_diff_capture { Some(blobs.clone()) } else { None };
            ...
            let blocking_result = tokio::task::spawn_blocking(move || {
                ...
                // the live NuDB-backed SHAMap. This lets post-mismatch
                ...
                let forensic_keys: Vec<crate::ledger::Key> =
                    if let Some(ref mh) = meta_with_hashes_for_capture {
                        ...
                        info!(
                            "follower forensic scope seq={}: engine_touched={} metadata_union={} metadata_only={}",
            ...
            let (mut replay_result, patched, pruned_engine_only, missing_modified, created_miss_keys, incomplete_book_dirs, prestate_captured, pristine_base_snapshot, post_state) = match blocking_result {
                Ok((rr, p, pruned, _lock_wait, prestate, pristine_base, post)) => (rr, p.applied, pruned, p.missing_modified, p.created_override_miss_keys, p.incomplete_book_dir_keys, prestate, pristine_base, post),
                Err(e) => {
            ...
            if matched { tx_engine_successes += 1; }
            ...
            if sync_complete && !first_hash_check_done {
                ...
                if matched {
                    ...
                    info!("  local={}", hex::encode_upper(&replay_result.header.account_hash[..16]));
                    info!("  network={}", hex::encode_upper(&seq_account_hash[..16]));
                    info!("========================================");
                } else {
                    error!("========================================");
                    error!("  FIRST LEDGER HASH: MISMATCH seq={}", seq);
                    error!("  local={}", hex::encode_upper(&replay_result.header.account_hash[..16]));
            ...
            if sync_complete && !matched {
                ...
                // we need for offline replay_fixture iteration — before
                // rippled's retention window rolls past this ledger.
                if let (Some(blobs_saved), Some(prestate)) = (
                    tx_blobs_for_capture.clone(),
                    prestate_captured.clone(),
                ) {
                    let touched_raw: Vec<[u8; 32]> =
                        post_state.iter().map(|(k, _)| *k).collect();
                    ...
                    let inputs = crate::ledger::forensic::CaptureInputs {
                        ...
                        per_tx_attribution: std::mem::take(
                            &mut replay_result.per_tx_attribution,
                        ),
                    ...
                    match crate::ledger::forensic::capture_forensic_bundle(inputs).await {
                        Ok((path, rippled_ref)) => {
                            ...
                            if !rippled_ref.is_empty() {
                                ...
                                let mut authoritative_upserts: Vec<(crate::ledger::Key, Vec<u8>)> = Vec::new();
                                ...
                                for (key, local_bytes) in &post_state {
                                    if let Some(ref_bytes) = rippled_ref.get(key) {
                                        authoritative_upserts.push((crate::ledger::Key(*key), ref_bytes.clone()));
                                        match local_bytes {
                                            Some(local) if local == ref_bytes => { matched += 1; }
                                            Some(local) => {
                                                divergent += 1;
                                                ...
                                                let first_diff = local.iter().zip(ref_bytes.iter())
                                                    .position(|(a, b)| a != b)
                                                ...
                                                let end = (first_diff + 16).min(local.len()).min(ref_bytes.len());
                                                error!("  local   [{}..{}]: {}", start, end, hex::encode_upper(&local[start..end]));
                                                error!("  expected[{}..{}]: {}", start, end, hex::encode_upper(&ref_bytes[start..end]));
                                            }
                                            None => {
                                                local_missing += 1;
                                                error!("KEY MISSING LOCALLY: key={}", hex::encode_upper(&key[..8]));
                                            }
                                        }
                                    } else {
                                        not_in_ref += 1;
                                ...
                                let local_deletes: Vec<crate::ledger::Key> = post_state
                                    ...
                                    .collect();
                                let authoritative_overlay_hash = {
                                    let ls = ledger_state.lock().unwrap_or_else(|e| e.into_inner());
                ...
                follower_state.hash_mismatches.fetch_add(1, Ordering::Relaxed);
                ...
                if successful_since_sync == 0 {
                    ...
                    follower_state.resync_requested.store(true, Ordering::SeqCst);
                    follower_state.running.store(false, Ordering::SeqCst);
                    return;
                }
            }
            ...
            let meta_t0 = std::time::Instant::now();
            {
                ...
                ss.ctx.history.write().unwrap_or_else(|e| e.into_inner()).insert_ledger(
            ...
            follower_state.ledgers_applied.fetch_add(1, Ordering::Relaxed);
            follower_state.txs_applied.fetch_add(replay_result.applied_count as u64, Ordering::Relaxed);
            if matched {
            ...
            for (hash, seq, needs_header, needs_tx) in &retries {
                ...
                };
                {
                    ...
                    if *needs_tx {
                        ...
                        let req = crate::network::relay::encode_get_ledger_txs_for_hash(hash, cookie);
                ...
                info!(
                    ...
                    what, seq, &hex::encode_upper(&hash[..4]), retry_num,
                );
            }
        }
...
/// Apply transaction metadata (AffectedNodes) to our LedgerState.
///
...
fn apply_modified_node(node: &Value, state: &mut crate::ledger::LedgerState) {
    ...
    if final_fields.is_null() { return; }
...
fn apply_deleted_node(node: &Value, state: &mut crate::ledger::LedgerState) {
    ...
    // For deletion we need the SHAMap key, which is the LedgerIndex
    ...
    match entry_type {
        ...
        "RippleState" => {
            if let Some(k) = key { state.remove_trustline(&k); }
        }
        "Offer" => {
            if let Some(k) = key { state.remove_offer(&k); }
        }
        "Check" => {
            if let Some(k) = key { state.remove_check(&k); }
        }
        "Escrow" => {
            if let Some(k) = key { state.remove_escrow(&k); }
        }
        "PayChannel" => {
            if let Some(k) = key { state.remove_paychan(&k); }
        }
        "DepositPreauth" => {
            if let Some(k) = key { state.remove_deposit_preauth(&k); }
        }
        "Ticket" => {
            if let Some(k) = key { state.remove_ticket(&k); }
        }
        "NFTokenOffer" => {
            if let Some(k) = key { state.remove_nft_offer(&k); }
        }
        _ => {}
    }
}
...
fn parse_account_from_json(fields: &Value) -> Option<crate::ledger::AccountRoot> {
    ...
    let domain = fields["Domain"].as_str()
...
fn parse_trustline_from_json(fields: &Value) -> Option<crate::ledger::RippleState> {
    ...
    let currency_str = low_limit["currency"].as_str()
    ...
    let low_node = fields["LowNode"].as_str()
        .and_then(|s| u64::from_str_radix(s, 16).ok()).unwrap_or(0);
    let high_node = fields["HighNode"].as_str()
        .and_then(|s| u64::from_str_radix(s, 16).ok()).unwrap_or(0);
...
fn parse_offer_from_json(fields: &Value) -> Option<crate::ledger::Offer> {
    ...
    let book_node = fields["BookNode"].as_str()
        .and_then(|s| u64::from_str_radix(s, 16).ok()).unwrap_or(0);
    let owner_node = fields["OwnerNode"].as_str()
        .and_then(|s| u64::from_str_radix(s, 16).ok()).unwrap_or(0);
    ...
    Some(crate::ledger::Offer {
        account, sequence, taker_pays, taker_gets, flags,
        book_directory, book_node, owner_node, expiration, domain_id, additional_books,
...
fn parse_check_from_json(fields: &Value) -> Option<crate::ledger::Check> {
    ...
    Some(crate::ledger::Check {
        account, destination, send_max, sequence, expiration,
        owner_node: 0, destination_node: 0,
        source_tag: None, destination_tag: None,
            raw_sle: None,
    })
}
...
fn parse_escrow_from_json(fields: &Value) -> Option<crate::ledger::Escrow> {
    ...
    Some(crate::ledger::Escrow {
        account, destination, amount, sequence, finish_after, cancel_after,
        held_amount,
        condition: None,
        owner_node: 0, destination_node: 0,
        source_tag: None, destination_tag: None,
...
fn parse_paychan_from_json(fields: &Value) -> Option<crate::ledger::PayChannel> {
    ...
    let cancel_after = fields["CancelAfter"].as_u64().unwrap_or(0) as u32;
    let expiration = fields["Expiration"].as_u64().unwrap_or(0) as u32;
    let public_key = fields["PublicKey"].as_str()
        .and_then(|s| hex::decode(s).ok())
        .unwrap_or_default();

    Some(crate::ledger::PayChannel {
        account, destination, amount, balance, settle_delay,
        public_key, sequence, cancel_after, expiration,
        owner_node: 0, destination_node: 0,
        source_tag: None, destination_tag: None,
            raw_sle: None,
    })
...
fn parse_ticket_from_json(fields: &Value) -> Option<crate::ledger::Ticket> {
    let account = parse_account_id_from_json(fields, "Account")?;
    let sequence = fields["TicketSequence"].as_u64()
...
fn parse_nft_offer_from_json(fields: &Value) -> Option<crate::ledger::NFTokenOffer> {
    ...
    Some(crate::ledger::NFTokenOffer {
        account, sequence, nftoken_id, amount, destination, expiration, flags,
        owner_node: 0,
        nft_offer_node: 0,
        previous_txn_id: [0u8; 32],
        previous_txn_lgrseq: 0,
...
fn parse_amount_json(val: &Value) -> Option<crate::transaction::Amount> {
    ...
    if let Some(s) = val.as_str() {
        ...
        Some(Amount::Iou { value, currency, issuer })
    } else {
        None
    }
}
...
async fn fetch_sle_binary(local_host: &str, local_port: u16, req: &str) -> Option<Vec<u8>> {
    ...
    // Round-robin across public servers with multi-server fallback.
    // A single public RPC miss can be transient or server-specific.
...
async fn compare_engine_touched_keys(
    ...
    info!(
        ...
        seq, fetched.len(), touched_keys.len(),
    );
...
async fn reconcile_touched_keys_with_rpc(
        ...
        std::collections::HashMap::new();
    {
        ...
        while next_idx < touched_keys.len() || in_flight > 0 {
            ...
            let Some(res) = tasks.join_next().await else { break; };
            in_flight = in_flight.saturating_sub(1);
            if let Ok((key, remote)) = res {
                fetched.insert(key, remote);
            }
    ...
    let mut ls = ledger_state.lock().unwrap_or_else(|e| e.into_inner());
    for key in touched_keys {
        match fetched.get(key).and_then(|v| v.clone()) {
            Some(raw) => {
                ls.insert_raw(*key, raw.clone());
                if let Some(sle) = crate::ledger::sle::SLE::from_raw(*key, raw.clone()) {
                    sync_typed(&mut ls, sle.entry_type() as u16, key, &raw);
    ...
    info!(
        "follower: touched-key RPC reconcile seq={}: upserted={} removed={} missing={} touched={}",
        seq, upserted, removed, missing, touched_keys.len(),
    );
    (upserted, removed, missing)
}
...
async fn reconcile_metadata_keys_with_rpc(
    ...
    info!(
        "follower: metadata-key RPC reconcile seq={}: upserted={} removed={} missing={} metadata_keys={}",
        seq, upserted, removed, missing, keys.len(),
    );
    (upserted, removed, missing, keys.len())
}
...
/// Test mode: load state from storage, apply ONE ledger, check hash, exit.
/// No node startup, no peers, no following. Just one hash check.
pub async fn test_one_ledger(
    ...
    if leaf_cache.exists() {
        info!("loading cached leaf hashes from {}...", leaf_cache.display());
        ...
        info!("loaded {} leaf hashes in {:.1}s", count, start.elapsed().as_secs_f64());
    } else {
    ...
    let (resp, account_hash_hex) = if let Some(ref path) = cache_path {
        ...
        let ah = resp["result"]["ledger"]["account_hash"].as_str()
            .unwrap_or("").to_string();
        if ah.is_empty() {
            // Need to fetch account_hash separately
            let hdr_req = format!(
                r#"{{"method":"ledger","params":[{{"ledger_index":{}}}]}}"#, seq
            );
            let ah2 = if let Ok(body) = crate::rpc_sync::http_post(rpc_host, rpc_port, &hdr_req).await {
                serde_json::from_str::<Value>(&body).ok()
                    .and_then(|v| v["result"]["ledger"]["account_hash"].as_str().map(|s| s.to_string()))
                    .unwrap_or_default()
            } else { String::new() };
            (resp, ah2)
        } else {
        ...
        // Verify initial hash
        let (_, net_hash) = fetch_ledger_binary(rpc_host, rpc_port, sync_seq).await
            .unwrap_or_default();
        ...
        match fetch_ledger_binary(rpc_host, rpc_port, seq).await {
            Some(r) => r,
            None => { error!("could not fetch ledger {}", seq); return Ok(()); }
        }
    };
    info!("network account_hash for {}: {}", seq, account_hash_hex);

    let txs = resp["result"]["ledger"]["transactions"].as_array()
        .map(|a| a.to_vec())
        .unwrap_or_default();
    info!("transactions: {}", txs.len());

    // Parse metadata and collect operations
    let mut created_nodes: Vec<(crate::ledger::Key, crate::ledger::meta::AffectedNode, Option<[u8; 32]>, u32)> = Vec::new();
    let mut created_keys: Vec<crate::ledger::Key> = Vec::new();
    let mut modifies: Vec<(crate::ledger::Key, crate::ledger::meta::AffectedNode, Option<[u8; 32]>, u32)> = Vec::new();
    let mut deletes: Vec<crate::ledger::Key> = Vec::new();
    let mut touched_entry_types: std::collections::HashMap<crate::ledger::Key, u16> = std::collections::HashMap::new();
    let mut total_nodes = 0u64;

    for tx in &txs {
        let meta_hex = match tx["meta"].as_str() { Some(s) => s, None => continue };
        let meta_bytes = match hex::decode(meta_hex) { Ok(b) => b, Err(_) => continue };
        let tx_hash: Option<[u8; 32]> = tx["tx_blob"].as_str()
        ...
        let nodes = match std::panic::catch_unwind(|| {
            crate::ledger::meta::parse_metadata(&meta_bytes)
        }) {
            Ok(n) => n,
            Err(_) => { warn!("metadata parse panic"); continue; }
        };
        ...
        for node in nodes {
            ...
            match node.action {
                ...
                crate::ledger::meta::Action::Modified => { modifies.push((key, node, tx_hash, seq)); }
                crate::ledger::meta::Action::Deleted => { deletes.push(key); }
            }
        }
    }

    info!("parsed {} nodes: {} creates, {} modifies, {} deletes",
        total_nodes, created_keys.len(), modifies.len(), deletes.len());
    ...
    if creates_cache.exists() {
        ...
        info!("fetched {} / {} — saving cache", created_sles.len(), created_keys.len());
        ...
        match created_map.get(key) {
            ...
            Some(actual) => {
                ...
                if let (Some(bp), Some(ap)) = (built_parsed, actual_parsed) {
                    let built_keys: std::collections::HashSet<(u16, u16)> =
                        bp.fields.iter().map(|f| (f.type_code, f.field_code)).collect();
                    let actual_keys: std::collections::HashSet<(u16, u16)> =
                        ap.fields.iter().map(|f| (f.type_code, f.field_code)).collect();
    ...
    if !skip_modifies {
        ...
        let mut key_counts: std::collections::HashMap<crate::ledger::Key, usize> = std::collections::HashMap::new();
        for (key, _, _, _) in &modifies {
            *key_counts.entry(*key).or_insert(0) += 1;
        }
        let multi_keys: Vec<crate::ledger::Key> = key_counts.iter()
            .filter(|(_, c)| **c > 1)
            .map(|(k, _)| *k)
            .collect();
        info!("applying {} modifies ({} unique, {} multi-modified → fetching from RPC)",
            modifies.len(), key_counts.len(), multi_keys.len());

        // Fetch final state for multi-modified objects from ledger_entry
        let mut multi_sles: std::collections::HashMap<crate::ledger::Key, Vec<u8>> = std::collections::HashMap::new();
        ...
        let mut patched_cache: std::collections::HashMap<crate::ledger::Key, Vec<u8>> = std::collections::HashMap::new();
        let mut applied: std::collections::HashSet<crate::ledger::Key> = std::collections::HashSet::new();
        for (key, node, tx_hash, ledger_seq) in &modifies {
            if multi_sles.contains_key(key) {
                ...
                if !applied.insert(*key) { continue; }
            ...
            if let Some(existing_data) = existing {
                ...
                let patched = crate::ledger::meta::patch_sle(
                    &existing_data, &node.fields, *tx_hash, Some(*ledger_seq), &deleted,
                );
    ...
    if hash_match {
        ...
        let mut touched_keys: std::collections::BTreeSet<crate::ledger::Key> = std::collections::BTreeSet::new();
        ...
        for key in touched_keys {
            ...
            let kind = match (&local, &remote) {
                (Some(l), Some(r)) if l == r => { exact += 1; "match" }
                (Some(_), Some(_)) => { mismatched += 1; "mismatch" }
                (Some(_), None) => { local_only += 1; "local_only" }
                (None, Some(_)) => { remote_only += 1; "remote_only" }
                (None, None) => { missing_both += 1; "missing_both" }
            };
            if kind != "match" {
...
mod tests {
    ...
    fn header(seq: u32, parent_hash: [u8; 32], account_hash: [u8; 32]) -> crate::ledger::LedgerHeader {
    ...
    fn sync_anchor_validation_checks_sequence_hash_and_account_hash() {
        ...
        assert!(validate_sync_anchor_header(10, Some(hdr.hash), Some(hdr.account_hash), &hdr));
        assert!(!validate_sync_anchor_header(11, Some(hdr.hash), Some(hdr.account_hash), &hdr));
        assert!(!validate_sync_anchor_header(10, Some([0x11; 32]), Some(hdr.account_hash), &hdr));
        assert!(!validate_sync_anchor_header(10, Some(hdr.hash), Some([0x22; 32]), &hdr));
    }
    ...
    fn restart_reads_latest_persisted_anchor_after_follow_progress() {
        ...
        storage.save_meta(followed.sequence, &hex::encode_upper(followed.hash), &followed).unwrap();
    ...
    fn validated_reacquired_sync_header_accepts_matching_inbound_header() {
        ...
        assert!(inbound.got_header(&hdr.hash, hdr.clone()) || inbound.hash_for_seq(hdr.sequence).is_some());
