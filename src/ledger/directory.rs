    ...
    pub fn decode(data: &[u8], key: [u8; 32]) -> Result<Self, DirectoryDecodeError> {
        ...
        while pos < data.len() {
            ...
            match (type_code, field_code) {
                ...
                (2, 5) => {
                    if pos + 4 > data.len() { break; }
                    previous_txn_lgr_seq = Some(u32::from_be_bytes(
                ...
                (3, 2) => {
                    if pos + 8 > data.len() { break; }
                    index_previous = u64::from_be_bytes(
                        data[pos..pos + 8].try_into().unwrap(),
                    );
                ...
                (3, 6) => {
                    if pos + 8 > data.len() { break; }
                    exchange_rate = Some(u64::from_be_bytes(
                        data[pos..pos + 8].try_into().unwrap(),
                    ));
                    pos += 8;
                }
                ...
                // Unknown type — we can't determine size, bail out
                _ => {
                    break;
                }
            }
...
pub fn dir_add(state: &mut LedgerState, owner: &[u8; 20], entry_key: [u8; 32]) -> u64 {
    ...
    // Safe: we just checked root exists at line 576. Clone it for mutation.
    ...
    let new_page_num = if last_page_num == 0 {
        // Root is full; root's IndexNext tells us the next page, but if it's 0
        // there are no other pages yet, so new page = 1
        if root.index_next == 0 { 1 } else {
...
pub fn dir_remove_root(state: &mut LedgerState, root_key: &Key, entry_key: &[u8; 32]) -> bool {
    ...
    loop {
        ...
        if let Some(idx) = page.indexes.iter().position(|k| k == entry_key) {
            ...
            // next_page_num == 0 means we point back to root
...
pub fn owner_dir_contains_entry(state: &LedgerState, owner: &[u8; 20], entry_key: &[u8; 32]) -> bool {
    let root_key = owner_dir_key(owner);
    let Some(_) = load_directory_fresh(state, &root_key) else {
        return false;
    };
