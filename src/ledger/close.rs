...
use crate::ledger::tx::{run_tx, classify_result, ApplyOutcome};
...
fn stamp_touched_previous_fields(
    ...
    for (key, before) in touched {
        ...
        match crate::ledger::sle::SLE::from_raw(*key, updated_raw.clone())
            .map(|s| s.entry_type())
        {
    ...
    match result {
        ...
        other => {
            ...
            crate::ledger::ter::token_to_code(other)
                .map(|r| r.code())
                .unwrap_or_else(|| {
                    warn!("close_ledger: unknown TER {}, defaulting metadata code to 0", other);
                    0
                })
        }
    }
    ...
    for (key, before_opt) in touched {
        ...
        match (before_opt.as_deref(), after_opt.as_deref()) {
            ...
            (Some(before), Some(after)) => {
                let Some(pre) = crate::ledger::meta::parse_sle(before) else { continue; };
                let Some(post) = crate::ledger::meta::parse_sle(after) else { continue; };
                let previous_fields = crate::ledger::meta::diff_previous_fields(&pre.fields, &post.fields);
...
fn round_close_time(close_time: u64, resolution: u8) -> u64 {
    if resolution == 0 { return close_time; }
...
fn next_close_time_resolution(previous_resolution: u8, previous_agree: bool, ledger_seq: u32) -> u8 {
...
fn build_skip_list_sle(
    ...
    // LedgerHashes SLEs are raw-only objects in this codebase, so we build
    ...
    // Public/mainnet LedgerHashes uses sfHashes at (19, 2). We still read the
...
impl CanonicalEntry {
    fn new(account: &[u8; 20], sequence: u32, hash: [u8; 32], blob: Vec<u8>, salt: &[u8; 32]) -> Self {
        ...
        Self { salted_account: salted, sequence, hash, blob }
    }
}

impl Ord for CanonicalEntry {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.salted_account.cmp(&other.salted_account)
...
pub fn close_ledger(
    ...
    // We derive it from all tx hashes in the set.
    let salt: [u8; 32] = {
    ...
    for entry in entries {
        ...
        txns.push(CanonicalEntry::new(
            &parsed.account, parsed.sequence, entry.hash, entry.blob, &salt,
        ));
    }
    txns.sort();
    ...
    for pass in 0..LEDGER_TOTAL_PASSES {
        let flags = if certain_retry { ApplyFlags::RETRY } else { ApplyFlags::NONE };
    ...
    let header = LedgerHeader {
        ...
        close_flags:           if have_close_time_consensus { 0 } else { CLOSE_TIME_NO_CONSENSUS },
    };

    let hash = header.compute_hash();
    let header = LedgerHeader { hash, ..header };

    CloseResult { header, tx_records, applied_count, failed_count, skipped_count }
}
...
/// Replay a validated ledger's transactions against our state.
///
...
/// We parse each, sort by sfTransactionIndex, apply via apply_tx,
...
pub fn replay_ledger(
    ...
    for (blob, meta) in tx_blobs_with_meta {
        ...
        let tx_index = match tx_index_opt {
            Some(idx) => idx,
            None => {
                warn!("replay_ledger: tx {} has no sfTransactionIndex, skipping",
        ...
        replay_txs.push(ReplayTx { blob, meta, tx_index, tx_id });
    }
    ...
    for rtx in &replay_txs {
        let parsed = match parse_blob(&rtx.blob) {
            Ok(p) => p,
            Err(e) => {
                warn!("replay_ledger: failed to parse tx {}: {:?}",
                    hex::encode_upper(&rtx.tx_id[..4]), e);
                skipped_count += 1;
                continue;
            }
        };
        ...
        if byte_diff {
            let created_keys: Vec<[u8; 32]> = result.touched.iter()
                .filter(|(_, before)| before.is_none())
                .map(|(k, _)| k.0)
                .collect();
            let modified_keys: Vec<[u8; 32]> = result.touched.iter()
    ...
    //
    // total_coins: use the validated header's value directly. Since we're
    // replaying a validated ledger, we know the correct total_coins.
    // This avoids double-counting fees (apply_tx deducts from account
    // balance; we must not also subtract from total_coins separately).
    let header = LedgerHeader {
        sequence:              new_seq,
        hash:                  [0u8; 32],
        parent_hash:           prev.hash,
    ...
    // Diagnostic: compare our computed fees against validated header delta
    ...
    if computed_coins != validated_header.total_coins {
        debug!(
            ...
            computed_coins, validated_header.total_coins,
...
///   - Leaf type 0x00 (raw tx, no metadata) → skip (we need metadata)
///
/// Returns Vec<(tx_blob, meta_blob)> pairs.
pub fn extract_tx_blobs_from_tx_tree(nodes: &[crate::proto::TmLedgerNode]) -> Vec<(Vec<u8>, Vec<u8>)> {
    ...
    for node in nodes {
        ...
        if wire_type != 0x04 { continue; } // only wireTypeTransactionWithMeta
...
mod tests {
    ...
    fn test_close_with_one_payment() {
        ...
        let (tx_index, nodes) = crate::ledger::meta::parse_metadata_with_index(&result.tx_records[0].meta);
    ...
    fn test_tx_hash_changes_with_different_txs() {
        ...
        assert_ne!(
            close1.header.transaction_hash,
            close2.header.transaction_hash,
    ...
    fn test_close_flags_set_when_no_consensus() {
        ...
        assert_eq!(r2.header.close_flags & CLOSE_TIME_NO_CONSENSUS, CLOSE_TIME_NO_CONSENSUS);
    }
    ...
    fn test_replay_ledger_fee_matches_close() {
        ...
        let tx_blobs_with_meta: Vec<(Vec<u8>, Vec<u8>)> = close_result.tx_records.iter()
        ...
        assert_eq!(
            replay_result.header.account_hash, close_result.header.account_hash,
    ...
    fn test_close_creates_short_skip_list() {
        ...
        let raw = state.get_raw_owned(&key).expect("short skip list should exist");
    ...
    fn test_close_migrates_legacy_short_skip_list_encoding() {
        ...
        let raw = state.get_raw_owned(&short_key).expect("short skip list should exist");
    ...
    fn diag_close_vs_replay_state_diff() {
        ...
        let common_diff: Vec<(&[u8; 32], &Vec<u8>, &Vec<u8>)> = close_map
            .iter()
            .filter_map(|(k, a)| {
                replay_map.get(k).and_then(|b| if a != b { Some((k, a, b)) } else { None })
            })
            .collect();
    ...
    /// paths, the printed values will all match, and we'll know.
    #[test]
    ...
    fn test_forensic_bundle_round_trip() {
        ...
        rt.block_on(async {
            ...
            // is cheap. In the real follower path, we collect only metadata-
            // affected keys, but for the round-trip validator we want to
            ...
            let (bundle_dir, _rippled_ref) = crate::ledger::forensic::capture_forensic_bundle(inputs)
                .await
            ...
            assert_eq!(
                replay_result.header.account_hash, expected_account_hash,
