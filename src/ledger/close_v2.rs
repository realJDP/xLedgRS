...
use crate::ledger::views::{ReadView, ApplyFlags};
...
pub fn close_ledger_v2(
    ...
    let salt: [u8; 32] = {
        ...
        for e in &entries { data.extend_from_slice(&e.hash); }
        if data.is_empty() { [0u8; 32] } else { sha512_first_half(&data) }
    };
    ...
    for pass in 0..LEDGER_TOTAL_PASSES {
        ...
        for entry in txns {
            if failed_set.contains(&entry.hash) { continue; }
            ...
            let result = transact::apply_transaction(
                &mut open, &parsed, &tx_hash, handler.as_ref(), flags,
            );
        ...
        if changes == 0 && !certain_retry { break; }
    ...
    let state_changes: Vec<(crate::ledger::Key, StateChangeAction, Option<Vec<u8>>)> = {
        ...
        open.state_table().iter().map(|(key, (action, sle_opt))| {
    ...
    let resolution = next_close_time_resolution(prev_resolution, have_close_time_consensus, new_seq);
    let effective_time = effective_close_time(close_time, resolution, prev_close_time);
    let close_flags = if !have_close_time_consensus { 0x01u8 } else { 0x00u8 };
...
fn build_metadata(
    ...
    use crate::ledger::meta::{AffectedNode, Action, encode_metadata};
        ...
        let previous_fields = if node.action == AffectedAction::Modified {
            if let (Some(ref before), Some(ref after)) = (&node.before, &node.after) {
                let pre = crate::ledger::meta::parse_sle(before)
                    .map(|p| p.fields).unwrap_or_default();
...
fn update_skip_list(
    ...
    use crate::ledger::views::{ReadView, RawView};
    ...
    if (prev_seq & 0xFF) == 0 && prev_seq > 0 {
        ...
        let sle = std::sync::Arc::new(SLE::new(long_kl.key, LedgerEntryType::LedgerHashes, sle_data));
        if hashes.len() == 1 {
            open.raw_insert(sle);
        } else {
    ...
    let sle_data = build_skip_list_sle(&hashes, prev_seq);
    let sle = std::sync::Arc::new(SLE::new(short_kl.key, LedgerEntryType::LedgerHashes, sle_data));
    if exists {
        open.raw_replace(sle);
    } else {
        open.raw_insert(sle);
...
fn is_permanent_failure(ter: &transact::TER) -> bool {
    matches!(ter, transact::TER::Malformed(_) | transact::TER::LocalFail(_))
}
...
fn next_close_time_resolution(prev: u8, agreed: bool, seq: u32) -> u8 {
    ...
    if !agreed && seq % 1 == 0 {
        if let Some(&next) = POSSIBLE_RESOLUTIONS.get(idx + 1) { return next; }
    }
...
fn effective_close_time(close_time: u64, resolution: u8, prior: u64) -> u64 {
    ...
    let rounded = if res > 0 { (close_time + res / 2) / res * res } else { close_time };
    rounded.max(prior + 1)
}
...
mod tests {
    ...
    fn test_skip_list_updated() {
        ...
        let has_skip = result.state_changes.iter()
