//! RPC-based state sync — download full ledger state from a rippled admin RPC.
//!
//! Instead of syncing via the peer protocol (which rate-limits), this module
//! fetches the entire account state via `ledger_data` RPC calls with binary
//! mode and marker-based pagination.  A single rippled node with admin access
//! can serve the entire state without disconnecting us.
...
/// Number of objects per RPC page.
const PAGE_LIMIT: u32 = 2048;

/// Progress counters visible to the rest of the node.
...
/// `ledger_data` and stores them in the given Storage.
pub async fn run_rpc_sync(
    ...
    let mut pages: u64 = 0;
    // Use a specific validated ledger so state is consistent across pages
    ...
    loop {
        ...
        if result["status"].as_str() != Some("success") {
            let err = result["error_message"].as_str()
                .or(result["error"].as_str())
                .unwrap_or("unknown");
            error!("RPC sync error: {err} — retrying in 10s");
            tokio::time::sleep(std::time::Duration::from_secs(10)).await;
            // Reset marker on error — start over
            marker = None;
            ledger_index = None;
            continue;
        }

        // Lock to the specific ledger index on first page
        ...
        // Parse binary objects: each has "index" (hex key) and "data" (hex STObject)
        ...
        // Leaf persistence is now handled by SHAMap/NuDB — just track counts.
        {
            total_objects += page_count;
            total_accounts += leaves.len() as u64;
            pages += 1;

            sync_state.objects_downloaded.store(total_objects, Ordering::Relaxed);
            sync_state.accounts_parsed.store(total_accounts, Ordering::Relaxed);
    ...
    // Persist the exact synced ledger header so follower startup has a real anchor.
    if let Some(seq) = ledger_index {
...
/// Minimal HTTP POST client using raw TCP — no external deps needed.
...
pub async fn http_post(host: &str, port: u16, body: &str) -> anyhow::Result<String> {
    ...
    // Read full response with timeout (rippled can stall on large pages)
...
mod tests {
    ...
    fn test_parse_ledger_header_from_result_rejects_hash_mismatch() {
        ...
        let err = parse_ledger_header_from_result(&result).unwrap_err().to_string();
