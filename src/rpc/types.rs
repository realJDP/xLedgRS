//! JSON-RPC envelope types — request, response, and error.
...
impl RpcRequest {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, serde_json::Error> {
        ...
        // Unwrap rippled-style params array: [{"key": "val"}] → {"key": "val"}
...
/// A JSON-RPC response matching rippled's envelope format.
...
impl RpcResponse {
    pub fn ok(mut result: Value, id: Value) -> Self {
        // rippled always includes "status": "success" inside result
        if let Value::Object(ref mut map) = result {
            map.entry("status").or_insert_with(|| Value::String("success".into()));
        }
        Self { result, id }
    }
