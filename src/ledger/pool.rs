...
impl FeeMetrics {
    ...
    pub fn escalated_fee_level(&self, current: u64) -> u64 {
        ...
        let num = self.escalation_multiplier as u128
            * current as u128
            * current as u128;
    ...
    pub fn fee_level_to_drops(fee_level: u64, base_fee: u64) -> u64 {
        ((fee_level as u128 * base_fee as u128 + BASE_LEVEL as u128 - 1)
            / BASE_LEVEL as u128)
    ...
    pub fn fee_level_from_drops(fee_drops: u64, base_fee: u64) -> u64 {
        ...
        ((fee_drops as u128 * BASE_LEVEL as u128) / base_fee as u128)
            .min(u64::MAX as u128) as u64
    }
}
...
impl TxPool {
    ...
    pub fn insert(&mut self, hash: [u8; 32], blob: Vec<u8>, parsed: &ParsedTx) -> bool {
        ...
        // Per-account limit
        let acct_count = self.account_counts.get(&parsed.account).copied().unwrap_or(0);
        if acct_count >= MAX_TXN_PER_ACCOUNT {
            return false;
        }

        self.entries.insert(hash, PoolEntry {
            hash,
            blob,
            parsed: PoolTxInfo {
    ...
    fn evict_lowest_fee(&mut self) {
        if let Some((&worst_hash, _)) = self.entries.iter()
            .min_by_key(|(_, e)| e.parsed.fee)
        {
...
mod tests {
    ...
    fn test_fee_escalation_below_target() {
        let m = FeeMetrics { escalation_multiplier: 128_000, txns_expected: 32 };
    ...
    fn test_metrics_update_healthy() {
        let mut m = FeeMetrics { escalation_multiplier: 128_000, txns_expected: 32 };
    ...
    fn test_metrics_update_slow() {
        let mut m = FeeMetrics { escalation_multiplier: 128_000, txns_expected: 64 };
        m.update(100, true);
