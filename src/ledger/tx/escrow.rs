...
fn apply_mpt_escrow_finish(
    ...
    if escrow.destination == issuer {
        ...
        let dest_raw =
            match ensure_mpt_holder(state, &issuance_key, &escrow.destination, &mptid, require_auth)
            {
                Ok(raw) => raw,
                Err(err) => return err,
            };
...
pub(crate) fn apply_escrow_create(
    ...
    if let Some(drops) = xrp_amount {
    ...
    } else if let Some(Amount::Iou { value, currency, issuer }) = &tx.amount {
        ...
        let mut tl = super::ripple_calc::load_or_create_trustline(state, &key, &tx.account, issuer, currency);
    ...
    let escrow = crate::ledger::Escrow {
        ...
        held_amount: tx.amount.clone().filter(|amount| !matches!(amount, Amount::Xrp(_))),
        sequence,
...
mod tests {
    ...
    fn holder_raw(holder: &[u8; 20], mptid: &[u8; 24], amount: u64, locked: u64, flags: u32) -> Vec<u8> {
        let issuance_key = mpt_issuance_key(mptid);
    ...
    fn escrow_create_locks_mpt_balance() {
        ...
        assert_eq!(escrow.held_amount.as_ref().and_then(Amount::mpt_parts), Some((40, mptid)));
    }
    ...
    fn escrow_finish_moves_mpt_to_destination() {
        ...
        state.insert_raw(issuance_key, patch_uint64_fields(&issuance_raw, &[(25, 40)]));
    ...
    fn escrow_cancel_unlocks_mpt_balance_back_to_owner() {
        ...
        state.insert_raw(issuance_key, patch_uint64_fields(&issuance_raw, &[(25, 35)]));
        let owner_key = mptoken_key(&mpt_issuance_key(&mptid).0, &owner);
